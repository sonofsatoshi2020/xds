...
{
    /// <summary>
    /// Checks if transactions match the white-listing criteria. This rule and <see cref="MainNetOutputNotWhitelistedMempoolRule"/> must correspond.
    /// </summary>
    public class MainNetOutputNotWhitelistedRule : PartialValidationConsensusRule
    {
        public override Task RunAsync(RuleContext context)
        {
            ...
            foreach (var transaction in context.ValidationContext.BlockToValidate.Transactions)
            {
                ...
                foreach (var output in transaction.Outputs)
                {
                    ...
                    this.Logger.LogTrace($"(-)[FAIL_{nameof(MainNetOutputNotWhitelistedRule)}]".ToUpperInvariant());
                    MainNetConsensusErrors.OutputNotWhitelisted.Throw();
                }
            }
