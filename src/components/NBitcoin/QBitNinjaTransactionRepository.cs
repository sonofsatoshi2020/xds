...
namespace NBitcoin
{
    public class QBitNinjaTransactionRepository : ITransactionRepository
    {
        ...
        public QBitNinjaTransactionRepository(Network network)
        {
            ...
            var isTest = this.network.NetworkType == NetworkType.Testnet ||
                         this.network.NetworkType == NetworkType.Regtest;
        ...
        public async Task<Transaction> GetAsync(uint256 txId)
        {
            ...
            {
                var tx = await client.GetAsync(this.BaseUri.AbsoluteUri + "transactions/" + txId + "?format=raw")
