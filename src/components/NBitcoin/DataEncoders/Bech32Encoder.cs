...
namespace NBitcoin.DataEncoders
{
    ...
    {
        static readonly byte[] Byteset = Encoders.ASCII.DecodeData("qpzry9x8gf2tvdw0s3jn54khce6mua7l");
        static readonly uint[] Generator = {0x3b6a57b2U, 0x26508e6dU, 0x1ea119faU, 0x3d4233ddU, 0x2a1462b3U};

        readonly byte[] _HrpExpand;

        readonly int[] GF1024_EXP =
        {
        ...
        {
            if (residue == 0) return new int[0];
            var syn = syndrome(residue);
            var s0 = syn & 0x3FF;
            var s1 = (syn >> 10) & 0x3FF;
            var s2 = syn >> 20;
            var l_s0 = this.GF1024_LOG[s0];
            var l_s1 = this.GF1024_LOG[s1];
            var l_s2 = this.GF1024_LOG[s2];
            ...
            {
                var s2_s1p1 = s2 ^ (s1 == 0 ? 0 : this.GF1024_EXP[(l_s1 + p1) % 1023]);
                if (s2_s1p1 == 0)
                    continue;
                var s1_s0p1 = s1 ^ (s0 == 0 ? 0 : this.GF1024_EXP[(l_s0 + p1) % 1023]);
                if (s1_s0p1 == 0)
                    continue;
                var l_s1_s0p1 = this.GF1024_LOG[s1_s0p1];
                var p2 = (this.GF1024_LOG[s2_s1p1] - l_s1_s0p1 + 1023) % 1023;
                if (p2 >= length || p1 == p2)
                    continue;
                var s1_s0p2 = s1 ^ (s0 == 0 ? 0 : this.GF1024_EXP[(l_s0 + p2) % 1023]);
                if (s1_s0p2 == 0)
                    continue;
                var inv_p1_p2 = 1023 - this.GF1024_LOG[this.GF1024_EXP[p1] ^ this.GF1024_EXP[p2]];
                var l_e2 = l_s1_s0p1 + inv_p1_p2 + (1023 - 997) * p2;
                if (l_e2 % 33 != 0)
                    continue;
                var l_e1 = this.GF1024_LOG[s1_s0p2] + inv_p1_p2 + (1023 - 997) * p1;
                if (l_e1 % 33 != 0)
                    continue;
                if (p1 < p2)
                    return new[] {p1, p2};
        ...
        static uint Polymod(byte[] values)
        {
            ...
            foreach (var value in values)
            {
                ...
                foreach (var i in Enumerable.Range(0, 5)) chk ^= ((top >> i) & 1) == 1 ? Generator[i] : 0;
            }
        ...
        {
            errorPosition = null;
            var values = this._HrpExpand.Concat(data);
        ...
        byte[] CreateChecksum(byte[] data, int offset, int count)
        {
            ...
            var polymod = Polymod(values) ^ 1;
            var ret = new byte[6];
            foreach (var i in Enumerable.Range(0, 6)) ret[i] = (byte) ((polymod >> (5 * (5 - i))) & 31);
            return ret;
        }
        ...
        {
            var combined = new byte[this.HumanReadablePart.Length + 1 + count + 6];
            var combinedOffset = 0;
            Array.Copy(this.HumanReadablePart, 0, combined, 0, this.HumanReadablePart.Length);
            combinedOffset += this.HumanReadablePart.Length;
            ...
            for (var i = 0; i < count + 6; i++)
                combined[this.HumanReadablePart.Length + 1 + i] =
                    Byteset[combined[this.HumanReadablePart.Length + 1 + i]];
        ...
        public override byte[] DecodeData(string encoded)
        {
            ...
            if (buffer.Any(b => b < 33 || b > 126)) throw new FormatException("bech chars are out of range");
            ...
            if (!hrp.SequenceEqual(this.HumanReadablePart))
        ...
        static byte[] ConvertBits(IEnumerable<byte> data, int fromBits, int toBits, bool pad = true)
        {
            var acc = 0;
            var bits = 0;
            var maxv = (1 << toBits) - 1;
