...
namespace NBitcoin.OpenAsset
{
    public class AssetMoney : IComparable, IComparable<AssetMoney>, IEquatable<AssetMoney>, IMoney
    {
        ...
        public IEnumerable<AssetMoney> Split(int parts)
        {
            ...
            long remain;
            var result = DivRem(this._Quantity, parts, out remain);

            for (var i = 0; i < parts; i++)
            {
                yield return new AssetMoney(this.Id, result + (remain > 0 ? 1 : 0));
                remain--;
            }
        }
        ...
        public decimal ToDecimal(int divisibility)
        {
            ...
            return (decimal) this.Quantity / dec;
        }
        ...
        {
            if (other.Id != this.Id)
                throw new ArgumentException("AssetMoney instance of different assets can't be computed together",
                    param);
        }
