...
namespace NBitcoin.OpenAsset
{
    public class ColorMarker : IBitcoinSerializable
    {
        ...
        const ulong MAX_QUANTITY = (1UL << 63) - 1;
        ...
        public void ReadWrite(BitcoinStream stream)
        {
            ...
            else
            {
                Script script = null;
                stream.ReadWrite(ref script);
                if (!ReadScript(script)) throw new FormatException("Invalid ColorMarker");
            }
        }
        ...
        static ulong ReadLEB128(BitcoinStream stream)
        {
            ulong value = 0;
            value = stream.ReadWrite((byte) 0);
            if ((value & 128uL) == 0uL) return value;
            value &= 127uL;
            ulong chunk = stream.ReadWrite((byte) 0);
            value |= (chunk & 127uL) << 7;
            if ((chunk & 128uL) == 0uL) return value;
            ...
            chunk = stream.ReadWrite((byte) 0);
            value |= chunk << 63;
            if ((chunk & 18446744073709551614uL) != 0uL) throw new FormatException("Invalid LEB128 number");
            return value;
        }

        void WriteLEB128(ulong value, BitcoinStream stream)
        {
        ...
        public Uri GetMetadataUrl()
        {
            if (this.Metadata == null || this.Metadata.Length == 0)
                return null;
            var result = Encoders.ASCII.EncodeData(this.Metadata);
