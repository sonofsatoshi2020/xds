...
namespace NBitcoin
{
    ...
    public class DefaultCoinSelector : ICoinSelector
    {
        ...
        public IEnumerable<ICoin> Select(IEnumerable<ICoin> coins, IMoney target)
        {
            ...
            var orderedCoinGroups = coins
                .GroupBy(c => this.GroupByScriptPubKey ? c.TxOut.ScriptPubKey : new Key().ScriptPubKey)
            ...
            var sortedUsedCoins = result.OrderBy(c => c.Amount).ToList();
            var excess = total.Sub(target);
            for (var i = 0; i < sortedUsedCoins.Count; i++)
            {
    ...
    public class NotEnoughFundsException : Exception
    {
        ...
        static string BuildMessage(string message, string group, IMoney missing)
        {
    ...
    {
        static readonly TxNullDataTemplate _OpReturnTemplate = new TxNullDataTemplate(1024 * 1024);

        readonly List<BuilderGroup> _BuilderGroups = new List<BuilderGroup>();

        Transaction _CompletedTransaction;
        BuilderGroup _CurrentGroup;

        AssetId _IssuedAsset;

        readonly List<Key> _Keys = new List<Key>();

        readonly List<Tuple<PubKey, ECDSASignature>> _KnownSignatures = new List<Tuple<PubKey, ECDSASignature>>();
        ...
        readonly Dictionary<Script, Script> _ScriptPubKeyToRedeem = new Dictionary<Script, Script>();
        ...
        {
            foreach (var coin in coins) this.CurrentGroup.Coins.AddOrReplace(coin.Outpoint, coin);
            return this;
        }
        ...
        public TransactionBuilder Send(Script scriptPubKey, Money amount)
        {
            ...
            if (this.DustPrevention && amount < GetDust(scriptPubKey) &&
                !_OpReturnTemplate.CheckScriptPubKey(scriptPubKey))
            {
                SendFees(amount);
                return this;
            }
        ...
        public TransactionBuilder Shuffle()
        {
            ...
            foreach (var group in this._BuilderGroups)
                group.Shuffle();
            return this;
        }
        ...
        public TransactionBuilder SendAsset(Script scriptPubKey, AssetMoney asset)
        {
            ...
            var builders = this.CurrentGroup.BuildersByAsset.TryGet(asset.Id);
            if (builders == null)
            {
        ...
        void AssertOpReturn(string name)
        {
        ...
        public TransactionBuilder IssueAsset(Script scriptPubKey, AssetMoney asset)
        {
            ...
            this.CurrentGroup.IssuanceBuilders.Add(ctx =>
            {
                ...
                {
                    var issuance = ctx.Group.Coins.Values.OfType<IssuanceCoin>().Where(i => i.AssetId == asset.Id)
                        .FirstOrDefault();
                    if (issuance == null)
                    ...
                    if (issuance.DefinitionUrl != null) marker.SetMetadataUrl(issuance.DefinitionUrl);
                }
        ...
        public TransactionBuilder SendFeesSplit(Money fees)
        {
            ...
            var lastGroup = this.CurrentGroup; //Make sure at least one group exists
            var totalWeight = this._BuilderGroups.Select(b => b.FeeWeight).Sum();
        ...
        public Transaction BuildTransaction(bool sign, SigHash sigHash)
        {
            ...
            foreach (var group in this._BuilderGroups)
            {
                ...
                BuildTransaction(ctx, group, group.Builders, group.Coins.Values.OfType<Coin>().Where(IsEconomical),
                    Money.Zero);
            }

            ctx.Finish();

            if (sign) SignTransactionInPlace(ctx.Transaction, sigHash);
        ...
        IEnumerable<ICoin> BuildTransaction(
            ...
            IMoney zero)
        {
            ...
            var fees = this._TotalFee + ctx.AdditionalFees;
            ...
            for (var i = 0; i < builderList.Count; i++)
                if (builderList[i].Target == this._SubstractFeeBuilder)
                {
                    ...
                    var newTxOut = this._SubstractFeeBuilder._TxOut.Clone();
                    newTxOut.Value -= fees;
            ...
            if (ctx.CoverOnly != null) target = ctx.CoverOnly.Add(ctx.ChangeAmount);

            var unconsumed = coins.Where(c => ctx.ConsumedCoins.All(cc => cc.Outpoint != c.Outpoint));
            var selection = this.CoinSelector.Select(unconsumed, target);
            if (selection == null)
        ...
        {
            var tx = this.Network.CreateTransaction(transaction.ToBytes());
        ...
        public Transaction SignTransactionInPlace(Transaction transaction, SigHash sigHash)
        {
            ...
            foreach (var input in transaction.Inputs.AsIndexedInputs())
            {
                ...
                if (coin != null) Sign(ctx, coin, input);
            }
        ...
        public ICoin FindSignableCoin(IndexedTxIn txIn)
        {
            ...
            if (hash != null)
            {
                var redeem = this._ScriptPubKeyToRedeem.TryGet(coin.TxOut.ScriptPubKey);
                ...
                if (redeem == null)
                {
                    if (hash is WitScriptId)
                        redeem = PayToWitScriptHashTemplate.Instance.ExtractWitScriptParameters(txIn.WitScript,
                            (WitScriptId) hash);
                    if (hash is ScriptId)
                    {
                        var parameters =
                            PayToScriptHashTemplate.Instance.ExtractScriptSigParameters(this.Network, txIn.ScriptSig,
                                (ScriptId) hash);
                        if (parameters != null)
        ...
        public bool Verify(Transaction tx, FeeRate expectedFeeRate, out TransactionPolicyError[] errors)
        {
            if (tx == null)
                throw new ArgumentNullException("tx");
            return Verify(tx,
                expectedFeeRate == null
                    ? null
                    : expectedFeeRate.GetFee(tx, this.Network.Consensus.Options.WitnessScaleFactor), out errors);
        }
        ...
        public TransactionPolicyError[] Check(Transaction tx, FeeRate expectedFeeRate)
        {
            return Check(tx,
                expectedFeeRate == null
                    ? null
                    : expectedFeeRate.GetFee(tx, this.Network.Consensus.Options.WitnessScaleFactor));
        }
        ...
        CoinNotFoundException CoinNotFound(IndexedTxIn txIn)
        {
        ...
        {
            var result = this._BuilderGroups.Select(c => c.Coins.TryGet(outPoint)).FirstOrDefault(r => r != null);
        ...
        ///     If true, returns the size on which fee calculation are based, else returns the physical byte
        ///     size
        /// </param>
        /// <returns></returns>
        public int EstimateSize(Transaction tx, bool virtualSize)
        {
            ...
            var clone = this.Network.CreateTransaction(tx.ToHex());
            clone.Inputs.Clear();
            ...
            return virtualSize
                ? witSize / this.Network.Consensus.Options.WitnessScaleFactor + baseSize
                : witSize + baseSize;
        }

        void EstimateScriptSigSize(ICoin coin, ref int witSize, ref int baseSize)
        {
            if (coin is IColoredCoin)
                coin = ((IColoredCoin) coin).Bearer;
            ...
            if (coin is ScriptCoin scriptCoin)
            {
                ...
                if (p2sh != null)
                {
                    ...
                    if (scriptCoin.RedeemType == RedeemType.WitnessV0) coin = new ScriptCoin(coin, scriptCoin.Redeem);
                }
        ...
        public Money EstimateFees(FeeRate feeRate)
        {
            ...
            var builderCount = this.CurrentGroup.Builders.Count;
        ...
        void Sign(TransactionSigningContext ctx, ICoin coin, IndexedTxIn txIn)
        {
            var input = txIn.TxIn;
            if (coin is StealthCoin)
            {
                ...
                var spendKeys = stealthCoin.Address.SpendPubKeys.Select(p => FindKey(ctx, p.ScriptPubKey))
        ...
        static Script RemoveRedeem(Script script)
        {
        ...
        Script CombineScriptSigs(ICoin coin, Script a, Script b)
        {
        ...
        Script CreateScriptSig(TransactionSigningContext ctx, ICoin coin, IndexedTxIn txIn)
        {
            var scriptPubKey = coin.GetScriptCode(this.Network);
            ...
            foreach (var extension in this.Extensions)
                if (extension.CanGenerateScriptSig(this.Network, scriptPubKey))
                {
                    ...
                    if (scriptSig2 != null) scriptSig2 = signer2.ReplaceDummyKeys(scriptSig2);
                    if (scriptSig1 != null && scriptSig2 != null &&
                        extension.CanCombineScriptSig(this.Network, scriptPubKey, scriptSig1, scriptSig2))
                    {
        ...
        Key FindKey(TransactionSigningContext ctx, Script scriptPubKey)
        {
            var key = this._Keys
                .Concat(ctx.AdditionalKeys)
                .FirstOrDefault(k => IsCompatibleKey(k.PubKey, scriptPubKey));
            if (key == null && this.KeyFinder != null) key = this.KeyFinder(scriptPubKey);
            return key;
        }

        static bool IsCompatibleKey(PubKey k, Script scriptPubKey)
        {
        ...
        {
            var group = this._BuilderGroups.FirstOrDefault(g => g.Name == groupName);
            if (group == null)
            {
        ...
        public TransactionBuilder CoverTheRest()
        {
            ...
            var spent = this._CompletedTransaction.Inputs.AsIndexedInputs().Select(txin =>
                {
            ...
            var toComplete = this._CompletedTransaction.TotalOut - spent;
            this.CurrentGroup.Builders.Add(ctx =>
            {
        ...
        public TransactionBuilder AddKnownRedeems(params Script[] knownRedeems)
        {
            ...
            {
                this._ScriptPubKeyToRedeem.AddOrReplace(redeem.WitHash.ScriptPubKey.Hash.ScriptPubKey,
                    redeem); //Might be P2SH(PWSH)
        ...
        public Transaction CombineSignatures(params Transaction[] transactions)
        {
            ...
            return CombineSignatures(false, transactions);
        }
        ...
        public Transaction CombineSignatures(bool requireFirstSigned, params Transaction[] transactions)
        {
            ...
            var tx = this.Network.CreateTransaction(transactions[0].ToHex());
            for (var i = 1; i < transactions.Length; i++)
            {
                var signed = transactions[i];
                tx = CombineSignaturesCore(tx, signed, requireFirstSigned);
            }
        ...
        Transaction CombineSignaturesCore(Transaction signed1, Transaction signed2, bool requireFirstSigned)
        {
            if (signed1 == null)
                return signed2;
            ...
            var tx = this.Network.CreateTransaction(signed1.ToHex());

            var signed1Inputs = signed1.Inputs.AsIndexedInputs().ToArray();
            var signed2Inputs = signed2.Inputs.AsIndexedInputs().ToArray();
            ...
            for (var i = 0; i < tx.Inputs.Count; i++)
            {
                ...
                if (i == 0 && requireFirstSigned && tx.GetHash() == signed1.GetHash()) return tx;
            }

            return tx;
        }

        ScriptSigs GetScriptSigs(IndexedTxIn indexedTxIn)
        {
        ...
        Script DeduceScriptPubKey(Script scriptSig)
        {
            var p2sh = PayToScriptHashTemplate.Instance.ExtractScriptSigParameters(this.Network, scriptSig);
            if (p2sh != null && p2sh.RedeemScript != null) return p2sh.RedeemScript.Hash.ScriptPubKey;
            foreach (var extension in this.Extensions)
        ...
        {
            readonly TransactionBuilder builder;
            readonly ICoin coin;
            readonly SigHash sigHash;
            readonly IndexedTxIn txIn;
        ...
        {
            readonly TransactionSigningContext _Ctx;
            readonly TransactionBuilder _TxBuilder;
        ...
        {
            readonly Dictionary<uint256, PubKey> _DummyToRealKey = new Dictionary<uint256, PubKey>();
            readonly List<Tuple<PubKey, ECDSASignature>> _KnownSignatures;
            readonly Dictionary<KeyId, ECDSASignature> _VerifiedSignatures = new Dictionary<KeyId, ECDSASignature>();
            readonly TransactionBuilder builder;
            readonly ICoin coin;
            readonly SigHash sigHash;
            readonly IndexedTxIn txIn;

            public KnownSignatureSigner(TransactionBuilder builder,
                List<Tuple<PubKey, ECDSASignature>> _KnownSignatures, ICoin coin, SigHash sigHash, IndexedTxIn txIn)
            {
                this.builder = builder;
            ...
            public Key FindKey(Script scriptPubKey)
            {
                foreach (var tv in this._KnownSignatures.Where(tv => IsCompatibleKey(tv.Item1, scriptPubKey)))
                {
                    var hash = this.txIn.GetSignatureHash(this.builder.Network, this.coin, this.sigHash);
        ...
        internal class TransactionBuildingContext
        {
            ...
            public IssuanceCoin IssuanceCoin { get; set; }
            ...
            TxOut EnsureMarkerInserted()
            {
                uint position;
                var dummy = this.Transaction.AddInput(new TxIn(new OutPoint(new uint256(1),
                    0))); //Since a transaction without input will be considered without marker, insert a dummy
                try
                {
                ...
                var txout = this.Transaction.AddOutput(new TxOut
                {
        ...
        {
            readonly TransactionBuilder _Parent;
            ...
            internal void Shuffle()
            {
                ...
                foreach (var builders in this.BuildersByAsset)
                    Shuffle(builders.Value);
                Shuffle(this.IssuanceBuilders);
            }

            void Shuffle(List<Builder> builders)
            {
