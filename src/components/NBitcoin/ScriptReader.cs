...
namespace NBitcoin
{
    public class Op
    {
        ...
        static readonly string unknown = "OP_UNKNOWN(0x";
        ...
        public bool IsSmallUInt =>
            this.Code == OpcodeType.OP_0 ||
            OpcodeType.OP_1 <= this.Code && this.Code <= OpcodeType.OP_16;

        public bool IsSmallInt => this.IsSmallUInt || this.Code == OpcodeType.OP_1NEGATE;
        ...
        static bool[] GetValidOpCode()
        {
            var valid = new bool[256];
            foreach (var val in Enum.GetValues(typeof(OpcodeType))) valid[(byte) val] = true;
            for (byte i = 0;; i++)
            {
        ...
        void PushDataToStream(byte[] data, Stream result)
        {
        ...
        internal byte[] ReadData(Stream stream)
        {
            ...
            if ((byte) OpcodeType.OP_1 <= (byte) this.Code && (byte) this.Code <= (byte) OpcodeType.OP_16)
                return new[] {(byte) (this.Code - OpcodeType.OP_1 + 1)};

            if (this.Code == OpcodeType.OP_1NEGATE) return new byte[] {0x81};
        ...
        public override string ToString()
        {
            if (this.PushData != null)
            {
                if (this.PushData.Length == 0)
                    return "0";
                var result = Encoders.Hex.EncodeData(this.PushData);
                return result.Length == 2 && result[0] == '0' ? result.Substring(1) : result;
            }

            if (this.Name == "OP_UNKNOWN")
        ...
        public void WriteTo(Stream stream)
        {
            ...
            if (this.PushData != null) PushDataToStream(this.PushData, stream);
        }
        ...
        static string ReadWord(TextReader textReader)
        {
