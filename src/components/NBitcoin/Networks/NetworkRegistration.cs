...
namespace NBitcoin.Networks
{
    ...
    {
        static readonly ConcurrentDictionary<string, Network> registeredNetworks =
            new ConcurrentDictionary<string, Network>();

        /// <summary>
        ///     Register an immutable <see cref="Network" /> instance so it is queryable through <see cref="GetNetwork" /> and
        ///     <see cref="GetNetworks" />.
        ///     <para>
        ///         If the network already exists, the already registered instance will be returned from the
        ///         <see cref="registeredNetworks" /> collection.
        ///     </para>
        /// </summary>
        public static Network Register(Network network)
        {
            var existing = GetNetwork(network.Name);
            if (existing != null)
                return existing;

            var networkNames = network.AdditionalNames != null
                ? new[] {network.Name}.Concat(network.AdditionalNames)
                : new[] {network.Name};
        ...
        public static IEnumerable<Network> GetNetworks()
        {
            if (registeredNetworks.Any())
            {
                var others = registeredNetworks.Values.Distinct().ToList();
