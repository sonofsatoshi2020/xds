...
namespace NBitcoin
{
    ...
    public class TransactionChecker
    {
        ...
        public TxIn Input => this.Transaction.Inputs[this.Index];
    ...
    public class ScriptEvaluationContext
    {
        ...
        static readonly byte[] vchTrue = {1};

        static readonly byte[] vchMaxModOrder =
        {
        ...
        static readonly byte[] vchMaxModHalfOrder =
        {
        ...
        public ContextStack<byte[]> Stack { get; set; } = new ContextStack<byte[]>();
        ...
        public IEnumerable<SignedHash> SignedHashes => this._SignedHashes;
        ...
        public bool Result
        {
            get
            {
                if (this.Stack.Count == 0)
                    return false;
                return CastToBool(this.Stack.Top(-1));
            }
        }
        ...
        {
            var witness = checker.Input.WitScript;
            ...
            if (!EvalScript(scriptSig, checker, 0))
                return false;
            if ((this.ScriptVerify & ScriptVerify.P2SH) != 0) evaluationCopy = Clone();
            ...
            if ((this.ScriptVerify & ScriptVerify.Witness) != 0)
            {
                ...
                if (wit != null)
                {
                    ...
                    if (!VerifyWitnessProgram(witness, wit, checker)) return false;
            ...
            if ((this.ScriptVerify & ScriptVerify.Witness) != 0)
            {
                ...
                if (!hadWitness && witness.PushCount != 0) return SetError(ScriptError.WitnessUnexpected);
            }

            return true;
        }

        bool VerifyWitnessProgram(WitScript witness, WitProgramParameters wit, TransactionChecker checker)
        {
            ...
            if (wit.Version == 0)
            {
                if (wit.Program.Length == 32)
                {
                    ...
                    if (witness.PushCount == 0) return SetError(ScriptError.WitnessProgramEmpty);
                    scriptPubKey = Script.FromBytesUnsafe(witness.GetUnsafePush(witness.PushCount - 1));
                    for (var i = 0; i < witness.PushCount - 1; i++) stack.Add(witness.GetUnsafePush(i));
            ...
            for (var i = 0; i < ctx.Stack.Count; i++)
                if (ctx.Stack.Top(-(i + 1)).Length > MAX_SCRIPT_ELEMENT_SIZE)
                    return SetError(ScriptError.PushSize);
            if (!ctx.EvalScript(scriptPubKey, checker, 1)) return SetError(ctx.Error);
        ...
        bool EvalScript(Script s, TransactionChecker checker, int hashversion)
        {
            ...
            var nOpCount = 0;
            var fRequireMinimal = (this.ScriptVerify & ScriptVerify.MinimalData) != 0;

            try
            {
                Op opcode;

                using (var scriptReader = s.CreateReader())
                {
                    while ((opcode = scriptReader.Read()) != null)
                    {
                        ...
                        if (fExec && 0 <= (int) opcode.Code && (int) opcode.Code <= (int) OpcodeType.OP_PUSHDATA4)
                        {
                            ...
                            this.Stack.Push(opcode.PushData);
                        }
                        ...
                        else if (fExec || OpcodeType.OP_IF <= opcode.Code && opcode.Code <= OpcodeType.OP_ENDIF)
                        {
                            switch (opcode.Code)
                            {
                                ...
                                case OpcodeType.OP_CHECKLOCKTIMEVERIFY:
                                {
                                    ...
                                    var nLockTime = new CScriptNum(this.Stack.Top(-1), fRequireMinimal, 5);
                                ...
                                case OpcodeType.OP_NOTIF:
                                {
                                    ...
                                    if (fExec)
                                    {
                                        ...
                                        var vch = this.Stack.Top(-1);

                                        if (hashversion == (int) HashVersion.Witness &&
                                            (this.ScriptVerify & ScriptVerify.MinimalIf) != 0)
                                        {
                                            if (vch.Length > 1)
                                ...
                                case OpcodeType.OP_VERIFY:
                                {
                                    ...
                                    if (!CastToBool(this.Stack.Top(-1)))
                                ...
                                case OpcodeType.OP_TOALTSTACK:
                                {
                                    ...
                                    altstack.Push(this.Stack.Top(-1));
                                    this.Stack.Pop();
                                    break;
                                }
                                case OpcodeType.OP_FROMALTSTACK:
                                {
                                    ...
                                    this.Stack.Push(altstack.Top(-1));
                                    altstack.Pop();
                                    break;
                                }
                                ...
                                case OpcodeType.OP_2DUP:
                                {
                                    ...
                                    var vch1 = this.Stack.Top(-2);
                                    var vch2 = this.Stack.Top(-1);
                                ...
                                case OpcodeType.OP_3DUP:
                                {
                                    ...
                                    var vch1 = this.Stack.Top(-3);
                                    var vch2 = this.Stack.Top(-2);
                                    var vch3 = this.Stack.Top(-1);
                                    this.Stack.Push(vch1);
                                    this.Stack.Push(vch2);
                                ...
                                case OpcodeType.OP_2OVER:
                                {
                                    ...
                                    var vch1 = this.Stack.Top(-4);
                                    var vch2 = this.Stack.Top(-3);
                                    this.Stack.Push(vch1);
                                    this.Stack.Push(vch2);
                                    break;
                                }
                                case OpcodeType.OP_2ROT:
                                {
                                    ...
                                    if (this.Stack.Count < 6)
                                        return SetError(ScriptError.InvalidStackOperation);

                                    var vch1 = this.Stack.Top(-6);
                                    var vch2 = this.Stack.Top(-5);
                                    this.Stack.Remove(-6, -4);
                                    this.Stack.Push(vch1);
                                    this.Stack.Push(vch2);
                                    break;
                                }
                                ...
                                case OpcodeType.OP_IFDUP:
                                {
                                    ...
                                    var vch = this.Stack.Top(-1);
                                    if (CastToBool(vch)) this.Stack.Push(vch);
                                    break;
                                }
                                case OpcodeType.OP_DEPTH:
                                {
                                    // -- stacksize
                                    var bn = new CScriptNum(this.Stack.Count);
                                    this.Stack.Push(bn.getvch());
                                    break;
                                }
                                ...
                                case OpcodeType.OP_ROLL:
                                {
                                    ...
                                    var n = new CScriptNum(this.Stack.Top(-1), fRequireMinimal).getint();
                                    this.Stack.Pop();
                                    if (n < 0 || n >= this.Stack.Count)
                                        return SetError(ScriptError.InvalidStackOperation);

                                    var vch = this.Stack.Top(-n - 1);
                                    if (opcode.Code == OpcodeType.OP_ROLL) this.Stack.Remove(-n - 1);
                                    this.Stack.Push(vch);
                                    break;
                                }
                                ...
                                case OpcodeType.OP_SIZE:
                                {
                                    ...
                                    var bn = new CScriptNum(this.Stack.Top(-1).Length);
                                    this.Stack.Push(bn.getvch());
                                    break;
                                }
                                //
                                ...
                                case OpcodeType.OP_EQUALVERIFY:
                                {
                                    ...
                                    this.Stack.Pop();
                                    this.Stack.Pop();
                                    this.Stack.Push(fEqual ? vchTrue : vchFalse);
                                ...
                                case OpcodeType.OP_0NOTEQUAL:
                                {
                                    ...
                                    var bn = new CScriptNum(this.Stack.Top(-1), fRequireMinimal);
                                    switch (opcode.Code)
                                    {
                                        case OpcodeType.OP_1ADD:
                                            bn += 1;
                                ...
                                case OpcodeType.OP_MAX:
                                {
                                    ...
                                    var bn1 = new CScriptNum(this.Stack.Top(-2), fRequireMinimal);
                                    var bn2 = new CScriptNum(this.Stack.Top(-1), fRequireMinimal);
                                ...
                                case OpcodeType.OP_WITHIN:
                                {
                                    ...
                                    var bn1 = new CScriptNum(this.Stack.Top(-3), fRequireMinimal);
                                    var bn2 = new CScriptNum(this.Stack.Top(-2), fRequireMinimal);
                                    var bn3 = new CScriptNum(this.Stack.Top(-1), fRequireMinimal);
                                    var fValue = bn2 <= bn1 && bn1 < bn3;
                                    this.Stack.Pop();
                                    this.Stack.Pop();
                                    this.Stack.Pop();
                                    this.Stack.Push(fValue ? vchTrue : vchFalse);
                                    break;
                                }
                                //
                                // Crypto
                                ...
                                case OpcodeType.OP_CHECKSIGVERIFY:
                                {
                                    ...
                                    var vchSig = this.Stack.Top(-2);
                                    var vchPubKey = this.Stack.Top(-1);
                                    ...
                                    var fSuccess = CheckSig(vchSig, vchPubKey, scriptCode, checker, hashversion);
                                    if (!fSuccess && (this.ScriptVerify & ScriptVerify.NullFail) != 0 &&
                                        vchSig.Length != 0)
                                        return SetError(ScriptError.NullFail);

                                    this.Stack.Pop();
                                    this.Stack.Pop();
                                    this.Stack.Push(fSuccess ? vchTrue : vchFalse);
                                ...
                                case OpcodeType.OP_CHECKMULTISIGVERIFY:
                                {
                                    ...
                                    var i = 1;
                                    if (this.Stack.Count < i)
                                        return SetError(ScriptError.InvalidStackOperation);

                                    var nKeysCount = new CScriptNum(this.Stack.Top(-i), fRequireMinimal).getint();
                                    ...
                                    var nSigsCount = new CScriptNum(this.Stack.Top(-i), fRequireMinimal).getint();
                                    ...
                                    {
                                        var vchSig = this.Stack.Top(-isig - k);
                                        if (hashversion == (int) HashVersion.Original)
                                            scriptCode.FindAndDelete(vchSig);
                                    }
                                    ...
                                    {
                                        var vchSig = this.Stack.Top(-isig);
                                        var vchPubKey = this.Stack.Top(-ikey);

                                        // Note how this makes the exact order of pubkey/signature evaluation
                                        // distinguishable by CHECKMULTISIG NOT if the STRICTENC flag is set.
                                        // See the script_(in)valid tests for details.
                                        if (!CheckSignatureEncoding(vchSig) ||
                                            !CheckPubKeyEncoding(vchPubKey, hashversion))
                                            // serror is set
                                            return false;

                                        var fOk = CheckSig(vchSig, vchPubKey, scriptCode, checker, hashversion);
                                    ...
                                    while (i-- > 1)
                                    {
                                        ...
                                        if (!fSuccess && (this.ScriptVerify & ScriptVerify.NullFail) != 0 &&
                                            ikey2 == 0 && this.Stack.Top(-1).Length != 0)
                                            return SetError(ScriptError.NullFail);
                                        if (ikey2 > 0)
                                            ikey2--;
                                        this.Stack.Pop();
                                    ...
                                    if ((this.ScriptVerify & ScriptVerify.NullDummy) != 0 &&
                                        this.Stack.Top(-1).Length != 0)
                        ...
                        // Size limits
                        if (this.Stack.Count + altstack.Count > 1000)
        ...
        bool CheckSequence(CScriptNum nSequence, TransactionChecker checker)
        {
            ...
            var txToSequence = (long) txTo.Inputs[nIn].Sequence;
        ...
        bool SetSuccess(ScriptError scriptError)
        {
            this.Error = ScriptError.OK;
            return true;
        }

        bool SetError(ScriptError scriptError)
        {
        ...
        internal bool CheckSignatureEncoding(byte[] vchSig)
        {
            ...
            if ((this.ScriptVerify & (ScriptVerify.DerSig | ScriptVerify.LowS | ScriptVerify.StrictEnc)) != 0 &&
        ...
        bool CheckPubKeyEncoding(byte[] vchPubKey, int sigversion)
        {
            ...
            if ((this.ScriptVerify & ScriptVerify.WitnessPubkeyType) != 0 && sigversion == (int) HashVersion.Witness &&
                !IsCompressedPubKey(vchPubKey)) return SetError(ScriptError.WitnessPubkeyType);
            return true;
        }
        ...
        {
            if (!IsValidSignatureEncoding(vchSig, haveSigHash)) return false;
            int nLenR = vchSig[3];
            ...
            if (!CheckSignatureElement(vchSig, S, nLenS, true)) return false;
        ...
        {
            if (vchSig.Length == 0) return false;
        ...
        static bool CheckSignatureElement(byte[] vchSig, int i, int len, bool half)
        {
            return vchSig != null
                   &&
        ...
        static int CompareBigEndian(byte[] c1, int ic1, int c1len, byte[] c2, int c2len)
        {
            var ic2 = 0;
            while (c1len > c2len)
            {
        ...
        public bool CheckSig(TransactionSignature signature, PubKey pubKey, Script scriptPubKey,
            TransactionChecker checker, HashVersion hashVersion)
        {
        ...
        bool CheckSig(byte[] vchSig, byte[] vchPubKey, Script scriptCode, TransactionChecker checker, int sigversion)
        {
            ...
            var sighash = Script.SignatureHash(this.Network, scriptCode, checker.Transaction, checker.Index,
                scriptSig.SigHash, checker.Amount, (HashVersion) sigversion, checker.PrecomputedTransactionData);
        ...
        void Load(ScriptEvaluationContext other)
        {
            this.Stack = new ContextStack<byte[]>(other.Stack);
        ...
        public ScriptEvaluationContext Clone()
        {
            ...
            {
                Stack = new ContextStack<byte[]>(this.Stack),
        ...
        class CScriptNum
        {
            ...
            {
                if (vch.Length > nMaxNumSize) throw new ArgumentException("script number overflow", "vch");
    ...
    public class ContextStack<T> : IEnumerable<T>
    {
        ...
        /// </summary>
        public int Count => this._position + 1;
        ...
        public void Swap(int i, int j)
        {
            ...
            var t = this._array[this.Count + i];
        ...
        public void Insert(int position, T value)
        {
            ...
            for (var i = this._position; i >= position + 1; i--) this._array[i + 1] = this._array[i];
        ...
        public void Remove(int from, int to)
        {
            ...
            for (var i = this.Count + from; i < this.Count + from + toRemove; i++)
            for (var y = this.Count + from; y < this.Count; y++)
                this._array[y] = this._array[y + 1];
        ...
        {
            readonly ContextStack<T> _stack;
            int _index;
            ...
            public T Current
            {
                get
                {
                    if (this._index == -1) throw new InvalidOperationException("Enumeration has ended");
