...
namespace NBitcoin.BouncyCastle.math.ec
{
    ...
    abstract class ECCurve
    {
        ...
        public virtual ECPoint ValidatePoint(BigInteger x, BigInteger y)
        {
            ...
            if (!p.IsValid()) throw new ArgumentException("Invalid point coordinates");
            return p;
        }
...
#pragma warning restore
        ...
        protected virtual ECMultiplier CreateDefaultMultiplier()
        {
            ...
            if (glvEndomorphism != null) return new GlvMultiplier(this, glvEndomorphism);
        ...
        public virtual void SetPreCompInfo(ECPoint point, string name, PreCompInfo preCompInfo)
        {
            ...
            {
                var table = point.m_preCompTable;
                if (null == table) point.m_preCompTable = table = Platform.CreateHashtable(4);
        ...
        {
            if (this == p.Curve) return p;
            if (p.IsInfinity) return this.Infinity;
...
#pragma warning restore
        ...
        public virtual ECMultiplier GetMultiplier()
        {
            lock (this)
            {
                if (this.m_multiplier == null) this.m_multiplier = CreateDefaultMultiplier();
        ...
        {
            ECPoint p = null;
            var expectedLength = (this.FieldSize + 7) / 8;
        ...
        public class Config
        {
            ...
            public ECCurve Create()
            {
                ...
                var c = this.outer.CloneCurve();
                if (c == this.outer) throw new InvalidOperationException("implementation returned current curve");
    ...
    abstract class AbstractFpCurve
        : ECCurve
    {
        ...
        protected override ECPoint DecompressPoint(int yTilde, BigInteger X1)
        {
            ...
            var rhs = x.Square().Add(this.A).Multiply(x).Add(this.B);
            var y = rhs.Sqrt();
        ...
        : AbstractFpCurve
    {
        ...
        protected FpCurve(BigInteger q, BigInteger r, ECFieldElement a, ECFieldElement b, BigInteger order,
            BigInteger cofactor)
            : base(q)
        {
            this.m_q = q;
        ...
        public override ECPoint Infinity => this.m_infinity;
    ...
    abstract class AbstractF2mCurve
        : ECCurve
    {
        /**
         * The auxiliary values
         * <code>s<sub>0</sub></code>
         * and
         ...
         */
        public virtual bool IsKoblitz => this.m_order != null && this.m_cofactor != null && this.m_b.IsOne &&
                                         (this.m_a.IsZero || this.m_a.IsOne);
        ...
        {
            if (k1 == 0) throw new ArgumentException("k1 must be > 0");

            if (k2 == 0)
            {
                if (k3 != 0) throw new ArgumentException("k3 must be 0 if k2 == 0");

                return FiniteFields.GetBinaryExtensionField(new[] {0, k1, m});
            }

            if (k2 <= k1) throw new ArgumentException("k2 must be > k1");

            if (k3 <= k2) throw new ArgumentException("k3 must be > k2");

            return FiniteFields.GetBinaryExtensionField(new[] {0, k1, k2, k3, m});
        }
        ...
        protected override ECPoint DecompressPoint(int yTilde, BigInteger X1)
        {
            ...
            {
                var beta = xp.Square().Invert().Multiply(this.B).Add(this.A).Add(xp);
                var z = SolveQuadradicEquation(beta);

                if (z != null)
                {
                    if (z.TestBitZero() != (yTilde == 1)) z = z.AddOne();
         ...
         * <code>z<sup>2</sup> + z = beta</code>
         * (X9.62
         * D.1.6) The other solution is
         * <code>z + 1</code>
         * .
         * 
         * @param beta
        ...
        internal virtual BigInteger[] GetSi()
        {
            if (this.si == null)
                lock (this)
                {
                    if (this.si == null) this.si = Tnaf.GetSi(this);
                }
        ...
        : AbstractF2mCurve
    {
        ...
        protected F2mCurve(int m, int k1, int k2, int k3, ECFieldElement a, ECFieldElement b, BigInteger order,
            BigInteger cofactor)
            : base(m, k1, k2, k3)
        {
            this.M = m;
        ...
        {
            if (this.IsKoblitz) return new WTauNafMultiplier();
