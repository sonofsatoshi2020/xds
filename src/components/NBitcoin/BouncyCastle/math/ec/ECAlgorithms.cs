...
namespace NBitcoin.BouncyCastle.math.ec
{
    class ECAlgorithms
    {
        ...
        public static ECPoint SumOfMultiplies(ECPoint[] ps, BigInteger[] ks)
        {
            ...
            imported[0] = p;
            for (var i = 1; i < count; ++i) imported[i] = ImportPoint(c, ps[i]);

            var glvEndomorphism = c.GetEndomorphism() as GlvEndomorphism;
            if (glvEndomorphism != null) return ValidatePoint(ImplSumOfMultipliesGlv(imported, ks, glvEndomorphism));
        ...
        public static ECPoint SumOfTwoMultiplies(ECPoint P, BigInteger a, ECPoint Q, BigInteger b)
        {
            ...
            // Point multiplication for Koblitz curves (using WTNAF) beats Shamir's trick
            {
                ...
                if (f2mCurve != null && f2mCurve.IsKoblitz) return ValidatePoint(P.Multiply(a).Add(Q.Multiply(b)));
            }

            var glvEndomorphism = cp.GetEndomorphism() as GlvEndomorphism;
            if (glvEndomorphism != null)
                return ValidatePoint(
                    ImplSumOfMultipliesGlv(new[] {P, Q}, new[] {a, b}, glvEndomorphism));
        ...
        */
        ...
        public static void MontgomeryTrick(ECFieldElement[] zs, int off, int len, ECFieldElement scale)
        {
            ...
            var i = 0;
            while (++i < len) c[i] = c[i - 1].Multiply(zs[off + i]);

            --i;

            if (scale != null) c[i] = c[i].Multiply(scale);
         ...
         * @param k
         * The multiplier.
         * @return The result of the point multiplication
         * <code>kP</code>
         * .
         */
            ...
            ECPoint Q, BigInteger l)
        {
            ...
            var preCompP = negK ? infoP.PreCompNeg : infoP.PreComp;
            var preCompQ = negL ? infoQ.PreCompNeg : infoQ.PreComp;
            var preCompNegP = negK ? infoP.PreComp : infoP.PreCompNeg;
            var preCompNegQ = negL ? infoQ.PreComp : infoQ.PreCompNeg;

            var wnafP = WNafUtilities.GenerateWindowNaf(widthP, k);
            var wnafQ = WNafUtilities.GenerateWindowNaf(widthQ, l);
        ...
        internal static ECPoint ImplShamirsTrickWNaf(ECPoint P, BigInteger k, ECPointMap pointMapQ, BigInteger l)
        {
            ...
            var width = System.Math.Max(2,
                System.Math.Min(16, WNafUtilities.GetWindowSize(System.Math.Max(k.BitLength, l.BitLength))));
            ...
            var wnafP = WNafUtilities.GenerateWindowNaf(width, k);
            var wnafQ = WNafUtilities.GenerateWindowNaf(width, l);
            ...
            ECPoint[] preCompQ, ECPoint[] preCompNegQ, byte[] wnafQ)
        {
            ...
            {
                var wiP = i < wnafP.Length ? (sbyte) wnafP[i] : 0;
                var wiQ = i < wnafQ.Length ? (sbyte) wnafQ[i] : 0;
                ...
                if (wiP != 0)
                {
                    ...
                    var tableP = wiP < 0 ? preCompNegP : preCompP;
                    r = r.Add(tableP[nP >> 1]);
                }
                ...
                if (wiQ != 0)
                {
                    ...
                    var tableQ = wiQ < 0 ? preCompNegQ : preCompQ;
                    r = r.Add(tableQ[nQ >> 1]);
                }
            ...
            if (zeroes > 0) R = R.TimesPow2(zeroes);
        ...
        internal static ECPoint ImplSumOfMultipliesGlv(ECPoint[] ps, BigInteger[] ks, GlvEndomorphism glvEndomorphism)
        {
            ...
            {
                var ab = glvEndomorphism.DecomposeScalar(ks[i].Mod(n));
                abs[j++] = ab[0];
                abs[j++] = ab[1];
            }

            var pointMap = glvEndomorphism.PointMap;
            if (glvEndomorphism.HasEfficientPointMap) return ImplSumOfMultiplies(ps, pointMap, abs);
        ...
        internal static ECPoint ImplSumOfMultiplies(ECPoint[] ps, ECPointMap pointMap, BigInteger[] ks)
        {
            ...
            for (var i = 0; i < halfCount; ++i)
            {
                ...
                var width = System.Math.Max(2,
                    System.Math.Min(16, WNafUtilities.GetWindowSize(System.Math.Max(kj0.BitLength, kj1.BitLength))));
        ...
        static ECPoint ImplSumOfMultiplies(bool[] negs, WNafPreCompInfo[] infos, byte[][] wnafs)
        {
            ...
            for (var i = 0; i < count; ++i) len = System.Math.Max(len, wnafs[i].Length);
            ...
            for (var i = len - 1; i >= 0; --i)
            {
                ...
                {
                    var wnaf = wnafs[j];
                    var wi = i < wnaf.Length ? (sbyte) wnaf[i] : 0;
                    if (wi != 0)
                    {
                        var n = System.Math.Abs(wi);
                        var info = infos[j];
                        var table = wi < 0 == negs[j] ? info.PreComp : info.PreCompNeg;
                        r = r.Add(table[n >> 1]);
                    }
                }
