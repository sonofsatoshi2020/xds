...
namespace NBitcoin.BouncyCastle.math.ec.multiplier
{
    abstract class WNafUtilities
    {
        ...
        static readonly int[] DEFAULT_WINDOW_SIZE_CUTOFFS = {13, 41, 121, 337, 897, 2305};

        static readonly byte[] EMPTY_BYTES = new byte[0];
        static readonly int[] EMPTY_INTS = new int[0];
        ...
        public static int[] GenerateCompactNaf(BigInteger k)
        {
            ...
            if (naf.Length > length) naf = Trim(naf, length);
        ...
        {
            if (width == 2) return GenerateCompactNaf(k);
            ...
            if (wnaf.Length > length) wnaf = Trim(wnaf, length);
        ...
        public static byte[] GenerateJsf(BigInteger g, BigInteger h)
        {
            ...
            if (jsf.Length > j) jsf = Trim(jsf, j);
        ...
        {
            if (width == 2) return GenerateNaf(k);
        ...
        {
            if (preCompInfo != null && preCompInfo is WNafPreCompInfo) return (WNafPreCompInfo) preCompInfo;
            ...
            ECPointMap pointMap)
        {
            ...
            for (var i = 0; i < preCompP.Length; ++i) preCompQ[i] = pointMap.Map(preCompP[i]);
            wnafPreCompQ.PreComp = preCompQ;

            if (includeNegated)
            {
                var preCompNegQ = new ECPoint[preCompQ.Length];
                for (var i = 0; i < preCompNegQ.Length; ++i) preCompNegQ[i] = preCompQ[i].Negate();
        ...
        public static WNafPreCompInfo Precompute(ECPoint p, int width, bool includeNegated)
        {
                            ...
                            if (ECAlgorithms.IsFpCurve(c) && c.FieldSize >= 64)
                                switch (c.CoordinateSystem)
                                {
                                    ...
                                    case ECCurve.COORD_JACOBIAN_MODIFIED:
                                    {
                                        ...
                                        if (iniPreCompLen == 0) preComp[0] = last;
                                        break;
                                    }
                                }
                        }
            ...
            if (includeNegated)
            {
                ...
                else
                {
                    ...
                    if (pos < reqPreCompLen) preCompNeg = ResizeTable(preCompNeg, reqPreCompLen);
                }
