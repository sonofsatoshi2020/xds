...
namespace NBitcoin.BouncyCastle.math.raw
{
    abstract class Mod
    {
        ...
        public static void Invert(uint[] p, uint[] x, uint[] z)
        {
            ...
            if ((u[0] & 1) == 0) InversionStep(p, u, len, a, ref ac);
            if (Nat.IsOne(len, u))
            {
            ...
            for (;;)
            {
                while (u[uvLen - 1] == 0 && v[uvLen - 1] == 0) --uvLen;
        ...
        public static void Subtract(uint[] p, uint[] x, uint[] y, uint[] z)
        {
            ...
            if (c != 0) Nat.AddTo(len, p, z);
        }

        static void InversionResult(uint[] p, int ac, uint[] a, uint[] z)
        {
            if (ac < 0)
        ...
        static void InversionStep(uint[] p, uint[] u, int uLen, uint[] x, ref int xc)
        {
            var len = p.Length;
            var count = 0;
            while (u[0] == 0)
        ...
        static int GetTrailingZeroes(uint x)
        {
            Debug.Assert(x != 0);
            var count = 0;
            while ((x & 1) == 0)
