...
namespace NBitcoin
{
    ...
    {
        static volatile byte[] additionalEntropy;
        static volatile int entropyIndex;
        ...
        static void PushEntropy(byte[] data)
        {
            if (additionalEntropy == null || data.Length == 0)
                return;
            var pos = entropyIndex;
        ...
        public static void AddEntropy(byte[] data)
        {
            ...
            {
                for (var i = 0; i < 32; i++) additionalEntropy[i] ^= entropy[i];
