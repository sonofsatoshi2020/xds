...
namespace NBitcoin
{
    public class PartialMerkleTree : IBitcoinSerializable
    {
        ...
        public uint TransactionCount
        {
            ...
            set => this._TransactionCount = value;
        }

        public List<uint256> Hashes => this._Hashes;

        public BitArray Flags { get; set; } = new BitArray(0);

        static void MarkNodes(MerkleNode root, bool[] vMatch)
        {
        ...
        static void MarkToTop(MerkleNode leaf, bool value)
        {
            leaf.IsMarked = value;
            foreach (var ancestor in leaf.Ancestors()) ancestor.IsMarked = value;
        }

        public MerkleNode GetMerkleRoot()
        {
            var node = MerkleNode.GetRoot((int) this.TransactionCount);
            var flags = new BitReader(this.Flags);
            var hashes = this.Hashes.GetEnumerator();
        ...
        void BuildCore(MerkleNode node, BitWriter flags)
        {
            if (node == null)
                return;
        ...
        {
            var flags = new BitReader(this.Flags);
            var root = MerkleNode.GetRoot((int) this.TransactionCount);
            var hashes = this.Hashes.GetEnumerator();
            return GetMatchedTransactionsCore(root, flags, hashes, false);
        }

        IEnumerable<uint256> GetMatchedTransactionsCore(MerkleNode node, BitReader flags, IEnumerator<uint256> hashes,
            bool calculateHash)
        {
            if (node == null)
        ...
        public PartialMerkleTree Trim(params uint256[] matchedTransactions)
        {
            ...
            foreach (var leaf in root.GetLeafs()) MarkToTop(leaf, false);
            var flags = new BitWriter();
        ...
        public void ReadWrite(BitcoinStream stream)
        {
            ...
            if (!stream.Serializing)
            {
                ...
                this.Flags = writer.ToBitArray();
            }
            else
            {
                vBytes = new byte[(this.Flags.Length + 7) / 8];
                for (var p = 0; p < this.Flags.Length; p++)
                    vBytes[p / 8] |= (byte) (ToByte(this.Flags.Get(p)) << (p % 8));
                stream.ReadWriteAsVarString(ref vBytes);
            }
        }
