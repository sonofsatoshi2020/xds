...
namespace NBitcoin
{
    public class OutPoint : IBitcoinSerializable
    {
        ...
        public bool IsNull => this.hash == uint256.Zero && this.n == uint.MaxValue;
    ...
    public class TxIn : IBitcoinSerializable
    {
        ...
        {
            get => this.nSequence;
            set => this.nSequence = value.Value;
        }

        public OutPoint PrevOut
        {
            get => this.prevout;
            set => this.prevout = value;
        }
        ...
        {
            get => this.scriptSig;
            set => this.scriptSig = value;
        }

        /// <summary>
        ///     The witness script (Witness script is not serialized and deserialized at the TxIn level, but at the Transaction
        ///     level)
        /// </summary>
        public WitScript WitScript { get; set; } = WitScript.Empty;

        public bool WitScriptEmpty => this.WitScript == WitScript.Empty || this.WitScript == null;

        public bool IsFinal => this.nSequence == uint.MaxValue;
        ...
        {
            var result = PayToPubkeyHashTemplate.Instance.ExtractScriptSigParameters(network, this.ScriptSig);
    ...
    public class TxOutCompressor : IBitcoinSerializable
    {
        ...
        public void ReadWrite(BitcoinStream stream)
        {
            ...
            {
                var val = CompressAmount((ulong) this.TxOut.Value.Satoshi);
            ...
            else
            {
                ulong val = 0;
                stream.ReadWriteAsCompactVarInt(ref val);
                this.TxOut.Value = new Money(DecompressAmount(val));
            }

            var cscript = new ScriptCompressor(this.TxOut.ScriptPubKey);
            stream.ReadWrite(ref cscript);
            if (!stream.Serializing) this.TxOut.ScriptPubKey = new Script(cscript.ScriptBytes);
        }
    ...
    public class ScriptCompressor : IBitcoinSerializable
    {
        ...
        public byte[] ScriptBytes => this._Script;
        ...
        {
            byte[] result = null;
            var script = Script.FromBytesUnsafe(this._Script);
        ...
        public void ReadWrite(BitcoinStream stream)
        {
            if (stream.Serializing)
            {
                ...
                var nSize = (uint) this._Script.Length + nSpecialScripts;
    ...
    {
        static readonly Money NullMoney = new Money(-1);
    ...
    public class IndexedTxIn
    {
        ...
        public OutPoint PrevOut
        {
            get => this.TxIn.PrevOut;
            set => this.TxIn.PrevOut = value;
        }

        public Script ScriptSig
        {
            get => this.TxIn.ScriptSig;
            set => this.TxIn.ScriptSig = value;
        }


        public WitScript WitScript
        {
            get => this.TxIn.WitScript;
            set => this.TxIn.WitScript = value;
        }

        public Transaction Transaction { get; set; }

        public bool VerifyScript(Network network, Script scriptPubKey,
            ScriptVerify scriptVerify = ScriptVerify.Standard)
        {
            return VerifyScript(network, scriptPubKey, scriptVerify, out var unused);
        }
        ...
        {
            return Script.VerifyScript(network, scriptPubKey, this.Transaction, (int) this.Index, null, scriptVerify,
                SigHash.Undefined, out error);
        }

        public bool VerifyScript(Network network, Script scriptPubKey, Money value, ScriptVerify scriptVerify,
            out ScriptError error)
        {
            return Script.VerifyScript(network, scriptPubKey, this.Transaction, (int) this.Index, value, scriptVerify,
                SigHash.Undefined, out error);
        }

        public bool VerifyScript(Network network, ICoin coin, ScriptVerify scriptVerify = ScriptVerify.Standard)
        {
            return VerifyScript(network, coin, scriptVerify, out var unused);
        }

        public bool VerifyScript(Network network, ICoin coin, ScriptVerify scriptVerify, out ScriptError error)
        {
            return Script.VerifyScript(network, coin.TxOut.ScriptPubKey, this.Transaction, (int) this.Index,
                coin.TxOut.Value, scriptVerify, SigHash.Undefined, out error);
        }
        ...
        {
            return Script.SignatureHash(network, coin.GetScriptCode(network), this.Transaction, (int) this.Index,
                sigHash, coin.TxOut.Value, coin.GetHashVersion(network));
        }
    }
    ...
    public class TxOutList : UnsignedList<TxOut>
    {
        ...
        {
            var txId = this.Transaction.GetHash();
            for (var i = 0; i < this.Count; i++) yield return new Coin(new OutPoint(txId, i), this[i]);
        }
    }
    ...
    public class WitScript
    {
        ...
        public WitScript(string script)
        {
            ...
            for (var i = 0; i < parts.Length; i++) this._Pushes[i] = Encoders.Hex.DecodeData(parts[i]);
        }
        ...
        public WitScript(byte[][] script, bool @unsafe = false)
        {
            ...
            else
            {
                ...
                for (var i = 0; i < this._Pushes.Length; i++) this._Pushes[i] = script[i].ToArray();
            }
        }
        ...
        public byte[] this[int index] => this._Pushes[index];

        public IEnumerable<byte[]> Pushes => this._Pushes;

        public static WitScript Empty { get; } = new WitScript(new byte[0][], true);

        public int PushCount => this._Pushes.Length;
        ...
        void ReadCore(BitcoinStream stream)
        {
            ...
            for (var i = 0; i < (int) pushCount; i++)
            {
                var push = ReadPush(stream);
                pushes.Add(push);
            }
        ...
        static byte[] ReadPush(BitcoinStream stream)
        {
            byte[] push = null;
        ...
        bool EqualsCore(WitScript item)
        {
            if (this._Pushes.Length != item._Pushes.Length)
                return false;
            for (var i = 0; i < this._Pushes.Length; i++)
        ...
        public byte[] ToBytes()
        {
            using (var ms = new MemoryStream())
            {
                ...
                var pushCount = (uint) this._Pushes.Length;
                stream.ReadWriteAsVarInt(ref pushCount);
        ...
        public TxDestination GetSigner(Network network)
        {
            ...
            if (pubKey != null) return pubKey.PublicKey.WitHash;
    ...
    class Witness
    {
        ...
        {
            for (var i = 0; i < this._Inputs.Count; i++)
                if (stream.Serializing)
                {
                    var bytes = (this._Inputs[i].WitScript ?? WitScript.Empty).ToBytes();
    ...
    public class Transaction : IBitcoinSerializable
    {
        ...
        static readonly ulong MAX_MONEY = long.MaxValue; // 21000000ul * Money.COIN;
        ...
        public bool RBF
        {
            get { return this.Inputs.Any(i => i.Sequence < 0xffffffff - 1); }
        }
        ...
        {
            get { return this.Outputs.Sum(v => v.Value); }
        }

        public LockTime LockTime
        {
            get => this.nLockTime;
            set => this.nLockTime = value;
        }

        public TxInList Inputs => this.vin;

        public TxOutList Outputs => this.vout;
        ...
        {
            get { return this.Inputs.Any(i => i.WitScriptEmpty == false); }
        }
        ...
        public virtual void ReadWrite(BitcoinStream stream)
        {
            ...
            if (!stream.Serializing)
            {
                ...
                var hasNoDummy = (this.nVersion & NoDummyInput) != 0 && this.vin.Count == 0;
            ...
            {
                var version = witSupported && this.vin.Count == 0 && this.vout.Count > 0
                    ? this.nVersion | NoDummyInput
                    : this.nVersion;
        ...
        {
            this._Hashes = invalidateExisting ? new uint256[2] : this._Hashes ?? new uint256[2];
            if (!lazily && this._Hashes[0] == null) this._Hashes[0] = GetHash();
            if (!lazily && this._Hashes[1] == null) this._Hashes[1] = GetWitHash();
        }
        ...
        public TransactionSignature SignInput(Network network, ISecret secret, ICoin coin,
        ...
        public virtual bool IsProtocolTransaction()
        {
            return
                this.IsCoinBase ||
                this.IsCoinStake; // this is now a POS specific check (it will fail for a BTC network)
        }
        ...
        */
        ...
        static RawFormatter GetFormatter(RawFormat rawFormat, Network network)
        {
        ...
        public bool IsFinal(DateTimeOffset blockTime, int blockHeight)
        {
            ...
            if (this.nLockTime <
                ((long) this.nLockTime < LockTime.LOCKTIME_THRESHOLD ? (long) blockHeight : nBlockTime))
                return true;

            foreach (var txin in this.Inputs)
                if (!txin.IsFinal)
                    return false;
        ...
        public bool CheckSequenceLocks(int[] prevHeights, ChainedHeader block,
        ...
        /// <param name="flags">If VerifySequence is not set, returns always true SequenceLock</param>
        /// <returns>Sequence lock of minimum SequenceLock to satisfy</returns>
        public SequenceLock CalculateSequenceLocks(int[] prevHeights, ChainedHeader chainedHeader,
            LockTimeFlags flags = LockTimeFlags.VerifySequence)
        {
            ...
            var fEnforceBIP68 = this.Version >= 2
                                && (flags & LockTimeFlags.VerifySequence) != 0;

            // Do not enforce sequence numbers as a relative lock time
            // unless we have been instructed to
            if (!fEnforceBIP68) return new SequenceLock(nMinHeight, nMinTime);

            for (var txinIndex = 0; txinIndex < this.Inputs.Count; txinIndex++)
            {
                var txin = this.Inputs[txinIndex];
                ...
                {
                    var nCoinTime =
                        (long) Utils.DateTimeToUnixTimeLong(chainedHeader.GetAncestor(Math.Max(nCoinHeight - 1, 0))
                            .GetMedianTimePast());
        ...
        public TransactionCheckResult Check()
        {
            ...
            long nValueOut = 0;
            foreach (var txout in this.Outputs)
            {
                if (txout.Value < 0)
