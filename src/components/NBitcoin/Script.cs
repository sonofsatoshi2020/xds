...
namespace NBitcoin
{
    ...
    public class ScriptSigs
    {
        ...
        public WitScript WitSig { get; set; }
    }
    ...
    public class Script
    {
        ...
        public Script(IEnumerable<Op> ops)
        {
            ...
            {
                foreach (var op in ops) op.WriteTo(ms);
        ...
        Script(byte[] data, bool @unsafe, bool unused)
        {
        ...
        public int Length => this._Script.Length;

        /// <summary>
        ///     Get the P2SH scriptPubKey of this script
        /// </summary>
        public Script PaymentScript => this._PaymentScript ??
                                       (this._PaymentScript =
                                           PayToScriptHashTemplate.Instance.GenerateScriptPubKey(this.Hash));
        ...
        public ScriptId Hash => this._Hash ?? (this._Hash = new ScriptId(this));

        public WitScriptId WitHash => this._WitHash ?? (this._WitHash = new WitScriptId(this));

        public bool IsUnspendable => this._Script.Length > 0 && this._Script[0] == (byte) OpcodeType.OP_RETURN;
        ...
        static byte[] Parse(string script)
        {
            using (var reader = new StringReader(script.Trim()))
            {
                using (var result = new MemoryStream())
                {
                    while (reader.Peek() != -1) Op.Read(reader).WriteTo(result);
        ...
        public static uint256 SignatureHash(Network network, ICoin coin, Transaction txTo,
            SigHash nHashType = SigHash.All)
        {
        ...
        public static uint256 SignatureHash(Network network, Script scriptCode, Transaction txTo, int nIn,
            SigHash nHashType, Money amount = null, HashVersion sigversion = HashVersion.Original)
        {
        ...
        //https://en.bitcoin.it/wiki/OP_CHECKSIG
        public static uint256 SignatureHash(Network network, Script scriptCode, Transaction txTo, int nIn,
            SigHash nHashType, Money amount, HashVersion sigversion,
            PrecomputedTransactionData precomputedTransactionData)
        {
            if (sigversion == HashVersion.Witness)
            {
                ...
                {
                    var ss = CreateHashWriter(sigversion);
                    ss.ReadWrite(txTo.Outputs[nIn]);
                    hashOutputs = GetHash(ss);
                }

                var sss = CreateHashWriter(sigversion);
                // Version
            ...
            var txCopy = network.CreateTransaction(txTo.ToBytes());

            //Set all TxIn script to empty string
            foreach (var txin in txCopy.Inputs) txin.ScriptSig = new Script();
        ...
        {
            uint256 hashOutputs;
            var ss = CreateHashWriter(HashVersion.Witness);
            foreach (var txout in txTo.Outputs) ss.ReadWrite(txout);
            hashOutputs = GetHash(ss);
            return hashOutputs;
        }
        ...
        internal static uint256 GetHashSequence(Transaction txTo)
        {
            ...
            var ss = CreateHashWriter(HashVersion.Witness);
            foreach (var input in txTo.Inputs) ss.ReadWrite(input.Sequence);
        ...
        internal static uint256 GetHashPrevouts(Transaction txTo)
        {
            ...
            var ss = CreateHashWriter(HashVersion.Witness);
            foreach (var input in txTo.Inputs) ss.ReadWrite(input.PrevOut);
        ...
        public TxDestination GetSigner(Network network)
        {
            ...
            if (pubKey != null) return pubKey.PublicKey.Hash;
        ...
        public PubKey[] GetDestinationPublicKeys(Network network)
        {
            ...
            else
            {
                ...
                if (multiSig != null) result.AddRange(multiSig.PubKeys);
            }
        ...
        public static bool VerifyScript(Network network, Script scriptSig, Script scriptPubKey, Transaction tx, int i,
            ScriptVerify scriptVerify = ScriptVerify.Standard, SigHash sigHash = SigHash.Undefined)
        {
            return VerifyScript(network, scriptSig, scriptPubKey, tx, i, null, scriptVerify, sigHash, out var unused);
        }

        public static bool VerifyScript(Network network, Script scriptSig, Script scriptPubKey, Transaction tx, int i,
            Money value, ScriptVerify scriptVerify = ScriptVerify.Standard, SigHash sigHash = SigHash.Undefined)
        {
            return VerifyScript(network, scriptSig, scriptPubKey, tx, i, value, scriptVerify, sigHash, out var unused);
        }

        public static bool VerifyScript(Network network, Script scriptSig, Script scriptPubKey, Transaction tx, int i,
            Money value, out ScriptError error)
        {
            return VerifyScript(network, scriptSig, scriptPubKey, tx, i, value, ScriptVerify.Standard,
                SigHash.Undefined, out error);
        }

        public static bool VerifyScript(Network network, Script scriptPubKey, Transaction tx, int i, Money value,
            ScriptVerify scriptVerify = ScriptVerify.Standard, SigHash sigHash = SigHash.Undefined)
        {
        ...
        public static bool VerifyScript(Network network, Script scriptSig, Script scriptPubKey, Transaction tx, int i,
            Money value, ScriptVerify scriptVerify, SigHash sigHash, out ScriptError error)
        {
        ...
        static extern int VerifyScriptConsensus(byte[] scriptPubKey, uint scriptPubKeyLen, byte[] txTo, uint txToLen,
            uint nIn, ScriptVerify flags, ref BitcoinConsensusError err);

        [DllImport(LibConsensusDll, EntryPoint = "bitcoinconsensus_verify_script_with_amount",
            CallingConvention = CallingConvention.Cdecl)]
        static extern int VerifyScriptConsensusWithAmount(byte[] scriptPubKey, uint scriptPubKeyLen, long amount,
            byte[] txTo, uint txToLen, uint nIn, ScriptVerify flags, ref BitcoinConsensusError err);

        public static bool VerifyScriptConsensus(Script scriptPubKey, Transaction tx, uint nIn, ScriptVerify flags)
        {
            return VerifyScriptConsensus(scriptPubKey, tx, nIn, flags, out var unused);
        }

        public static bool VerifyScriptConsensus(Script scriptPubKey, Transaction tx, uint nIn, Money amount,
            ScriptVerify flags)
        {
            return VerifyScriptConsensus(scriptPubKey, tx, nIn, amount, flags, out var unused);
        }
        ...
        public static Script CombineSignatures(Network network, Script scriptPubKey, Transaction transaction, int n,
            Script scriptSig1, Script scriptSig2)
        {
        ...
        public static ScriptSigs CombineSignatures(Network network, Script scriptPubKey, TransactionChecker checker,
            ScriptSigs input1, ScriptSigs input2)
        {
        ...
        static Script CombineSignatures(Network network, Script scriptPubKey, TransactionChecker checker,
            byte[][] sigs1, byte[][] sigs2, HashVersion hashVersion)
        {
            ...
            if (template is PayToScriptHashTemplate || template is PayToWitTemplate)
            {
                ...
                var result = CombineSignatures(network, redeem, checker, sigs1, sigs2, hashVersion);
        ...
        static Script CombineMultisig(Network network, Script scriptPubKey, TransactionChecker checker, byte[][] sigs1,
            byte[][] sigs2, HashVersion hashVersion)
        {
            ...
            foreach (var pubkey in multiSigParams.PubKeys)
            {
                ...
                var eval = new ScriptEvaluationContext(network);
                if (eval.CheckSig(sig, pubkey, scriptPubKey, checker, hashVersion)) sigs.AddOrReplace(pubkey, sig);
            }
        ...
        static Script PushAll(byte[][] stack)
        {
            var s = new Script();
            foreach (var push in stack) s += Op.GetPushOp(push);
            return s;
        }
        ...
        static byte[][] Max(byte[][] scriptSig1, byte[][] scriptSig2)
        {
