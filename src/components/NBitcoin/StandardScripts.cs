...
namespace NBitcoin
{
    ...
    {
        static readonly List<ScriptTemplate> standardTemplates = new List<ScriptTemplate>
        {
        ...
        static bool IsStandardScriptSig(Network network, Script scriptSig, Script scriptPubKey)
        {
        ...
        public static bool AreInputsStandard(Network network, Transaction tx, CoinsView coinsView)
        {
            ...
            {
                var prev = coinsView.GetOutputFor(input);
                if (prev == null)
                    return false;
