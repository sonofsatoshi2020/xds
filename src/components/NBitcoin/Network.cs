...
namespace NBitcoin
{
    ...
    public abstract class Network
    {
        ...
        /// </summary>
        /// <remarks>
        ///     The <see cref="MinRelayTxFee" /> and <see cref="MinTxFee" /> are typically the same value to prevent dos attacks on
        ///     the network.
        ///     If <see cref="MinRelayTxFee" /> is less than <see cref="MinTxFee" />, an attacker can broadcast a lot of
        ///     transactions with fees between these two values,
        ...
        /// <summary>
        ///     The name of the root folder containing blockchains operating with the same consensus rules.
        /// </summary>
        ...
        ///     A non-standard transaction can still be mined/staked by a willing node and the resulting block will be accepted by
        ///     the network.
        ...
        public static Block MineGenesisBlock(ConsensusFactory consensusFactory, string coinbaseText, Target target,
            Money genesisReward, int version = 1)
        {
            if (consensusFactory == null)
                throw new ArgumentException(
                    $"Parameter '{nameof(consensusFactory)}' cannot be null. Use 'new ConsensusFactory()' for Bitcoin-like proof-of-work blockchains and 'new PosConsensusFactory()' for proof-of-stake blockchains.");

            if (string.IsNullOrEmpty(coinbaseText))
                throw new ArgumentException(
                    $"Parameter '{nameof(coinbaseText)}' cannot be null. Use a news headline or any other appropriate string.");

            if (target == null)
                throw new ArgumentException(
                    $"Parameter '{nameof(target)}' cannot be null. Example use: new Target(new uint256(\"0000ffff00000000000000000000000000000000000000000000000000000000\"))");

            if (coinbaseText.Length >= 92)
                throw new ArgumentException(
                    $"Parameter '{nameof(coinbaseText)}' should be shorter than 92 characters.");

            if (genesisReward == null)
                throw new ArgumentException(
                    $"Parameter '{nameof(genesisReward)}' cannot be null. Example use: 'Money.Coins(50m)'.");

            var time = DateTimeOffset.Now;
            var unixTime = Utils.DateTimeToUnixTime(time);
            ...
            var genesis = consensusFactory.CreateBlock();
        ...
        protected static void Assert(bool condition)
        {
            ...
            if (!condition) throw new InvalidOperationException("Invalid network");
        }
        ...
        public Base58Type? GetBase58Type(string base58)
        {
            ...
            for (var i = 0; i < this.Base58Prefixes.Length; i++)
            {
        ...
        public static T Parse<T>(string str, Network expectedNetwork) where T : IBitcoinString
        {
            ...
            var networks = expectedNetwork == null ? NetworkRegistration.GetNetworks() : new[] {expectedNetwork};
            var maybeb58 = true;
        ...
        IBase58Data CreateBase58Data(Base58Type type, string base58)
        {
        ...
        BitcoinColoredAddress CreateColoredAddress(string base58)
        {
        ...
        BitcoinPassphraseCode CreatePassphraseCode(string base58)
        {
            return new BitcoinPassphraseCode(base58, this);
        }

        BitcoinEncryptedSecretNoEC CreateEncryptedKeyNoEC(string base58)
        {
            return new BitcoinEncryptedSecretNoEC(base58, this);
        }

        BitcoinEncryptedSecretEC CreateEncryptedKeyEC(string base58)
        {
            return new BitcoinEncryptedSecretEC(base58, this);
        }

        Base58Data CreateConfirmationCode(string base58)
        {
            return new BitcoinConfirmationCode(base58, this);
        }

        Base58Data CreateBitcoinExtPubKey(string base58)
        {
        ...
        public Money GetReward(int nHeight)
        {
            ...
            var halvings = nHeight / this.Consensus.SubsidyHalvingInterval;
