...
namespace NBitcoin.Protocol
{
    public class NetworkAddress : IBitcoinSerializable
    {
        ...
        public TimeSpan Ago
        {
            get => DateTimeOffset.UtcNow - this.Time;
            set => this.Time = DateTimeOffset.UtcNow - value;
        }

        public IPEndPoint Endpoint
        {
            get => new IPEndPoint(new IPAddress(this.ip), this.port);
            set
            {
        ...
        {
            get => Utils.UnixTimeToDateTime(this.ntime);
        ...
        {
            if (stream.Type == SerializationType.Disk) stream.ReadWrite(ref this.version);
            if (stream.Type == SerializationType.Disk
                || stream.ProtocolVersion >= ProtocolVersion.CADDR_TIME_VERSION &&
