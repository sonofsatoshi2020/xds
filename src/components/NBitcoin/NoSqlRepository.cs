...
namespace NBitcoin
{
    public abstract class NoSqlRepository
    {
        ...
        public async Task<T> GetAsync<T>(string key) where T : IBitcoinSerializable, new()
        {
            ...
            if (data == null)
                return default;
            var obj = this.Network.Consensus.ConsensusFactory.TryCreateNew<T>();
            if (obj == null)
                obj = Activator.CreateInstance<T>();
            obj.ReadWrite(data, this.Network.Consensus.ConsensusFactory);
            return obj;
        }
        ...
        {
            return PutBytesBatch(values.Select(s =>
                new Tuple<string, byte[]>(s.Item1, s.Item2 == null ? null : s.Item2.ToBytes())));
        }
