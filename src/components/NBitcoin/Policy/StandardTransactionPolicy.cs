...
namespace NBitcoin.Policy
{
    public class StandardTransactionPolicy : ITransactionPolicy
    {
        ...
        ///     That's also enough to spend a 20-of-20 CHECKMULTISIG scriptPubKey, though such a scriptPubKey is not considered
        ///     standard.
        /// </summary>
        ...
        public TransactionPolicyError[] Check(Transaction transaction, ICoin[] spentCoins)
        {
            ...
            foreach (var input in transaction.Inputs.AsIndexedInputs())
            {
                ...
                if (coin != null)
                    if (this.ScriptVerify != null)
                        if (!input.VerifyScript(this.network, coin.TxOut.ScriptPubKey, coin.TxOut.Value,
                            this.ScriptVerify.Value, out var error))
                            errors.Add(new ScriptPolicyError(input, error, this.ScriptVerify.Value,
                ...
                if (!txin.ScriptSig.IsPushOnly) errors.Add(new InputPolicyError("All operation should be push", input));
            ...
            if (fees != null)
                if (this.CheckFee)
                {
                    ...
                    {
                        var max = this.MaxTxFee.GetFee(txSize);
                        if (fees > max)
                    ...
                    if (this.MinRelayTxFee != null)
                        if (this.MinRelayTxFee != null)
                        {
                            var min = this.MinRelayTxFee.GetFee(txSize);
                            if (fees < min)
        ...
        protected virtual void CheckPubKey(Transaction transaction, List<TransactionPolicyError> errors)
        {
            if (this.CheckScriptPubKey)
                ...
                {
                    var template = this.network.StandardScriptsRegistry.GetTemplateFromScriptPubKey(txout.ScriptPubKey);
