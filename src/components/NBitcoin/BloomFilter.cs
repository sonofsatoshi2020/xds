...
namespace NBitcoin
{
    ...
    public class BloomFilter : IBitcoinSerializable
    {
        ...
        public BloomFilter(int nElements, double nFPRate, uint nTweakIn, BloomFlags nFlagsIn = BloomFlags.UPDATE_ALL)
        {
            ...
            this.vData = new byte[Math.Min((uint) (-1 / LN2SQUARED * nElements * (decimal) Math.Log(nFPRate)),
        ...
        uint Hash(uint nHashNum, byte[] vDataToHash)
        {
            ...
            return (uint) (Hashes.MurmurHash3(nHashNum * 0xFBA4C795 + this.nTweak, vDataToHash) %
                           (this.vData.Length * 8));
        }
            ...
            for (uint i = 0; i < tx.Outputs.Count; i++)
            {
                ...
                foreach (var op in txout.ScriptPubKey.ToOps())
                    if (op.PushData != null && op.PushData.Length != 0 && Contains(op.PushData))
                    {
                        ...
                        else if ((this.nFlags & (byte) BloomFlags.UPDATE_MASK) ==
                                 (byte) BloomFlags.UPDATE_P2PUBKEY_ONLY)
                        {
                            var template =
                                StandardScripts
                                    .GetTemplateFromScriptPubKey(txout.ScriptPubKey); // this is only valid for Bitcoin.
                            if (template != null &&
