...
namespace NBitcoin.Crypto.Cryptsharp
{
    ...
    public static class SCrypt
    {
            ...
            int cost, int blockSize, int parallel, int? maxThreads)
        {
            ...
            var B = Pbkdf2.ComputeDerivedKey(mac, S, 1, parallel * MFLen);
#endif
            var B0 = new uint[B.Length / 4];
            for (var i = 0; i < B0.Length; i++) B0[i] = BitPacking.UInt32FromLEBytes(B, i * 4);
            ...
            int cost, int blockSize, int parallel, int maxThreads)
        {
            ...
            for (var i = 0; i < threads.Length; i++) threads[i] = Task.Run(workerThread);
            workerThread();
            for (var i = 0; i < threads.Length; i++) threads[i].Wait();
        }
#endif
