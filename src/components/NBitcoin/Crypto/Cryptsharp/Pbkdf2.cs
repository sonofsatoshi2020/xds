...
namespace NBitcoin.Crypto.Cryptsharp
{
    ...
    class Pbkdf2 : Stream
    {
        ...
        readonly byte[] _saltBuffer;
        readonly byte[] _digest;
        readonly byte[] _digestT1;

#if NETCORE
        readonly IMac _hmacAlgorithm;
#else
        ...
        public Pbkdf2(IMac hmacAlgorithm, byte[] salt, int iterations)
        {
            ...
            var hmacLength = hmacAlgorithm.GetMacSize();
        ...
        public byte[] Read(int count)
        {
            ...
            if (bytes < count) throw Exceptions.Argument("count", "Can only return {0} bytes.", bytes);
        ...
        void ComputeBlock(uint pos)
        {
            ...
            for (var i = 1; i < this._iterations; i++)
            {
                ComputeHmac(this._digestT1, this._digestT1);
                for (var j = 0; j < this._digest.Length; j++) this._digest[j] ^= this._digestT1[j];
            }
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            while (count > 0)
            {
                if (this.Position < this._blockStart || this.Position >= this._blockEnd)
                {
                    ...
                    var pos = this.Position / this._digest.Length;
                    ComputeBlock((uint) (pos + 1));
                    this._blockStart = pos * this._digest.Length;
                    this._blockEnd = this._blockStart + this._digest.Length;
                }

                var bytesSoFar = (int) (this.Position - this._blockStart);
                var bytesThisTime = Math.Min(this._digest.Length - bytesSoFar, count);
