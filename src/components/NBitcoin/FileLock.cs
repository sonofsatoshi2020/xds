            ...
            while (true)
                try
                {
                    if (lockType == FileLockType.Read)
                        this._Fs = new FileStream(filePath, FileMode.Open, FileAccess.Read, FileShare.Read);
                    if (lockType == FileLockType.ReadWrite)
                        this._Fs = new FileStream(filePath, FileMode.Open, FileAccess.ReadWrite, FileShare.None);
                    break;
                }
                catch (IOException)
                {
