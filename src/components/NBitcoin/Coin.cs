...
namespace NBitcoin
{
    ...
    public class IssuanceCoin : IColoredCoin
    {
        ...
        public AssetId AssetId => this.Bearer.TxOut.ScriptPubKey.Hash.ToAssetId();

        public Uri DefinitionUrl { get; set; }

        public Script ScriptPubKey => this.Bearer.TxOut.ScriptPubKey;

        #region ICoin Members

        public Money Amount
        {
            get => this.Bearer.TxOut.Value;
            set => this.Bearer.TxOut.Value = value;
        }

        public TxOut TxOut => this.Bearer.TxOut;

        #endregion

        #region IColoredCoin Members

        public Coin Bearer { get; set; }


        public OutPoint Outpoint => this.Bearer.Outpoint;

        #endregion

        #region IColoredCoin Members

        AssetId IColoredCoin.AssetId => this.AssetId;

        Coin IColoredCoin.Bearer => this.Bearer;

        #endregion

        #region ICoin Members

        IMoney ICoin.Amount => this.Amount;

        OutPoint ICoin.Outpoint => this.Outpoint;

        TxOut ICoin.TxOut => this.TxOut;
    ...
    public class ColoredCoin : IColoredCoin
    {
        ...
        public AssetId AssetId => this.Amount.Id;

        public AssetMoney Amount { get; set; }
        ...
        public Script ScriptPubKey => this.Bearer.ScriptPubKey;
    ...
    public class Coin : ICoin
    {
        ...
        public virtual Script GetScriptCode(Network network)
        {
            ...
            var key = PayToWitPubKeyHashTemplate.Instance.ExtractScriptPubKeyParameters(network, this.ScriptPubKey);
            if (key != null)
        ...
        {
            return this._OverrideScriptCode != null || !this.ScriptPubKey.IsPayToScriptHash(network) &&
                   !PayToWitScriptHashTemplate.Instance.CheckScriptPubKey(this.ScriptPubKey);
        }
    ...
    public class ScriptCoin : Coin
    {
        ...
        public bool IsP2SH => this.ScriptPubKey.ToBytes(true)[0] == (byte) OpcodeType.OP_HASH160;

        public RedeemType RedeemType =>
            this.Redeem.Hash.ScriptPubKey == this.TxOut.ScriptPubKey ? RedeemType.P2SH : RedeemType.WitnessV0;
        ...
        public static ScriptCoin Create(Network network, uint256 txHash, uint outputIndex, Money amount,
            Script scriptPubKey, Script redeem)
        {
        ...
        public Script GetP2SHRedeem()
        {
            if (!this.IsP2SH)
                return null;
            var p2shRedeem = this.RedeemType == RedeemType.P2SH ? this.Redeem :
        ...
        public ScriptCoin AssertCoherent(Network network)
        {
            ...
            var expectedDestination = GetRedeemHash(network, this.TxOut.ScriptPubKey);
        ...
        public override Script GetScriptCode(Network network)
        {
            ...
            if (this._OverrideScriptCode != null)
                return this._OverrideScriptCode;
            var key = PayToWitPubKeyHashTemplate.Instance.ExtractScriptPubKeyParameters(network, this.Redeem);
            if (key != null)
                return key.AsKeyId().ScriptPubKey;
            return this.Redeem;
        }
        ...
        {
            return this._OverrideScriptCode != null || !this.IsP2SH ||
                   !PayToWitScriptHashTemplate.Instance.CheckScriptPubKey(this.Redeem);
        }

        public override HashVersion GetHashVersion(Network network)
        {
            var isWitness = PayToWitTemplate.Instance.CheckScriptPubKey(this.ScriptPubKey) ||
                            PayToWitTemplate.Instance.CheckScriptPubKey(this.Redeem) ||
                            this.RedeemType == RedeemType.WitnessV0;
    ...
    public class StealthCoin : Coin
    {
        ...
        public StealthCoin(OutPoint outpoint, TxOut txOut, Script redeem, StealthMetadata stealthMetadata,
            BitcoinStealthAddress address)
            : base(outpoint, txOut)
        {
        ...
        public StealthMetadata StealthMetadata { get; set; }

        public BitcoinStealthAddress Address { get; set; }
        ...
        public static StealthCoin Find(Transaction tx, BitcoinStealthAddress address, Key scan)
        {
            ...
            return new StealthCoin(new OutPoint(txId, tx.Outputs.IndexOf(txout)), txout, payment.Redeem,
