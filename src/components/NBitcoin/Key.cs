...
namespace NBitcoin
{
    public class Key : IBitcoinSerializable, IDestination
    {
        ...
        static readonly uint256 N = uint256.Parse("fffffffffffffffffffffffffffffffebaaedce6af48a03bbfd25e8cd0364141");
        ...
        public Key(byte[] data, int count = -1, bool fCompressedIn = true)
        {
            if (count == -1)
                count = data.Length;
            if (count != KEY_SIZE) throw new FormatException("The size of an EC key should be 32");
            if (Check(data))
        ...
        public void ReadWrite(BitcoinStream stream)
        {
            ...
            if (!stream.Serializing) this._ECKey = new ECKey(this.vch, true);
        }
        ...
        void SetBytes(byte[] data, int count, bool fCompressedIn)
        {
        ...
        public Key Derivate(byte[] cc, uint nChild, out byte[] ccChild)
        {
            ...
            if (nChild >> 31 == 0)
            {
                var pubKey = this.PubKey.ToBytes();
