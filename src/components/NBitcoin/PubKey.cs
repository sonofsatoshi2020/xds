...
namespace NBitcoin
{
    public class PubKey : IBitcoinSerializable, IDestination
    {
        ...
        public KeyId Hash
        {
            get
            {
                if (this._ID == null) this._ID = new KeyId(Hashes.Hash160(this.vch, 0, this.vch.Length));
                return this._ID;
            }
        }
        ...
        public WitKeyId WitHash
        {
            get
            {
                if (this._WitID == null) this._WitID = new WitKeyId(Hashes.Hash160(this.vch, 0, this.vch.Length));
                return this._WitID;
            }
        }
        ...
        /// <returns></returns>
        static ECDSASignature DecodeSigString(string signature)
        {
            var signatureEncoded = Encoders.Base64.DecodeData(signature);
            return DecodeSig(signatureEncoded);
        }

        static ECDSASignature DecodeSig(byte[] signatureEncoded)
        {
