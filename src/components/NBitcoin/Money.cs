...
namespace NBitcoin
{
    public static class MoneyExtensions
    {
        public static Money Sum(this IEnumerable<Money> moneys)
        {
            ...
            long result = 0;
            foreach (var money in moneys) result = checked(result + money.Satoshi);
        ...
        public static IMoney Sum(this IEnumerable<IMoney> moneys, IMoney zero)
        {
            ...
            var result = zero;
            foreach (var money in moneys) result = result.Add(money);
            return result;
        }
    ...
    {
        readonly List<IMoney> _bag = new List<IMoney>();
        ...
        MoneyBag(IEnumerable<IMoney> bag)
        {
            foreach (var money in bag) AppendMoney(money);
        }
        ...
        void AppendMoney(MoneyBag money)
        {
            foreach (var m in money._bag) AppendMoney(m);
        }

        void AppendMoney(IMoney money)
        {
            ...
            var firstCompatible = this._bag.FirstOrDefault(x => x.IsCompatible(money));
        ...
        public override string ToString()
        {
            ...
            foreach (var money in this._bag) sb.AppendFormat("{0} ", money);
            return sb.ToString();
        }
        ...
        public IEnumerable<MoneyBag> Split(int parts)
        {
            ...
            foreach (var money in this) splits.Add(money.Split(parts).ToList());

            for (var i = 0; i < parts; i++)
            {
                var bag = new MoneyBag();
                foreach (var split in splits) bag += split[i];
                yield return bag;
            }
        }
    }
    ...
    public class Money : IComparable, IComparable<Money>, IEquatable<Money>, IMoney
    {
        ...
        const NumberStyles BitcoinStyle =
        ...
        public static bool TryParse(string bitcoin, out Money nRet)
        {
            ...
            decimal value;
            if (!decimal.TryParse(bitcoin, BitcoinStyle, CultureInfo.InvariantCulture, out value)) return false;
        ...
        {
            Money result;
            if (TryParse(bitcoin, out result)) return result;
        ...
        public IEnumerable<Money> Split(int parts)
        {
            if (parts <= 0)
                throw new ArgumentOutOfRangeException("Parts should be more than 0", "parts");
            long remain;
            var result = DivRem(this._Satoshis, parts, out remain);
        ...
        public bool Almost(Money amount, decimal margin)
        {
            ...
            var dust = Satoshis(this.Satoshi * margin);
        ...
        static void CheckLongMinValue(long value)
        {
            if (value == long.MinValue)
                throw new OverflowException("satoshis amount should be greater than long.MinValue");
        }

        static void CheckMoneyUnit(MoneyUnit value, string paramName)
        {
    ...
    class BitcoinFormatter : IFormatProvider, ICustomFormatter
    {
        ...
        public string Format(string format, object arg, IFormatProvider formatProvider)
        {
            ...
            if (plus)
                i++;
            var decPos = 0;
            if (int.TryParse(format.Substring(i, 1), out decPos)) i++;
            var unit = format[i];
