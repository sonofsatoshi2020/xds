...
namespace UnnamedCoin.Bitcoin.Features.Consensus.ProvenBlockHeaders
{
    ...
    ///     </para>
    ///     <para>
    ///         Items in the pending batch are also saved to the least recently used
    ///         <see cref="MemorySizeCache{int, ProvenBlockHeader}" />.
    ///         Where the memory size is limited by <see cref="MemoryCacheSizeLimitInBytes" />.
    ///     </para>
    ///     <para>
    ///         When new <see cref="ProvenBlockHeader" /> items are saved to the database - in case
    ///         <see cref="IProvenBlockHeaderRepository" /> contains headers that
    ///         are no longer a part of the best chain - they are overwritten or ignored.
    ///     </para>
    ///     <para>
    ///         When <see cref="ProvenBlockHeaderStore" /> is being initialized it will overwrite blocks that are not on the
    ///         best chain.
    ///     </para>
    /// </remarks>
    public class ProvenBlockHeaderStore : IProvenBlockHeaderStore
    {
        ...
        readonly long MemoryCacheSizeLimitInBytes = 50 * 1024 * 1024;
        ...
        ///     </para>
        /// </remarks>
        readonly SortedDictionary<int, ProvenBlockHeader> pendingBatch;
        ...
        readonly IProvenBlockHeaderRepository provenBlockHeaderRepository;
        ...
        /// <summary>
        ///     Pending - not yet saved to disk - <see cref="IProvenBlockHeaderStore" /> tip hash and height that the
        ///     <see cref="ProvenBlockHeader" /> belongs to.
        ///     <para>All access to these items have to be protected by <see cref="lockObject" /></para>
        /// </summary>
        HashHeightPair pendingTipHashHeight;
        ...
        /// <summary>
        ///     Current <see cref="IProvenBlockHeaderStore" /> tip hash and height that the <see cref="ProvenBlockHeader" />
        ///     belongs to.
        /// </summary>
        ...
        public async Task<ProvenBlockHeader> GetAsync(int blockHeight)
        {
            using (new StopwatchDisposable(o => this.performanceCounter.AddQueryTime(o)))
            {
                ...
                {
                    if (this.pendingBatch.TryGetValue(blockHeight, out var headerFromBatch))
                    {
                ...
                if (this.Cache.TryGetValue(blockHeight, out var header))
                {
        ...
        public async Task SaveAsync()
        {
            ...
            try
            {
                ...
                {
                    // Save the items to disk.
                    await this.provenBlockHeaderRepository.PutAsync(pendingBatchInsert, hashHeight)
                ...
                if (this.initialBlockDownloadState.IsInitialBlockDownload())
                    ...
                    foreach (var provenBlockHeader in pendingBatchInsert)
        ...
        void CheckItemsAreInConsecutiveSequence(List<int> keys)
        {
            if (!keys.SequenceEqual(Enumerable.Range(keys.First(), keys.Count)))
            {
                this.logger.LogError("(-)[PROVEN_BLOCK_HEADERS_NOT_IN_CONSECUTIVE_SEQUENCE]: {0}",
                    string.Join(",", keys));
                throw new ProvenBlockHeaderException(
                    "Proven block headers are not in the correct consecutive sequence.");
            }
        }
        ...
        void AddComponentStats(StringBuilder log)
        {
            ...
            var totalCacheInMb = Convert.ToDecimal(this.Cache.TotalSize / Math.Pow(2, 20));
            var totalMaxCacheInMb = Convert.ToDecimal(this.Cache.MaxSize / Math.Pow(2, 20));
            var totalBatchInMb = Convert.ToDecimal(totalBytes / Math.Pow(2, 20));
