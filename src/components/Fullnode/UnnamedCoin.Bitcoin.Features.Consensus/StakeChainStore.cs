...
namespace UnnamedCoin.Bitcoin.Features.Consensus
{
    public class StakeChainStore : IStakeChain
    {
        ...
        readonly DBreezeCoinView dBreezeCoinView;

        readonly BlockStake genesis;

        readonly ConcurrentDictionary<uint256, StakeItem> items = new ConcurrentDictionary<uint256, StakeItem>();
        ...
        readonly int thresholdWindow;

        public StakeChainStore(Network network, ChainIndexer chainIndexer, DBreezeCoinView dBreezeCoinView,
        ...
        public void Load()
        {
            var hash = this.dBreezeCoinView.GetTipHash();
            var currentHeader = this.chainIndexer.GetHeader(hash);
            ...
            if (load.Any(l => l.BlockStake == null))
            {
                ...
                throw new ConfigurationException(
                    "Missing stake information, delete the data folder and re-download the chain");
            }
        ...
        public virtual BlockStake Get(uint256 blockid)
        {
            ...
            {
                this.logger.LogTrace("(-)[GENESIS]:*.{0}='{1}'", nameof(this.genesis.HashProof),
                    this.genesis.HashProof);
                return this.genesis;
            }
            ...
            if (block != null)
            {
                this.logger.LogTrace("(-)[LOADED]:*.{0}='{1}'", nameof(block.BlockStake.HashProof),
                    block.BlockStake.HashProof);
        ...
        public void Set(ChainedHeader chainedHeader, BlockStake blockStake)
        {
            ...
            {
                BlockId = chainedHeader.HashBlock, Height = chainedHeader.Height, BlockStake = blockStake,
                InStore = false
            };
            var added = this.items.TryAdd(chainedHeader.HashBlock, item);
            if (added)
                Flush(false);
        }
        ...
        public void Flush(bool disposeMode)
        {
            ...
            if (disposeMode || count > this.threshold)
            {
                ...
                var oldes = select.OrderBy(o => o.Value.Height).Take(this.thresholdWindow);
                StakeItem unused;
