...
namespace UnnamedCoin.Bitcoin.Features.Consensus.Rules.ProvenHeaderRules
{
    /// <summary>
    ///     Rule to check if the coinstake inside proven header is valid:
    ///     <list type="number">
    ///         <item>
    ///             <description>Check that coinstake tx has IsCoinStake property equal to true.</description>
    ///         </item>
    ///         <item>
    ///             <description>Header time is equal to the timestamp of the coinstake tx.</description>
    ///         </item>
    ///         <item>
    ///             <description>Check if coinstake tx timestamp is divisible by 16 (using timestamp mask).</description>
    ///         </item>
    ///         <item>
    ///             <description>Verify the coinstake age requirement (*).</description>
    ///         </item>
    ///         <item>
    ///             <description>
    ...
    ///                 <list type="number">
    ///                     <item>
    ///                         <description>Input comes from a UTXO.</description>
    ///                     </item>
    ///                     <item>
    ///                         <description>Verify the ScriptSig</description>
    ///                     </item>
    ///                 </list>
    ///             </description>
    ///         </item>
    ///         <item>
    ///             <description>Check if the coinstake kernel hash satisfies the difficulty requirement (*).</description>
    ///         </item>
    ///         <item>
    ///             <description>Check that coinstake tx is in the merkle tree using the merkle proof.</description>
    ///         </item>
    ///         <item>
    ///             <description>Verify header signature with the key from coinstake kernel (*).</description>
    ///         </item>
    ///     </list>
    /// </summary>
    /// <remarks>(*) - denotes rules that are expensive to execute.</remarks>
    /// <seealso cref="T:UnnamedCoin.Bitcoin.Features.Consensus.Rules.ProvenHeaderRules.ProvenHeaderRuleBase" />
    public class ProvenHeaderCoinstakeRule : ProvenHeaderRuleBase
    {
        ...
        /// </exception>
        void CheckCoinstakeIsNotNull(ProvenBlockHeader header)
        {
        ...
        UnspentOutputs GetAndValidatePreviousUtxo(ProvenBlockHeader header, PosRuleContext context)
        {
            // First try and find the previous trx in the database.
            var txIn = header.Coinstake.Inputs[0];

            UnspentOutputs prevUtxo = null;

            var coins = this.PosParent.UtxoSet.FetchCoins(new[] {txIn.PrevOut.Hash});
        ...
        /// </exception>
        void CheckIfCoinstakeIsTrue(ProvenBlockHeader header)
        {
            if (!header.Coinstake.IsCoinStake)
            {
        ...
        /// </exception>
        void CheckHeaderAndCoinstakeTimes(ProvenBlockHeader header)
        {
        ...
        /// </exception>
        void CheckCoinstakeAgeRequirement(ChainedHeader chainedHeader, UnspentOutputs unspentOutputs)
        {
            var prevChainedHeader = chainedHeader.Previous;

            var options = (PosConsensusOptions) this.PosParent.Network.Consensus.Options;
            var targetDepth = options.GetStakeMinConfirmations(chainedHeader.Height, this.PosParent.Network) - 1;
        ...
        /// </exception>
        void CheckSignature(ProvenBlockHeader header, UnspentOutputs unspentOutputs)
        {
        ...
        uint256 GetPreviousStakeModifier(ChainedHeader chainedHeader)
        {
            ...
            var previousStakeModifier =
                this.PosParent.StakeChain.Get(chainedHeader.Previous.HashBlock)?.StakeModifierV2;
        ...
        /// </exception>
        void CheckCoinstakeMerkleProof(ProvenBlockHeader header)
        {
        ...
        /// </exception>
        void CheckHeaderSignatureWithCoinstakeKernel(ProvenBlockHeader header)
        {
        ...
        UnspentOutputs CheckIfCoinstakeIsSpentOnAnotherChain(ProvenBlockHeader header, PosRuleContext context)
        {
            var coinstake = header.Coinstake;
            var input = coinstake.Inputs[0];

            var rewindDataIndex = this.PosParent.RewindDataIndexCache.Get(input.PrevOut.Hash, (int) input.PrevOut.N);
            ...
            var rewindData = this.PosParent.UtxoSet.GetRewindData(rewindDataIndex.Value);
        ...
        OutPoint GetPreviousOut(ProvenBlockHeader header)
        {
            var input = header.Coinstake.Inputs[0];
