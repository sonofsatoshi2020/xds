...
{
    /// <summary>
    ///     A rule that will verify the block time drift is according to the PoS consensus rules for the
    ///     <see cref="StratisMain" /> network (and its test networks).
    ...
    public class StratisBugFixPosFutureDriftRule : PosFutureDriftRule
    {
        ...
        const int BugFutureDriftSeconds = 128 * 60 * 60;
        ...
        static bool IsDriftReduced(long time)
        {
            // This is a specific  bug fix where the blockchain drifted 24 hour ahead as the protocol allowed that.
    ...
    public class PosFutureDriftRule : HeaderValidationConsensusRule
    {
        ...
        public override void Run(RuleContext context)
        {
            ...
            var adjustedTime = this.Parent.DateTimeProvider.GetAdjustedTimeAsUnixTimestamp();

            // Check timestamp.
            if (header.Time > adjustedTime + GetFutureDrift(adjustedTime))
            {
                // The block can be valid only after its time minus the future drift.
                context.ValidationContext.RejectUntil =
                    Utils.UnixTimeToDateTime(header.Time - GetFutureDrift(header.Time)).UtcDateTime;
