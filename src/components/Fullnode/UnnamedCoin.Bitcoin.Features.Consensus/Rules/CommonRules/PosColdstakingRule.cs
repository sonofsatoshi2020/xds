...
namespace UnnamedCoin.Bitcoin.Features.Consensus.Rules.CommonRules
{
    ...
    ///         it is set in <see cref="DeploymentFlags.ScriptFlags" /> when the block height is greater than or equal to
    ///         <see
    ///             cref="PosConsensusOptions.ColdStakingActivationHeight" />
    ///         .
    ///     </para>
    ///     <para>
    ///         The following conditions are enforced for cold staking transactions. This rule implements all but the first
    ///         one:
    ///         <list type="number">
    ///             <item>
    ///                 Check if the transaction spending an output, which contains this instruction, is a coinstake
    ///                 transaction.
    ///                 If it is not, the script fails. This has already been checked within the opcode implementation before
    ///                 setting
    ///                 <see cref="PosTransaction.IsColdCoinStake" /> so it will not be checked here.
    ///             </item>
    ///             <item>
    ///                 Check that ScriptPubKeys of all inputs of this transaction are the same. If they are not, the script
    ///                 fails.
    ///             </item>
    ///             <item>
    ///                 Check that ScriptPubKeys of all outputs of this transaction, except for the marker output (a special
    ///                 first output of each coinstake transaction) and the pubkey output (an optional special second output
    ///                 that
    ///                 contains public key in coinstake transaction), are the same as ScriptPubKeys of the inputs. If they are
    ///                 not,
    ///                 the script fails.
    ///             </item>
    ///             <item>
    ///                 Check that the sum of values of all inputs is smaller or equal to the sum of values of all outputs. If
    ///                 this does not hold, the script fails.
    ///             </item>
    ///         </list>
    ///     </para>
    /// </remarks>
    public class PosColdStakingRule : UtxoStoreConsensusRule
    {
        ...
        /// <exception cref="ConsensusErrors.BadColdstakeAmount">
        ///     Thrown if the total input is smaller or equal than the sum of
        ///     outputs.
        /// </exception>
        public override Task RunAsync(RuleContext context)
        {
            ...
            var coinstakeTransaction = (block.Transactions.Count >= 2 ? block.Transactions[1] : null) as PosTransaction;
            ...
            var scriptPubKey = posRuleContext.CoinStakePrevOutputs[coinstakeTransaction.Inputs[0]].ScriptPubKey;
            for (var i = 1; i < coinstakeTransaction.Inputs.Count; i++)
            ...
            for (var i = 2; i < coinstakeTransaction.Outputs.Count; i++)
