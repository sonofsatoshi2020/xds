...
namespace UnnamedCoin.Bitcoin.Features.Consensus.Rules.CommonRules
{
    ...
    public class BlockMerkleRootRule : IntegrityValidationConsensusRule
    {
        ...
        /// <exception cref="ConsensusErrors.BadTransactionDuplicate">
        ///     One of the leaf nodes of the merkle tree has a duplicate hash
        ///     within the subtree.
        /// </exception>
        public override void Run(RuleContext context)
        {
            ...
            var hashMerkleRoot2 = BlockMerkleRoot(block, out var mutated);
        ...
        /// <param name="mutated">
        ///     <c>true</c> if block contains repeating sequences of transactions without affecting the merkle
        ///     root of a block. Otherwise: <c>false</c>.
        /// </param>
        ...
        public static uint256 ComputeMerkleRoot(List<uint256> leaves, out bool mutated)
        {
            ...
            for (var i = 0; i < subTreeHashes.Length; i++)
            ...
            {
                var currentLeaveHash = leaves[(int) processedLeavesCount];
                var match = false;
