...
namespace UnnamedCoin.Bitcoin.Features.Consensus.Rules.CommonRules
{
    ...
    ///     This rule is only activated after the POW epoch is finished according to the value in
    ///     <see cref="Consensus.LastPOWBlock" />.
    /// </summary>
    public class CheckDifficultyPosRule : HeaderValidationConsensusRule
    {
        ...
        public override void Run(RuleContext context)
        {
            ...
            {
                var first = chainedHeader.Previous.Header;
                var second = chainedHeader.Previous.Previous.Header;

                var nextWorkRequired = this.PosParent.StakeValidator.CalculateRetarget(first.Time, first.Bits,
                    second.Time, this.Parent.Network.Consensus.ProofOfStakeLimitV2);
