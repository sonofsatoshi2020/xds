...
namespace UnnamedCoin.Bitcoin.Features.Consensus.Rules.CommonRules
{
    ...
    public class PosCoinstakeRule : PartialValidationConsensusRule
    {
        ...
        /// <exception cref="ConsensusErrors.BlockTimeBeforeTrx">
        ///     The block contains a transaction with a timestamp after the block
        ///     timestamp.
        /// </exception>
        public override Task RunAsync(RuleContext context)
        {
            ...
            if (BlockStake.IsProofOfStake(block))
            {
                ...
                if (block.Transactions[0].Outputs.Count != 1 || !block.Transactions[0].Outputs[0].IsEmpty)
                {
                    ...
                    foreach (var txOut in block.Transactions[0].Outputs.Skip(1))
                ...
                if (transaction is IPosTransactionWithTime posTrx)
                    ...
                    if (block.Header.Time < posTrx.Time)
                    {
                        this.Logger.LogDebug(
                            "Block contains transaction with timestamp {0}, which is greater than block's timestamp {1}.",
                            posTrx.Time, block.Header.Time);
