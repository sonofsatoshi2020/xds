...
namespace UnnamedCoin.Bitcoin.Features.Consensus.Rules.CommonRules
{
    ...
    /// </summary>
    /// <exception cref="ConsensusErrors.BadTransactionMissingInput">
    ///     Thrown if transaction tries to spend inputs that are
    ///     missing.
    /// </exception>
    /// <exception cref="ConsensusErrors.BadTransactionNonFinal">
    ///     Thrown if transaction's height or time is lower then provided
    ///     by SequenceLock for this block.
    /// </exception>
    /// <exception cref="ConsensusErrors.BadBlockSigOps">
    ///     Thrown if signature operation cost is greater then maximum block
    ///     signature operation cost.
    /// </exception>
    /// <exception cref="ConsensusErrors.BadTransactionScriptError">
    ///     Thrown if not all inputs are valid (no double spends,
    ...
    public abstract class CoinViewRule : FullValidationConsensusRule
    {
        ...
        public override async Task RunAsync(RuleContext context)
        {
            ...
            var fees = Money.Zero;
            var inputsToCheck =
                new List<(Transaction tx, int inputIndexCopy, TxOut txOut, PrecomputedTransactionData txData, TxIn input
            ...
            if (!context.SkipValidation)
            {
                ...
                var checkInputsInParallel = Task.Run(() =>
                {
                    return Parallel.ForEach(inputsToCheck, (input, state) =>
                    {
                        ...
                        if (!CheckInput(input.tx, input.inputIndexCopy, input.txOut, input.txData, input.input,
                            input.flags)) state.Stop();
                    });
                });

                var loopResult = await checkInputsInParallel.ConfigureAwait(false);
            ...
            {
                this.Logger.LogDebug("BIP68, SigOp cost, and block reward validation skipped for block at height {0}.",
                    index.Height);
            }
        }
        ...
        ///     Transaction which outputs will be added to the context's <see cref="UnspentOutputSet" /> and
        ///     which inputs will be removed from it.
        /// </param>
        ...
        /// <exception cref="ConsensusErrors.BadCoinbaseAmount">
        ///     Thrown if coinbase transaction output value is larger than
        ///     expected.
        /// </exception>
        ...
        /// <exception cref="ConsensusErrors.BadTransactionPrematureCoinbaseSpending">
        ///     Thrown if transaction tries to spend coins
        ///     that are not mature.
        /// </exception>
        public void CheckCoinbaseMaturity(UnspentOutputs coins, int spendHeight)
        {
            ...
            if (coins.IsCoinbase)
                ...
                {
                    this.Logger.LogDebug(
                        "Coinbase transaction height {0} spent at height {1}, but maturity is set to {2}.",
        ...
        public void CheckInputs(Transaction transaction, UnspentOutputSet inputs, int spendHeight)
        {
            ...
            var valueIn = Money.Zero;
            var fees = Money.Zero;
            for (var i = 0; i < transaction.Inputs.Count; i++)
            {
                var prevout = transaction.Inputs[i].PrevOut;
                ...
                if (!MoneyRange(coins.TryGetOutput(prevout.N).Value) || !MoneyRange(valueIn))
                {
            ...
            if (!transaction.IsProtocolTransaction())
            {
                ...
                var txFee = valueIn - transaction.TotalOut;
                if (txFee < 0)
                {
        ...
        public long GetTransactionSignatureOperationCost(Transaction transaction, UnspentOutputSet inputs,
            DeploymentFlags flags)
        {
            var signatureOperationCost =
                GetLegacySignatureOperationsCount(transaction) * this.ConsensusOptions.WitnessScaleFactor;

            if (transaction.IsCoinBase)
                return signatureOperationCost;

            if (flags.ScriptFlags.HasFlag(ScriptVerify.P2SH))
                signatureOperationCost += GetP2SHSignatureOperationsCount(transaction, inputs) *
                                          this.ConsensusOptions.WitnessScaleFactor;

            for (var i = 0; i < transaction.Inputs.Count; i++)
            {
                var prevout = inputs.GetOutputFor(transaction.Inputs[i]);
                signatureOperationCost +=
                    CountWitnessSignatureOperation(prevout.ScriptPubKey, transaction.Inputs[i].WitScript, flags);
            }
        ...
        long CountWitnessSignatureOperation(Script scriptPubKey, WitScript witness, DeploymentFlags flags)
        {
            witness = witness ?? WitScript.Empty;
            if (!flags.ScriptFlags.HasFlag(ScriptVerify.Witness))
                return 0;

            var witParams = PayToWitTemplate.Instance.ExtractScriptPubKeyParameters2(this.Parent.Network, scriptPubKey);
            ...
            if (witParams?.Version == 0)
            {
                ...
                {
                    var subscript = Script.FromBytesUnsafe(witness.GetUnsafePush(witness.PushCount - 1));
        ...
        uint GetP2SHSignatureOperationsCount(Transaction transaction, UnspentOutputSet inputs)
        {
            if (transaction.IsCoinBase)
                return 0;
        ...
        long GetLegacySignatureOperationsCount(Transaction transaction)
        {
            long sigOps = 0;
