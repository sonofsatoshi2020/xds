...
namespace UnnamedCoin.Bitcoin.Features.Consensus.Interfaces
{
    /// <summary>
    ...
    public interface IStakeValidator
    {
        ...
        bool CheckKernel(PosRuleContext context, ChainedHeader prevChainedHeader, uint headerBits, long transactionTime,
            OutPoint prevout);
        ...
        ///         <list type="number">
        ///             <item>
        ///                 <paramref name="prevStakeModifier" />: Scrambles computation to make it very difficult to precompute
        ///                 future proof-of-stake.
        ///             </item>
        ///             <item><paramref name="stakingCoins.Time" />: Time of the coinstake UTXO. Slightly scrambles computation.</item>
        ///             <item>
        ///                 <paramref name="prevout.Hash" /> Hash of stakingCoins UTXO, to reduce the chance of nodes generating
        ///                 coinstake at the same time.
        ///             </item>
        ///             <item>
        ///                 <paramref name="prevout.N" />: Output number of stakingCoins UTXO, to reduce the chance of nodes
        ...
        ///     </para>
        /// </remarks>
        /// <exception cref="ConsensusErrors.StakeTimeViolation">
        ///     Thrown in case transaction time is lower than it's own UTXO
        ///     timestamp.
        /// </exception>
        /// <exception cref="ConsensusErrors.StakeHashInvalidTarget">Thrown in case PoS hash doesn't meet target protocol.</exception>
        /// <returns><c>true</c> if the coin stake satisfies the weighted target, otherwise <c>false</c>.</returns>
        bool CheckStakeKernelHash(PosRuleContext context, uint headerBits, uint256 prevStakeModifier,
        ...
        /// <param name="headerBits">Chained block's header bits, which define the difficulty target.</param>
        void CheckProofOfStake(PosRuleContext context, ChainedHeader prevChainedHeader, BlockStake prevBlockStake,
            Transaction transaction, uint headerBits);
        ...
        /// <param name="proofOfStake">
        ///     Specifies what kind of block we are looking for: <c>true</c> for PoS or <c>false</c> for
        ///     PoW.
        /// </param>
        ...
        ChainedHeader GetLastPowPosChainedBlock(IStakeChain stakeChain, ChainedHeader startChainedHeader,
            bool proofOfStake);
        ...
        /// <param name="proofOfStake">
        ///     <c>true</c> for calculation of PoS difficulty target, <c>false</c> for calculation of PoW
        ///     difficulty target.
        /// </param>
        /// <returns>The difficulty target for the next block after <paramref name="chainedHeader" />.</returns>
        Target GetNextTargetRequired(IStakeChain stakeChain, ChainedHeader chainedHeader, IConsensus consensus,
            bool proofOfStake);
        ...
        /// <remarks>
        ///     The calculation of the next target is based on the last target value and the block time (aka spacing) of
        ///     <paramref name="firstBlockTime" />
        ///     (i.e. difference in time stamp of this block and its immediate predecessor). The target changes every block and it
        ///     is adjusted
        ///     down (i.e. towards harder to reach, or more difficult) if the time to mine last block was lower than the target
        ///     block time.
        ///     And it is adjusted up if it took longer than the target block time. The adjustments are done in a way the target is
        ///     moving towards
        ///     the target-spacing (expected block time) exponentially, so even a big change in the mining power on the network
        ///     will be fixed by retargeting relatively quickly.
        ///     <para>
        ///         Over <see cref="RetargetIntervalMinutes" /> minutes there are certain number (say <c>N</c>) of blocks expected
        ///         to be mined if the target block time
        ///         of <see cref="TargetSpacingSeconds" /> was reached every time. Then the next target is calculated as follows:
        ///     </para>
        ///     <code>
        ...
        ///     </para>
        /// </remarks>
        Target CalculateRetarget(uint firstBlockTime, Target firstBlockTarget, uint secondBlockTime,
        ...
        /// <returns>
        ///     <c>true</c> if the coins were spent within N blocks from <see cref="referenceChainedHeader" />, <c>false</c>
        ///     otherwise.
        /// </returns>
