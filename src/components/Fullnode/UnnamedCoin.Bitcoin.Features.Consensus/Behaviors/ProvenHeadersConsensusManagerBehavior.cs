...
namespace UnnamedCoin.Bitcoin.Features.Consensus.Behaviors
{
    ...
    public class ProvenHeadersConsensusManagerBehavior : ConsensusManagerBehavior
    {
        ...
        readonly bool isGateway;

        readonly int lastCheckpointHeight;
        readonly CheckpointInfo lastCheckpointInfo;
        ...
        public ProvenHeadersConsensusManagerBehavior(
            ...
            ConnectionManagerSettings connectionManagerSettings) : base(chainIndexer, initialBlockDownloadState,
            consensusManager, peerBanning, loggerFactory)
        {
        ...
        protected override async Task OnMessageReceivedAsync(INetworkPeer peer, IncomingMessage message)
        {
            switch (message.Message.Payload)
            {
                case ProvenHeadersPayload provenHeaders:
                    await ProcessHeadersAsync(peer, provenHeaders.Headers.Cast<BlockHeader>().ToList())
        ...
        {
            var height = this.ConsensusManager.Tip.Height;
            ...
            if (height > this.lastCheckpointHeight)
            {
                ...
                var tryCacheEveryBlocksCount = this.network.Consensus.MaxReorgLength / 5;

                // After last checkpoint.
                if (this.BestReceivedTip == null || height % tryCacheEveryBlocksCount == 0 ||
                    height >= this.BestReceivedTip.Height)
                    return true;
            }
        ...
        /// <inheritdoc />
        protected override Payload ConstructHeadersPayload(GetHeadersPayload getHeadersPayload,
            out ChainedHeader lastHeader)
        {
            ...
            if (!(getHeadersPayload is GetProvenHeadersPayload))
            {
                ...
                for (var i = 0; i < headersPayload.Headers.Count; i++)
                    if (headersPayload.Headers[i] is ProvenBlockHeader phHeader)
                    {
                        var newHeader = this.ChainIndexer.Network.Consensus.ConsensusFactory.CreateBlockHeader();
            ...
            for (var heightIndex = header.Height; heightIndex > fork.Height; heightIndex--)
            {
                if (!(header.Header is ProvenBlockHeader provenBlockHeader))
                {
                    ...
                    if (provenBlockHeader.GetHash() != header.HashBlock)
                    {
                        ...
                        this.logger.LogDebug("Stored PH hash is wrong. Expected: {0}, Found: {1}",
                            header.Header.GetHash(), provenBlockHeader.GetHash());
        ...
        /// </returns>
        bool CanPeerProcessProvenHeaders(INetworkPeer peer)
        {
            return peer.Version >= ProtocolVersion.PROVEN_HEADER_VERSION;
        }
        ...
        /// </summary>
        /// <returns>
        ///     The <see cref="GetHeadersPayload" /> instance.
        ///     If the node is a gateway and the peer is not whitelisted, return null (gateways can sync only from whitelisted
        ///     peers).
        ///     If the peer can serve PH, <see cref="GetProvenHeadersPayload" /> is returned, otherwise if it's a legacy peer but
        ///     it's whitelisted,
        ...
        protected override GetHeadersPayload BuildGetHeadersPayload()
        {
            ...
            var aboveLastCheckpoint = GetCurrentHeight() >= this.lastCheckpointHeight;
        ...
        protected async Task ProcessLegacyHeadersAsync(INetworkPeer peer, List<BlockHeader> headers)
        {
            ...
            var belowLastCheckpoint = GetCurrentHeight() <= this.lastCheckpointHeight;
            ...
            var distanceFromCheckPoint = this.lastCheckpointHeight - GetCurrentHeight();
            if (distanceFromCheckPoint < MaxItemsPerHeadersMessage)
            {
                ...
                for (var index = 0; index < headers.Count; index++)
                    ...
                    {
                        if (index != headers.Count - 1) headers.RemoveRange(index + 1, headers.Count - index - 1);
