...
namespace UnnamedCoin.Bitcoin.Features.Consensus.CoinViews
{
    ...
    public class BackendPerformanceCounter
    {
        ...
        {
            var snap = new BackendPerformanceSnapshot(this.insertedEntities, this.insertTime, this.queriedEntities,
                this.queryTime)
            {
                Start = this.Start,
    ...
    public class BackendPerformanceSnapshot
    {
        ...
        /// <summary>Time in ticks it took the database to perform insert operations.</summary>
        readonly long totalInsertTime;

        /// <summary>Number of queried entities from the database.</summary>
        readonly long totalQueriedEntities;

        /// <summary>Time in ticks it took the database to perform query operations.</summary>
        readonly long totalQueryTime;
        ...
        /// <remarks>
        ///     The two snapshots should be taken from a single performance counter.
        ///     Otherwise the start times of the snapshots will be different, which is not allowed.
        /// </remarks>
        public static BackendPerformanceSnapshot operator -(BackendPerformanceSnapshot end,
            BackendPerformanceSnapshot start)
        {
            ...
            var insertedEntities = end.totalInsertedEntities - start.totalInsertedEntities;
            var insertTime = end.totalInsertTime - start.totalInsertTime;
            var queriedEntities = end.totalQueriedEntities - start.totalQueriedEntities;
            var queryTime = end.totalQueryTime - start.totalQueryTime;
        ...
        public override string ToString()
        {
            ...
            if (this.TotalInsertedEntities > 0)
                builder.AppendLine("Insert speed:".PadRight(LoggingConfiguration.ColumnLength) +
                                   (this.TotalInsertTime.TotalMilliseconds / this.TotalInsertedEntities).ToString(
                                       "0.0000") + " ms/utxo");

            builder.AppendLine("Insert time:".PadRight(LoggingConfiguration.ColumnLength) +
                               TimespanToString(this.TotalInsertTime));
            builder.AppendLine(
                "Inserted UTXO:".PadRight(LoggingConfiguration.ColumnLength) + this.TotalInsertedEntities);

            if (this.TotalQueriedEntities > 0)
                builder.AppendLine("Query speed:".PadRight(LoggingConfiguration.ColumnLength) +
                                   (this.TotalQueryTime.TotalMilliseconds / this.TotalQueriedEntities).ToString(
                                       "0.0000") + " ms/utxo");

            builder.AppendLine("Query time:".PadRight(LoggingConfiguration.ColumnLength) +
                               TimespanToString(this.TotalQueryTime));
