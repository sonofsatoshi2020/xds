...
namespace UnnamedCoin.Bitcoin.Features.Consensus.CoinViews
{
    ...
    {
        /// <summary>Number of cache queries for which the result was found in the cache.</summary>
        readonly long hitCount;

        /// <summary>Number of cache queries for which the result was not found in the cache.</summary>
        readonly long missCount;
        ...
        /// <summary>Number of cache queries for which the result was found in the cache.</summary>
        public long TotalHitCount => this.hitCount;

        /// <summary>Number of cache queries for which the result was not found in the cache.</summary>
        public long TotalMissCount => this.missCount;
        ...
        public static CachePerformanceSnapshot operator -(CachePerformanceSnapshot end, CachePerformanceSnapshot start)
        {
            ...
            var missCount = end.missCount - start.missCount;
            var hitCount = end.hitCount - start.hitCount;
        ...
        public override string ToString()
        {
            ...
            if (total != 0)
            {
                builder.AppendLine("Hit:".PadRight(LoggingConfiguration.ColumnLength) +
                                   (this.TotalHitCount * 100m / total).ToString("0.00") + " %");
                builder.AppendLine("Miss:".PadRight(LoggingConfiguration.ColumnLength) +
                                   (this.TotalMissCount * 100m / total).ToString("0.00") + " %");
            }
