...
namespace UnnamedCoin.Bitcoin.Features.RPC.ModelBinders
{
    public class DestinationModelBinder : IModelBinder, IModelBinderProvider
    {
        ...
        {
            if (!SupportType(bindingContext.ModelType)) return Task.CompletedTask;
            ...
            if (!bindingContext.ModelType.IsInstanceOfType(data)) throw new FormatException("Invalid destination type");
        ...
        static bool SupportType(Type type)
        {
