...
namespace UnnamedCoin.Bitcoin.Features.RPC
{
    ...
    */
    ...
    {
        static readonly ConcurrentDictionary<Network, string>
            defaultPaths = new ConcurrentDictionary<Network, string>();

        string authentication;
        ConcurrentQueue<(RPCRequest request, TaskCompletionSource<RPCResponse> task)> batchedRequests;

        RPCClient(RPCCredentialString credentials, Uri address, Network network)
        {
        ...
        void RPCClientInit(Network network)
        {
            ...
            {
                var bitcoinFolder = Path.Combine(home, "." + network.Name.ToLower());
        ...
        Uri ReplaceHostName(Uri originalUri, string newHostName)
        {
        ...
        public async Task<RPCCapabilities> ScanRPCCapabilitiesAsync()
        {
            ...
            var waiting = Task.WhenAll(
                ...
                CheckCapabilities(rpc, "signrawtransactionwithkey",
                    v => capabilities.SupportSignRawTransactionWith = v),
        ...
        async Task SetVersionAsync(RPCCapabilities capabilities)
        {
            try
            {
                ...
                return;
                                          ...
                                          ex.RPCCode == RPCErrorCode.RPC_METHOD_DEPRECATED)
            {
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        static async Task CheckSegwitCapabilitiesAsync(RPCClient rpc, Action<bool> setResult)
        {
            ...
            try
            {
                var result = await rpc.SendCommandAsync("validateaddress", address.ToString());
                result.ThrowIfError();
        ...
        static async Task CheckCapabilitiesAsync(Func<Task> command, Action<bool> setResult)
        {
            try
            {
                await command();
        ...
        static Task CheckCapabilities(RPCClient rpc, string command, Action<bool> setResult)
        {
        ...
        {
            return defaultPaths.TryGetValue(network, out var path) ? path : null;
        }
        ...
        async Task SendBatchAsyncCoreAsync(List<(RPCRequest request, TaskCompletionSource<RPCResponse> task)> requests)
        {
        ...
        void TryRenewCookie(WebException ex)
        {
            if (GetCookiePath() == null)
                throw new InvalidOperationException("Bug in NBitcoin notify the developers");
        ...
        async Task<RPCResponse> SendCommandAsyncCoreAsync(RPCRequest request, bool throwIfRPCError)
        {
            RPCResponse response = null;
            var batches = this.batchedRequests;
        ...
        HttpWebRequest CreateWebRequest()
        {
            ...
            webRequest.Headers[HttpRequestHeader.Authorization] =
                "Basic " + Encoders.Base64.EncodeData(Encoders.ASCII.DecodeData(this.authentication));
            webRequest.ContentType = "application/json-rpc";
            webRequest.Method = "POST";
            return webRequest;
        }

        async Task<Stream> ToMemoryStreamAsync(Stream stream)
        {
        ...
        public async Task<PeerInfo[]> GetPeersInfoAsync()
        {
            ...
            foreach (var peer in peers)
            {
                ...
                result[i++] = new PeerInfo
                {
                    ...
                    Inflight = peer["inflight"] != null
                        ? peer["inflight"].Select(x => uint.Parse((string) x)).ToArray()
                        : new uint[] { }
                };
            }
        ...
        /// <remarks>
        ///     This method is not compatible with stratisX, which always returns a JSON object no matter what parameters are
        ...
        public async Task<Block> GetBlockAsync(uint256 blockId)
        {
            ...
            var resp = await SendCommandAsync(RPCOperations.getblock, blockId.ToString(), 0).ConfigureAwait(false);
        ...
        static BlockHeader ParseBlockHeader(RPCResponse resp, Network network)
        {
        ...
        {
            var response = SendCommand(RPCOperations.gettxout, txid.ToString(), vout, includeMemPool);
        ...
        {
            var response = await SendCommandAsync(RPCOperations.gettxout, txid.ToString(), vout, includeMemPool)
                .ConfigureAwait(false);
            var responseObject = response.Result as JObject;
            if (responseObject == null)
                return null;
        ...
        public IEnumerable<Transaction> GetTransactions(uint256 blockHash)
        {
            ...
            if (tx != null)
                ...
                {
                    var result = GetRawTransaction(uint256.Parse(item.ToString()), null, false);
                    if (result != null)
                        yield return result;
                }
        }
        ...
        public async Task<Transaction> GetRawTransactionAsync(uint256 txid, uint256 blockHash = null,
            bool throwIfNotFound = true)
        {
            var response =
                await SendCommandAsync(
                    new RPCRequest(RPCOperations.getrawtransaction, new[] {txid.ToString(), blockHash?.ToString()}),
                    throwIfNotFound).ConfigureAwait(false);
        ...
        {
            var res = SendCommand(RPCOperations.validateaddress, address.ToString());
        ...
        async Task<FeeRate> EstimateFeeRateImplAsync(int nblock)
        {
            var response = await SendCommandAsync(RPCOperations.estimatefee, nblock).ConfigureAwait(false);
            var result = response.Result.Value<decimal>();
            var money = Money.Coins(result);
        ...
        /// <param name="commentDest">
        ///     A locally-stored (not broadcast) comment assigned to this transaction. Meant to be used for
        ///     describing who the payment was sent to. Default is no comment
        /// </param>
        /// <returns>The TXID of the sent transaction</returns>
        public uint256 SendToAddress(BitcoinAddress address, Money amount, string commentTx = null,
        ...
        /// </param>
        /// <returns>The TXID of the sent transaction</returns>
        public async Task<uint256> SendToAddressAsync(BitcoinAddress address, Money amount, string commentTx = null,
            string commentDest = null)
        {
        ...
        {
            return SendCommand(RPCOperations.settxfee, feeRate.FeePerK.ToString()).Result.ToString() == "true";
        }
