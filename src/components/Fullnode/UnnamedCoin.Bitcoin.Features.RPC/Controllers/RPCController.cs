...
namespace UnnamedCoin.Bitcoin.Features.RPC.Controllers
{
    ...
    public class RPCController : Controller
    {
        ...
        readonly RpcSettings rpcSettings;
        ...
        Dictionary<string, ControllerActionDescriptor> ActionDescriptors { get; set; }

        /// <summary>
        ///     Returns the collection of available action descriptors.
        /// </summary>
        Dictionary<string, ControllerActionDescriptor> GetActionDescriptors()
        {
            if (this.ActionDescriptors == null)
            {
                ...
                var actionDescriptorProvider =
                    this.fullNode?.RPCHost.Services.GetService(typeof(IActionDescriptorCollectionProvider)) as
                ...
                foreach (var actionDescriptor in actionDescriptorProvider?.ActionDescriptors.Items
        ...
        RPCResponse SendRPCRequest(RPCRequest request)
        {
            ...
            var nodeEndPoint = this.rpcSettings.Bind.Where(b => b.Address.ToString() == "127.0.0.1").FirstOrDefault() ??
                               this.rpcSettings.Bind[0];
            var rpcClient = this.rpcClientFactory.Create(this.rpcSettings, new Uri($"http://{nodeEndPoint}"),
        ...
        public IActionResult ListMethods()
        {
            try
            {
                ...
                var listMethods = new List<RpcCommandModel>();
                foreach (var descriptor in GetActionDescriptors().Values
                    .Where(desc => desc.ActionName == desc.ActionName.ToLower()))
                {
                    var attr = descriptor.MethodInfo.CustomAttributes
                        .Where(x => x.AttributeType == typeof(ActionDescription)).FirstOrDefault();
                    ...
                    foreach (var param in descriptor.Parameters.OfType<ControllerParameterDescriptor>())
                        ...
                        {
                            var value = $"<{param.ParameterInfo.Name.ToLower()}>";
                    ...
                    var method = $"{descriptor.ActionName} {string.Join(" ", parameters.ToArray())}";

                    listMethods.Add(new RpcCommandModel {Command = method.Trim(), Description = description});
                }
