...
namespace UnnamedCoin.Bitcoin.Features.RPC
{
    public class RPCMiddleware
    {
        ...
        readonly IRPCAuthorization authorization;
        readonly DataFolder dataFolder;

        readonly IHttpContextFactory httpContextFactory;

        readonly ILogger logger;
        readonly RequestDelegate next;

        public RPCMiddleware(RequestDelegate next, IRPCAuthorization authorization, ILoggerFactory loggerFactory,
            IHttpContextFactory httpContextFactory, DataFolder dataFolder)
        {
        ...
        public async Task InvokeAsync(HttpContext httpContext)
        {
            ...
            try
            {
                var request = await ReadRequestAsync(httpContext.Request).ConfigureAwait(false);

                this.logger.LogDebug("RPC request: {0}",
                    string.IsNullOrEmpty(request) ? request : JToken.Parse(request).ToString(Formatting.Indented));

                var token = string.IsNullOrEmpty(request) ? null : JToken.Parse(request);
                JToken response;
                ...
                using (var streamWriter = new StreamWriter(httpContext.Response.Body, Encoding.Default, 1024, true))
        ...
        async Task<string> ReadRequestAsync(HttpRequest request)
        {
            ...
            var requestBuffer = new byte[request.ContentLength.Value];

            while (true)
            {
                var remainingBytesCount = await request.Body.ReadAsync(requestBuffer, 0, requestBuffer.Length)
        ...
        async Task HandleRpcInvokeExceptionAsync(HttpContext httpContext, Exception ex)
        {
            if (ex is ArgumentException || ex is FormatException)
            {
                var response = CreateError(RPCErrorCode.RPC_MISC_ERROR, "Argument error: " + ex.Message);
                httpContext.Response.ContentType = ContentType;
                await httpContext.Response.WriteAsync(response.ToString(Formatting.Indented));
            }
            else if (ex is BlockNotFoundException)
            {
                var response = CreateError(RPCErrorCode.RPC_INVALID_REQUEST, "Argument error: " + ex.Message);
                httpContext.Response.ContentType = ContentType;
                await httpContext.Response.WriteAsync(response.ToString(Formatting.Indented));
            }
            else if (ex is ConfigurationException)
            {
                var response = CreateError(RPCErrorCode.RPC_INTERNAL_ERROR, ex.Message);
                httpContext.Response.ContentType = ContentType;
                await httpContext.Response.WriteAsync(response.ToString(Formatting.Indented));
            }
            else if (ex is RPCServerException)
            {
                ...
                var response = CreateError(rpcEx.ErrorCode, ex.Message);
                httpContext.Response.ContentType = ContentType;
                await httpContext.Response.WriteAsync(response.ToString(Formatting.Indented));
            }
            else if (httpContext.Response?.StatusCode == 404)
            {
                var response = CreateError(RPCErrorCode.RPC_METHOD_NOT_FOUND, "Method not found");
                httpContext.Response.ContentType = ContentType;
                await httpContext.Response.WriteAsync(response.ToString(Formatting.Indented));
            }
            else if (IsDependencyFailure(ex))
            {
                var response = CreateError(RPCErrorCode.RPC_METHOD_NOT_FOUND, ex.Message);
                httpContext.Response.ContentType = ContentType;
                await httpContext.Response.WriteAsync(response.ToString(Formatting.Indented));
            }
            else if (httpContext.Response?.StatusCode == 500 || ex != null)
            {
                var response = CreateError(RPCErrorCode.RPC_INTERNAL_ERROR, "Internal error");
                httpContext.Response.ContentType = ContentType;
        ...
        async Task<JArray> InvokeBatchAsync(HttpContext httpContext, JArray requests)
        {
            var responseArray = new JArray();
            foreach (JObject requestObj in requests)
            {
                var response = await InvokeSingleAsync(httpContext, requestObj);
        ...
        async Task<JObject> InvokeSingleAsync(HttpContext httpContext, JObject requestObj)
        {
        ...
        bool IsDependencyFailure(Exception ex)
        {
            var invalidOp = ex as InvalidOperationException;
            if (invalidOp == null)
                return false;
            return invalidOp.Source.Equals("Microsoft.Extensions.DependencyInjection.Abstractions",
                StringComparison.Ordinal);
        }

        bool Authorized(HttpContext httpContext)
        {
            ...
            if (!httpContext.Request.Headers.TryGetValue("Authorization", out var auth) || auth.Count != 1)
            {
                ...
                var cookie = File.ReadAllText(this.dataFolder.RpcCookieFile);
