...
namespace UnnamedCoin.Bitcoin.Features.RPC
{
    ...
    */
    public partial class RPCClient
    {
        ...
        {
            var data = await SendCommandAsync(RPCOperations.getbalance, "*", minConf, includeWatchOnly)
            ...
            FundRawTransactionOptions options = null)
        {
            ...
            if (options != null)
            {
                ...
                if (options.SubtractFeeFromOutputs != null)
                {
                    ...
                    foreach (var v in options.SubtractFeeFromOutputs) array.Add(new JValue(v));
        ...
        string ToHex(Transaction tx)
        {
            ...
            return Encoders.Hex.EncodeData(tx.ToBytes(ProtocolVersion.WITNESS_VERSION - 1));
        }
        ...
        {
            await SendCommandAsync(RPCOperations.importaddress, scriptPubKey.ToHex(), label, rescan)
                .ConfigureAwait(false);
        }
        ...
        {
            await SendCommandAsync(RPCOperations.importaddress, address.ToString(), label, rescan)
                .ConfigureAwait(false);
        }
        ...
        IEnumerable<RPCAccount> AsRPCAccount(RPCResponse response)
        {
        ...
        {
            var addr = from a in addresses select a.ToString();
            var response = await SendCommandAsync(RPCOperations.listunspent, minconf, maxconf, addr.ToArray())
                .ConfigureAwait(false);
            return response.Result.Select(i => new UnspentCoin((JObject) i, this.Network)).ToArray();
        }
        ...
        void LockUnspentCore(bool unlock, OutPoint[] outpoints)
        {
            LockUnspentCoreAsync(unlock, outpoints).GetAwaiter().GetResult();
        }

        async Task LockUnspentCoreAsync(bool unlock, OutPoint[] outpoints)
        {
        ...
        ///     walletpassphrase command while the wallet is already unlocked will set a new unlock time that overrides the old
        ///     one.
        /// </summary>
