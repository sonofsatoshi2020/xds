...
namespace UnnamedCoin.Bitcoin.Features.RPC
{
    ...
    public class RpcSettings
    {
        ...
        void LoadSettingsFromConfig(NodeSettings nodeSettings)
        {
            var config = nodeSettings.ConfigReader;

            this.Server = config.GetOrDefault("server", false, this.logger);
            this.RPCPort = config.GetOrDefault("rpcport", nodeSettings.Network.DefaultRPCPort, this.logger);
        ...
        void CheckConfigurationValidity(ILogger logger)
        {
            ...
            if (this.RpcPassword != null && this.RpcUser != null)
            {
                ...
                if (this.AllowIp.Count == 0)
                {
                    if (this.Bind.Count > 0)
                        logger.LogWarning(
                            "WARNING: RPC bind selection (-rpcbind) was ignored because allowed ip's (-rpcallowip) were not specified, refusing to allow everyone to connect");
        ...
        public static void PrintHelp(Network network)
        {
            ...
            builder.AppendLine(
                $"-rpcport=<0-65535>        Listen for JSON-RPC connections on <port>. Default: {network.DefaultRPCPort}");
            builder.AppendLine(
