...
namespace UnnamedCoin.Bitcoin.Features.Api
{
    ...
    {
        /// <summary>The default port used by the API.</summary>
        ...
        public ApiSettings(NodeSettings nodeSettings)
        {
            ...
            var apiHost = config.GetOrDefault("apiuri", defaultApiHost, this.logger);
            var apiUri = new Uri(apiHost);

            // Find out which port should be used for the API.
            var apiPort = config.GetOrDefault("apiport", nodeSettings.Network.DefaultAPIPort, this.logger);
            ...
            var keepAlive = config.GetOrDefault("keepalive", 0, this.logger);
            if (keepAlive > 0)
        ...
        public static void PrintHelp(Network network)
        {
            ...
            builder.AppendLine(
                $"-apiport=<0-65535>                Port of node's API interface. Defaults to {network.DefaultAPIPort}.");
            builder.AppendLine(
