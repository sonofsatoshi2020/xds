...
using UnnamedCoin.Bitcoin.Features.ColdStaking.Models;
...
namespace UnnamedCoin.Bitcoin.Features.ColdStaking
{
    ...
    ///         <item>
    ///             <description>
    ///                 Generating cold staking address via the <see cref="GetFirstUnusedColdStakingAddress" /> method. These
    ///                 addresses are used for generating the cold staking setup.
    ///             </description>
    ///         </item>
    ///         <item>
    ///             <description>
    ///                 Creating a build context for generating the cold staking setup via the
    ///                 <see
    ///                     cref="GetColdStakingSetupTransaction" />
    ///                 method.
    ///             </description>
    ///         </item>
    ///     </list>
    ...
    public class ColdStakingManager : WalletManager, IWalletManager
    {
        ...
        static Func<HdAccount, bool> coldStakingAccounts =
            a => a.Index >= Wallet.Wallet.SpecialPurposeAccountIndexesStart;
        ...
        /// <param name="asyncProvider">
        ///     Factory for creating and also possibly starting application defined tasks inside async
        ///     loop.
        /// </param>
        ...
        {
            var templates = base.GetValidStakingTemplates();
        ...
        {
            return base.GetTransactionBuilderExtensionsForStaking().Concat(new List<BuilderExtension>
                {new ColdStakingBuilderExtension(true)});
        }
        ...
        public override IEnumerable<UnspentOutputReference> GetSpendableTransactionsInWalletForStaking(
            string walletName, int confirmations = 0)
        {
            return GetUnspentTransactionsInWallet(walletName, confirmations,
                a => a.Index < Wallet.Wallet.SpecialPurposeAccountIndexesStart || a.Index == HotWalletAccountIndex);
        }
        ...
        /// </summary>
        /// <inheritdoc />
        public override Wallet.Wallet RecoverWallet(string password, string name, string mnemonic,
            DateTime creationTime, string passphrase, int? coinType = null)
        {
            var wallet = base.RecoverWallet(password, name, mnemonic, creationTime, passphrase, coinType);
        ...
        /// </summary>
        /// <inheritdoc />
        public override bool ProcessTransaction(Transaction transaction, int? blockHeight = null, Block block = null,
            bool isPropagated = true)
        {
            foreach (var utxo in transaction.Outputs)
            {
                ...
                if (script.IsUnspendable)
                {
                    ...
                    if (data.Length == 1)
                        if (data[0].Length == 40)
                        {
                            ...
                            {
                                var destination =
                                    ColdStakingScriptTemplate.Instance.GenerateScriptPubKey(hotPubKey, coldPubKey);
                                address.RedeemScript = destination;

                                // Find the type of script for the opreturn (P2SH or P2WSH)
                                foreach (var utxoInner in transaction.Outputs)
                                {
                                    if (utxoInner.ScriptPubKey == destination.Hash.ScriptPubKey)
                                    {
                                        if (!this.scriptToAddressLookup.TryGetValue(destination.Hash.ScriptPubKey,
                                            out _))
                                    ...
                                    {
                                        if (!this.scriptToAddressLookup.TryGetValue(destination.WitHash.ScriptPubKey,
                                            out _))
        ...
        /// <remarks>
        ///     <para>
        ///         In order to keep track of cold staking addresses and balances we are using <see cref="HdAccount" />'s
        ...
        internal HdAccount GetColdStakingAccount(Wallet.Wallet wallet, bool isColdWalletAccount)
        {
            ...
            var account = wallet.GetAccount(isColdWalletAccount ? ColdWalletAccountName : HotWalletAccountName);
            if (account == null)
            {
        ...
        internal HdAccount GetOrCreateColdStakingAccount(string walletName, bool isColdWalletAccount,
            string walletPassword)
        {
            var wallet = GetWalletByName(walletName);

            var account = GetColdStakingAccount(wallet, isColdWalletAccount);
            if (account != null)
            {
            ...
            this.logger.LogDebug("The {0} wallet account for '{1}' does not exist and will now be created.",
                isColdWalletAccount ? "cold" : "hot", wallet.Name);
            ...
            account = wallet.AddNewAccount(walletPassword, this.dateTimeProvider.GetTimeOffset(), accountIndex,
                accountName);

            // Maintain at least one unused address at all times. This will ensure that wallet recovery will also work.
            var newAddresses = account.CreateAddresses(wallet.Network, 1);
        ...
        internal HdAddress GetFirstUnusedColdStakingAddress(string walletName, bool isColdWalletAddress)
        {
            ...
            var wallet = GetWalletByName(walletName);
            var account = GetColdStakingAccount(wallet, isColdWalletAddress);
            if (account == null)
            {
                this.logger.LogTrace("(-)[ACCOUNT_DOES_NOT_EXIST]:null");
                return null;
        ...
        public IEnumerable<UnspentOutputReference> GetSpendableTransactionsInColdWallet(string walletName,
            bool isColdWalletAccount, int confirmations = 0)
        {
            Guard.NotEmpty(walletName, nameof(walletName));
        ...
        {
            if (ColdStakingScriptTemplate.Instance.ExtractScriptPubKeyParameters(script, out var hotPubKeyHash,
                out var coldPubKeyHash))
            {
            ...
            if (script.IsScriptType(ScriptType.P2SH) || script.IsScriptType(ScriptType.P2WSH))
                if (this.scriptToAddressLookup.TryGetValue(script, out var address))
                    if (ColdStakingScriptTemplate.Instance.ExtractScriptPubKeyParameters(address.RedeemScript,
                        out hotPubKeyHash, out coldPubKeyHash))
                    {
                        base.TransactionFoundInternal(hotPubKeyHash.ScriptPubKey,
                            a => a.Index == HotWalletAccountIndex);
                        base.TransactionFoundInternal(coldPubKeyHash.ScriptPubKey,
                            a => a.Index == ColdWalletAccountIndex);
        ...
        protected override void AddAddressToIndex(HdAddress address)
        {
            ...
            if (address.RedeemScript != null)
            {
                ...
                if (!this.scriptToAddressLookup.TryGetValue(address.RedeemScript.Hash.ScriptPubKey, out _))
                    this.scriptToAddressLookup[address.RedeemScript.Hash.ScriptPubKey] = address;

                if (!this.scriptToAddressLookup.TryGetValue(address.RedeemScript.WitHash.ScriptPubKey, out _))
