...
namespace UnnamedCoin.Bitcoin.Features.ColdStaking
{
    ...
    public class ColdStakingScriptTemplate : ScriptTemplate
    {
        ...
        /// </summary>
        /// <remarks>
        ///     Two keys control the balance associated with the script.
        ...
        ///     opcode within the script flow related to hot wallet key usage. It sets the
        ///     <see cref="PosTransaction.IsColdCoinStake" />
        ///     flag if the transaction spending an output, which contains this instruction, is a coinstake
        ///     transaction. If this flag is set then further rules are enforced by
        ///     <see cref="Stratis.Bitcoin.Features.Consensus.Rules.CommonRules.PosColdStakingRule" />.
        /// </remarks>
        ...
        public bool ExtractScriptPubKeyParameters(Script scriptPubKey, out KeyId hotPubKeyHash,
            out KeyId coldPubKeyHash)
        {
            if (!FastCheckScriptPubKey(scriptPubKey, out var needMoreCheck))
            {
                hotPubKeyHash = null;
                coldPubKeyHash = null;
