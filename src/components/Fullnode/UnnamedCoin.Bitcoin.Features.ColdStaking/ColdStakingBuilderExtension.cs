...
namespace UnnamedCoin.Bitcoin.Features.ColdStaking
{
    ...
    public class ColdStakingBuilderExtension : BuilderExtension
    {
        ...
        public override Script GenerateScriptSig(Network network, Script scriptPubKey, IKeyRepository keyRepo,
            ISigner signer)
        {
            ColdStakingScriptTemplate.Instance.ExtractScriptPubKeyParameters(scriptPubKey, out var hotKey,
                out var coldKey);

            // The scriptPubKey will be different depending on whether we are spending or cold staking.
            var key = keyRepo.FindKey((this.usesColdPubKey ? coldKey : hotKey).ScriptPubKey);
