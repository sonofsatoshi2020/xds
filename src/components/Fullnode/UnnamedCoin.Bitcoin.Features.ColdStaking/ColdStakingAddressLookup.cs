...
namespace UnnamedCoin.Bitcoin.Features.ColdStaking
{
    ...
    public class ColdStakingAddressLookup : ScriptToAddressLookup
    {
        ...
        public ColdStakingAddressLookup(Network network)
        {
        ...
        public override HdAddress this[Script script]
        {
            get
            {
                ...
                if (TryGetValue(script, out var address))
                    return address;

                // This method should throw an exception if the value is not found.
                throw new Exception(
                    $"Could not resolve address from script '{script}' using '{nameof(ScriptToAddressLookup)}' collection.");
            }
            ...
            set
            {
                ...
                if (ColdStakingScriptTemplate.Instance.ExtractScriptPubKeyParameters(script, out var hotPubKey,
                    out var coldPubKey))
                {
                    var destination = value.ScriptPubKey.GetDestination(this.network);
