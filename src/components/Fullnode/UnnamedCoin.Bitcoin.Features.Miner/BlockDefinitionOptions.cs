...
namespace UnnamedCoin.Bitcoin.Features.Miner
{
    ...
    public sealed class BlockDefinitionOptions
    {
        ...
        const uint MinBlockSize = 1000;

        public BlockDefinitionOptions(uint blockMaxWeight, uint blockMaxSize)
        {
            this.BlockMaxWeight = blockMaxWeight;
            this.BlockMaxSize = blockMaxSize;
            this.BlockMinFeeRate =
                new FeeRate(PowMining.DefaultBlockMinTxFee); // TODO: Where should this be set, really?
        }
        ...
        {
            var minAllowedBlockWeight = MinBlockSize * (uint) network.Consensus.Options.WitnessScaleFactor;
            this.BlockMaxWeight = Math.Max(minAllowedBlockWeight,
                Math.Min(network.Consensus.Options.MaxBlockWeight, this.BlockMaxWeight));
            this.BlockMaxSize = Math.Max(MinBlockSize,
                Math.Min(network.Consensus.Options.MaxBlockSerializedSize, this.BlockMaxSize));
