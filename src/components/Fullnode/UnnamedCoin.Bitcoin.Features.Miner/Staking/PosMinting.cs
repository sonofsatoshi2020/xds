...
namespace UnnamedCoin.Bitcoin.Features.Miner.Staking
{
    ...
    ///             <item>
    ///                 Split these UTXOs in subsets and create tasks processing each subset to allow for parallel
    ///                 processing.
    ///             </item>
    ///             <item>
    ///                 Each of the tasks mentioned above will try to find a solution for proof of stake target. This is done
    ///                 by creating a coinstake
    ///                 transaction with each of the available UTXOs combined with all valid unix timestamps that were not
    ///                 checked.
    ///                 Those timestamps are within a time interval from now to now - searchInterval seconds. Only timestamps
    ///                 that are divisible by
    ///                 <c><see cref="BlockHeaderPosContextualRule.StakeTimestampMask" /> + 1</c> are valid candidates (this is
    ///                 done to decrease granularity of timestamps).
    ///                 Search interval is a length of an unexplored block time space in seconds.
    ///                 Task calculates the kernel's hash (kernel is the first input in the coinstake transaction) using the
    ///                 next formula:
    ...
    ///                 We compare kernel's hash against the staking target, if it's greater then we met the criteria and
    ///                 kernel is found.
    ///                 So the more coins we stake the higher the staking target and so the higher the chance to meet the
    ///                 criteria.
    ///             </item>
    ...
    ///     </para>
    ///     <para>
    ///         Coinstake transaction invalidates previous inputs and spends the inputs to new outputs with the additional
    ///         stake reward.
    ///     </para>
    ///     <para>
    ...
    public class PosMinting : IPosMinting
    {
        ...
        ///     multiplied by this value. See <see cref="GetSplitStake(int, ChainedHeader)" />.
        /// </summary>
        ...
        /// <remarks>
        ///     To achieve a good level of parallelism, this should be low enough so that CPU threads are used,
        ///     but high enough to compensate for tasks' overhead.
        /// </remarks>
        const int UtxoStakeDescriptionsPerCoinstakeWorker = 25;
        ...
        /// <summary>
        ///     <c>true</c> if coinstake transaction splits the coin and generates extra UTXO
        ///     to prevent halting chain; <c>false</c> to disable coinstake splitting.
        /// </summary>
        ...
        readonly int minerSleep;

        /// <summary>
        ///     Minimum value of a split utxo we are aiming for (after splitting it into <see cref="SplitFactor" /> equal
        ///     parts).
        /// </summary>
        readonly ulong MinimumSplitCoinValue;
        ...
        readonly int systemTimeOutOfSyncSleep;
        ...
        ///     </para>
        /// </summary>
        uint256 lastCoinStakeSearchPrevBlockHash;
        ...
        /// <remarks>
        ///     This object does not need a synchronized access because there is no execution logic
        ///     that depends on the reported information.
        /// </remarks>
        readonly GetStakingInfoModel rpcGetStakingInfoModel;

        /// <summary>
        ///     A cancellation token source that can cancel the staking processes and is linked to the
        ...
        public void Stake(WalletSecret walletSecret)
        {
            ...
            if (Interlocked.CompareExchange(ref this.currentState, (int) CurrentState.StakingRequested,
                    (int) CurrentState.Idle) != (int) CurrentState.Idle)
            {
            ...
            this.stakingLoop = this.asyncProvider.CreateAndRunAsyncLoop("PosMining.Stake", async token =>
                ...
                TimeSpan.FromMilliseconds(this.minerSleep),
                TimeSpans.Second);

            Interlocked.CompareExchange(ref this.currentState, (int) CurrentState.StakingInProgress,
                (int) CurrentState.StakingRequested);
        }

        /// <inheritdoc />
        public void StopStake()
        {
            if (Interlocked.CompareExchange(ref this.currentState, (int) CurrentState.StopStakingRequested,
                    (int) CurrentState.StakingInProgress) != (int) CurrentState.StakingInProgress)
            {
            ...
            Interlocked.CompareExchange(ref this.currentState, (int) CurrentState.Idle,
                (int) CurrentState.StopStakingRequested);
        }
        ...
        public async Task GenerateBlocksAsync(WalletSecret walletSecret, CancellationToken cancellationToken)
        {
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                if (this.timeSyncBehaviorState.IsSystemTimeOutOfSync)
                {
                    ...
                    await Task.Delay(TimeSpan.FromMilliseconds(this.systemTimeOutOfSyncSleep), cancellationToken)
                        .ConfigureAwait(false);
                    continue;
                }
                ...
                if (this.consensusManager.Tip.HashBlock != this.walletManager.WalletTipHash)
                {
                    ...
                    await Task.Delay(TimeSpan.FromMilliseconds(this.minerSleep), cancellationToken)
                        .ConfigureAwait(false);
                    continue;
                }
                ...
                var chainTip = this.consensusManager.Tip;
                ...
                if (this.lastCoinStakeSearchPrevBlockHash != chainTip.HashBlock)
                {
                    ...
                    this.logger.LogDebug("New block '{0}' detected, setting last search time to its timestamp {1}.",
                        chainTip, chainTip.Header.Time);

                    // Reset the template as the chain advanced.
                    blockTemplate = null;
                }

                var coinstakeTimestamp = (uint) this.dateTimeProvider.GetAdjustedTimeAsUnixTimestamp() &
                                         ~PosConsensusOptions.StakeTimestampMask;
                if (coinstakeTimestamp <= this.lastCoinStakeSearchTime)
                {
                    this.logger.LogDebug("Current coinstake time {0} is not greater than last search timestamp {1}.",
                        coinstakeTimestamp, this.lastCoinStakeSearchTime);
                    this.logger.LogTrace("(-)[NOTHING_TO_DO]");
                    return;
                }

                var utxoStakeDescriptions = GetUtxoStakeDescriptions(walletSecret, cancellationToken);
                ...
                if (await StakeAndSignBlockAsync(utxoStakeDescriptions, blockTemplate, chainTip, blockTemplate.TotalFee,
                    coinstakeTimestamp).ConfigureAwait(false))
                {
                ...
                {
                    this.logger.LogDebug("{0} failed to create POS block, waiting {1} ms for next round.",
                        nameof(StakeAndSignBlockAsync), this.minerSleep);
                    await Task.Delay(TimeSpan.FromMilliseconds(this.minerSleep), cancellationToken)
                        .ConfigureAwait(false);
                }
            }
        ...
        /// <inheritdoc />
        public async Task<bool> CreateCoinstakeAsync(List<UtxoStakeDescription> utxoStakeDescriptions,
            BlockTemplate blockTemplate, ChainedHeader chainTip, long searchInterval, long fees,
            CoinstakeContext coinstakeContext)
        {
            ...
            var (balance, immature) = await GetMatureBalanceAsync(utxoStakeDescriptions).ConfigureAwait(false);
            ...
            var stakingUtxoDescriptions = await GetUtxoStakeDescriptionsSuitableForStakingAsync(utxoStakeDescriptions,
                chainTip, coinstakeContext.StakeTimeSlot, balance - this.targetReserveBalance).ConfigureAwait(false);
            ...
            var ourWeight = stakingUtxoDescriptions.Sum(s => s.TxOut.Value);
            var expectedTime = StakeValidator.TargetSpacingSeconds * this.networkWeight / ourWeight;
            var ourPercent = this.networkWeight != 0 ? 100.0m * ourWeight / this.networkWeight : 0;
            ...
            this.logger.LogDebug(
                ...
                stakingUtxoDescriptions.Count, minimalAllowedTime, coinstakeContext.StakeTimeSlot);
            ...
            var coinIndex = 0;
            var workerCount = (stakingUtxoDescriptions.Count + UtxoStakeDescriptionsPerCoinstakeWorker - 1) /
            ...
            for (var workerIndex = 0; workerIndex < workerCount; workerIndex++)
            {
                ...
                {
                    Index = workerIndex,
                    Logger = this.loggerFactory.CreateLogger(GetType().FullName, $"[Worker #{workerIndex}] "),
                    utxoStakeDescriptions = new List<UtxoStakeDescription>(),
                    CoinstakeContext = coinstakeContext,
                    Result = workersResult
                };

                var stakingUtxoCount = Math.Min(stakingUtxoDescriptions.Count - coinIndex,
                    UtxoStakeDescriptionsPerCoinstakeWorker);
            ...
            await Task.Run(() => Parallel.ForEach(workerContexts,
                cwc => { CoinstakeWorker(cwc, chainTip, blockTemplate.Block, minimalAllowedTime, searchInterval); }));
            ...
            for (var i = blockTemplate.Block.Transactions.Count - 1; i >= 1; i--)
                ...
                if (blockTemplate.Block.Transactions[i] is IPosTransactionWithTime posTrx)
                {
                    ...
                    this.logger.LogDebug(
                        "Removing transaction with timestamp {0} as it is greater than coinstake transaction timestamp {1}. New fee amount {2}.",
                        posTrx.Time, coinstakeContext.StakeTimeSlot, fees);
            ...
            var coinstakeInput = workersResult.KernelCoin;

            // Total amount of input values in coinstake transaction.
            long coinstakeOutputValue = coinstakeInput.TxOut.Value + reward;

            var eventuallyStakableUtxosCount = utxoStakeDescriptions.Count;
            var coinstakeTx = PrepareCoinStakeTransactions(chainTip.Height, coinstakeContext, coinstakeOutputValue,
                eventuallyStakableUtxosCount, ourWeight);

            if (coinstakeTx is IPosTransactionWithTime posTrxn) posTrxn.Time = coinstakeContext.StakeTimeSlot;
            ...
            {
                this.logger.LogDebug("Coinstake size {0} bytes exceeded limit {1} bytes.", serializedSize,
                    MaxBlockSizeGen / 5);
            ...
            List<UtxoStakeDescription> utxoStakeDescriptions)
        {
            ...
            foreach (var utxoStakeDescription in utxoStakeDescriptions)
            {
                // Must wait until coinbase is safely deep enough in the chain before valuing it.
                if ((utxoStakeDescription.UtxoSet.IsCoinbase || utxoStakeDescription.UtxoSet.IsCoinstake) &&
                    await GetBlocksCountToMaturityAsync(utxoStakeDescription).ConfigureAwait(false) > 0)
                {
        ...
        public double GetDifficulty(ChainedHeader block)
        {
            ...
            var shift = (block.Header.Bits >> 24) & 0xFF;
            var diff = 0x0000FFFF / (double) (block.Header.Bits & 0x00FFFFFF);
        ...
        {
            var interval = 72;
            var stakeKernelsAvg = 0.0;
            var stakesHandled = 0;
            long stakesTime = 0;

            // Use consensus loop's tip rather than concurrent chain's tip
            // because consensus loop's tip is guaranteed to have block stake in the database.
            var block = this.consensusManager.Tip;
            ChainedHeader prevStakeBlock = null;

            var res = 0.0;
            while (block != null && stakesHandled < interval)
            {
                var blockStake = this.stakeChain.Get(block.HashBlock);
        ...
        internal List<UtxoStakeDescription> GetUtxoStakeDescriptions(WalletSecret walletSecret,
            CancellationToken cancellationToken)
        {
            var utxoStakeDescriptions = new List<UtxoStakeDescription>();

            var stakableUtxos = this.walletManager
                ...
                .Where(utxo =>
                    utxo.Transaction.Amount >= this.MinimumStakingCoinValue) // exclude dust from stake process
                .ToList();

            var fetchedCoinSet =
                this.coinView.FetchCoins(stakableUtxos.Select(t => t.Transaction.Id).Distinct().ToArray(),
                    cancellationToken);
            var utxoByTransaction = fetchedCoinSet.UnspentOutputs.Where(utxo => utxo != null)
                .ToDictionary(utxo => utxo.TransactionId, utxo => utxo);
            fetchedCoinSet = null; // allow GC to collect as soon as possible.

            for (var i = 0; i < stakableUtxos.Count; i++)
            {
                var stakableUtxo = stakableUtxos[i];
                ...
                var coinSet = utxoByTransaction.TryGet(stakableUtxo.Transaction.Id);
                if (coinSet == null || stakableUtxo.Transaction.Index >= coinSet.Outputs.Length)
                    continue;

                var utxo = coinSet.Outputs[stakableUtxo.Transaction.Index];
                if (utxo == null || utxo.Value < this.MinimumStakingCoinValue)
                    continue;

                var hashBlock = this.chainIndexer.GetHeader((int) coinSet.Height)?.HashBlock;
                if (hashBlock == null)
                    continue;
                ...
                this.logger.LogDebug("UTXO '{0}' with value {1} might be available for staking.",
                    utxoStakeDescription.OutPoint, utxo.Value);
            }

            this.logger.LogDebug("Wallet total staking balance is {0}.",
                new Money(utxoStakeDescriptions.Sum(d => d.TxOut.Value)));
        ...
        async Task CheckStakeAsync(Block block, ChainedHeader chainTip)
        {
            ...
            var prevBlockStake = this.stakeChain.Get(chainTip.HashBlock);
            ...
            // Validate the block.
            var chainedHeader = await this.consensusManager.BlockMinedAsync(block).ConfigureAwait(false);
            ...
            this.logger.LogInformation("==================================================================");
            this.logger.LogInformation("Found new POS block hash '{0}' at height {1}.", chainedHeader.HashBlock,
                chainedHeader.Height);
            this.logger.LogInformation("==================================================================");
        }
        ...
        /// <param name="coinstakeTimestamp">
        ///     Maximal timestamp of the coinstake transaction. The actual timestamp can be lower, but
        ///     not higher.
        /// </param>
        /// <returns><c>true</c> if the function succeeds, <c>false</c> otherwise.</returns>
        async Task<bool> StakeAndSignBlockAsync(List<UtxoStakeDescription> utxoStakeDescriptions,
            BlockTemplate blockTemplate, ChainedHeader chainTip, long fees, uint coinstakeTimestamp)
        {
            ...
            var searchInterval = searchTime - this.lastCoinStakeSearchTime;
            this.rpcGetStakingInfoModel.SearchInterval = (int) searchInterval;

            this.lastCoinStakeSearchTime = searchTime;
            this.logger.LogDebug("Search interval set to {0}, last coinstake search timestamp set to {1}.",
                searchInterval, this.lastCoinStakeSearchTime);

            if (await CreateCoinstakeAsync(utxoStakeDescriptions, blockTemplate, chainTip, searchInterval, fees,
                coinstakeContext).ConfigureAwait(false))
            {
                var minTimestamp = chainTip.Header.Time + 1;
                if (coinstakeContext.StakeTimeSlot >= minTimestamp)
                {
                    ...
                    var signature = coinstakeContext.Key.Sign(block.GetHash());
                ...
                this.logger.LogDebug(
                    "Coinstake transaction created with too early timestamp {0}, minimal timestamp is {1}.",
                    coinstakeContext.StakeTimeSlot, minTimestamp);
            }
            else
            {
                this.logger.LogDebug("Unable to create coinstake transaction.");
            }

            return false;
        }

        internal Transaction PrepareCoinStakeTransactions(int currentChainHeight, CoinstakeContext coinstakeContext,
            long coinstakeOutputValue, int utxosCount, long amountStaked)
        {
            // Split stake into SplitFactor utxos if above threshold.
            var shouldSplitStake = ShouldSplitStake(utxosCount, amountStaked, coinstakeOutputValue, currentChainHeight);

            var lastOutputIndex = coinstakeContext.CoinstakeTx.Outputs.Count - 1;

            if (!shouldSplitStake)
            {
                coinstakeContext.CoinstakeTx.Outputs[lastOutputIndex].Value = coinstakeOutputValue;
                this.logger.LogDebug("Coinstake output value is {0}.",
            ...
            var splitValue = coinstakeOutputValue / SplitFactor;
            var remainder = coinstakeOutputValue - (SplitFactor - 1) * splitValue;
            coinstakeContext.CoinstakeTx.Outputs[lastOutputIndex].Value = remainder;

            for (var i = 0; i < SplitFactor - 1; i++)
            {
        ...
        /// </summary>
        /// <param name="context">
        ///     Context information with worker task description. Results of the worker's attempt are also stored
        ///     in this context.
        /// </param>
        ...
        /// </param>
        void CoinstakeWorker(CoinstakeWorkerContext context, ChainedHeader chainTip, Block block,
            long minimalAllowedTime, long searchInterval)
        {
            ...
            var orderedUtxoStakeDescriptions =
                context.utxoStakeDescriptions.OrderByDescending(o => o.TxOut.Value).ToList();

            var stopWork = false;
            foreach (var utxoStakeInfo in orderedUtxoStakeDescriptions)
            {
                context.Logger.LogDebug("Trying UTXO from address '{0}', output amount {1}.",
                    utxoStakeInfo.Address.Address, utxoStakeInfo.TxOut.Value);

                // Script of the first coinstake input.
                var scriptPubKeyKernel = utxoStakeInfo.TxOut.ScriptPubKey;
                if (!this.ValidStakingTemplates.Any(a => a.Value.CheckScriptPubKey(scriptPubKeyKernel)))
                {
                    context.Logger.LogDebug("Kernel type must be {0}, kernel rejected.",
                        string.Join(" or ", this.ValidStakingTemplates.Keys));
                    continue;
                }
                ...
                for (uint n = 0; n < searchInterval; n++)
                {
                    ...
                    {
                        context.Logger.LogDebug("Different worker #{0} already found kernel, stopping work.",
                            context.Result.KernelFoundIndex);
                        stopWork = true;
                        break;
                    }
                    ...
                    var txTime = context.CoinstakeContext.StakeTimeSlot - n;
                    ...
                    try
                    {
                        ...
                        var validKernel = this.stakeValidator.CheckKernel(contextInformation, chainTip,
                            block.Header.Bits, txTime, prevoutStake);

                        if (!validKernel) continue;

                        if (context.Result.SetKernelFoundIndex(context.Index))
                        {
                            context.Logger.LogDebug("Kernel found with solution hash '{0}'.",
                                contextInformation.HashProofOfStake);

                            var wallet = this.walletManager.GetWalletByName(utxoStakeInfo.Secret.WalletName);
                            context.CoinstakeContext.Key = wallet
                                .GetExtendedPrivateKeyForAddress(utxoStakeInfo.Secret.WalletPassword,
                                    utxoStakeInfo.Address).PrivateKey;
                            ...
                            var scriptType = this.ValidStakingTemplates.Single(t =>
                                t.Value.CheckScriptPubKey(utxoStakeInfo.TxOut.ScriptPubKey)).Key;

                            // Default behavior.
                            if (scriptType == "P2PK" || scriptType == "P2PKH")
                            {
                                scriptPubKeyOut =
                                    PayToPubkeyTemplate.Instance.GenerateScriptPubKey(context.CoinstakeContext.Key
                                        .PubKey);
                            }
                            else
                                // Support for otherwise unsupported script types.
                            {
                                context.CoinstakeContext.CoinstakeTx.Outputs.Add(new TxOut(Money.Zero,
                                    new Script(OpcodeType.OP_RETURN,
                                        Op.GetPushOp(utxoStakeInfo.Key.PubKey.Compress().ToBytes()))));
                            ...
                            context.Logger.LogDebug("Kernel accepted, coinstake input is '{0}', stopping work.",
                                prevoutStake);
                        }
                        else
                        {
                            context.Logger.LogDebug(
                                "Kernel found, but worker #{0} announced its kernel earlier, stopping work.",
                                context.Result.KernelFoundIndex);
                        }
        ...
        /// <returns><c>true</c> if the function succeeds, <c>false</c> otherwise.</returns>
        bool SignTransactionInput(UtxoStakeDescription input, Transaction transaction)
        {
            var res = false;
            try
            {
                    ...
                    PayToWitScriptHashTemplate.Instance.CheckScriptPubKey(input.TxOut.ScriptPubKey))
                {
                    ...
                    var scriptCoin = ScriptCoin.Create(this.network, input.OutPoint, input.TxOut,
                        input.Address.RedeemScript);
                ...
                foreach (var extension in this.walletManager.GetTransactionBuilderExtensionsForStaking())
        ...
        internal async Task<List<UtxoStakeDescription>> GetUtxoStakeDescriptionsSuitableForStakingAsync(
            List<UtxoStakeDescription> utxoStakeDescriptions, ChainedHeader chainTip, uint spendTime, long targetValue)
        {
            ...
            long requiredDepth =
                ((PosConsensusOptions) this.network.Consensus.Options).GetStakeMinConfirmations(chainTip.Height + 1,
                    this.network) - 1;
            foreach (var utxoStakeDescription in utxoStakeDescriptions.OrderByDescending(x => x.TxOut.Value))
            {
                ...
                var depth = await GetDepthInMainChainAsync(utxoStakeDescription).ConfigureAwait(false);
                this.logger.LogDebug("Checking if UTXO '{0}' value {1} can be added, its depth is {2}.",
                    utxoStakeDescription.OutPoint, utxoStakeDescription.TxOut.Value, depth);
                ...
                {
                    this.logger.LogDebug("UTXO '{0}' depth {1} is lower than required minimum depth {2}.",
                        utxoStakeDescription.OutPoint, depth, requiredDepth);
                    continue;
                }
                ...
                {
                    this.logger.LogDebug(
                        "UTXO '{0}' can't be added because its time {1} is greater than coinstake time {2}.",
                        utxoStakeDescription.OutPoint, utxoStakeDescription.UtxoSet.Time, spendTime);
                    continue;
                }
                ...
                var toMaturity = await GetBlocksCountToMaturityAsync(utxoStakeDescription).ConfigureAwait(false);
                if (toMaturity > 0)
                {
                    this.logger.LogDebug(
                        "UTXO '{0}' can't be added because it is not mature, {1} blocks to maturity left.",
                        utxoStakeDescription.OutPoint, toMaturity);
                    continue;
                }
        ...
        /// <remarks>
        ///     Do NOT use this for general-purpose maturity calculations outside of <see cref="PosMinting" /> as it will give
        ///     off-by-one errors.
        ///     This method is making the assumption that we are adding a new block to the chain, and thus reduces the maturity
        ///     threshold by 1.
        /// </remarks>
        async Task<int> GetBlocksCountToMaturityAsync(UtxoStakeDescription utxoStakeDescription)
        {
            ...
            var minConf = (int) this.network.Consensus.CoinbaseMaturity;
            ...
            return Math.Max(0,
                minConf - 1 - await GetDepthInMainChainAsync(utxoStakeDescription).ConfigureAwait(false));
        }
        ...
        /// </returns>
        async Task<int> GetDepthInMainChainAsync(UtxoStakeDescription utxoStakeDescription)
        {
            var chainedBlock = this.chainIndexer.GetHeader(utxoStakeDescription.HashBlock);

            if (chainedBlock == null)
                return await this.mempoolLock
                    .ReadAsync(() => this.mempool.Exists(utxoStakeDescription.UtxoSet.TransactionId) ? 0 : -1)
                    .ConfigureAwait(false);
        ...
        /// </summary>
        /// <param name="stakedUtxosCount">
        ///     Number of UTXOs that the wallet could stake, if coin base maturity and stake minimum
        ///     confirmations were not taken into account.
        /// </param>
        ...
        /// <remarks>
        ///     We do not split a coin if the value of new coins after the split would be less than
        ///     <see cref="MinimumSplitCoinValue" />. Because we split the coin to multiple outputs defined by split factor, we
        ///     only consider coins with value at least <see cref="MinimumSplitCoinValue" /> * <see cref="SplitFactor" />.
        ///     <para>
        ///         If the above-mentioned criteria is satisfied, then we split the coin if its value is greater than an expected
        ///         average value of coins that we would have if we have perfect distribution of the value among all our coins
        ///         while having a specific number of coins that we aim for. The optimal number of coins we are looking for is
        ///         calculated based on consensus settings of coin maturity and minimum required coin age for staking.
        ///     </para>
        /// </remarks>
        ...
        internal bool ShouldSplitStake(int stakedUtxosCount, long amountStaked, long coinValue, int chainHeight)
        {
            ...
            long coinAgeLimit =
                ((PosConsensusOptions) this.network.Consensus.Options).GetStakeMinConfirmations(chainHeight + 1,
                    this.network);
            var coinMaturityLimit = this.network.Consensus.CoinbaseMaturity;
            var requiredCoinAgeForStaking = Math.Max(coinMaturityLimit, coinAgeLimit);
            this.logger.LogDebug("Required coin age for staking is {0}.", requiredCoinAgeForStaking);

            var targetCoinDistributionSize = (requiredCoinAgeForStaking + 1) * CoinstakeSplitLimitMultiplier;

            var coinAboveMinValue = coinValue > SplitFactor * (long) this.MinimumSplitCoinValue;
            var coinAboveTargetAverage = coinValue > amountStaked / targetCoinDistributionSize + Money.COIN;

            var shouldSplitCoin = coinAboveMinValue && coinAboveTargetAverage;
