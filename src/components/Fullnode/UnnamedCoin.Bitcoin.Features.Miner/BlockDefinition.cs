...
using UnnamedCoin.Bitcoin.Features.Miner.Comparers;
...
{
    /// <summary>
    ///     A high level class that will allow the ability to override or inject functionality based on what type of block
    ///     creation logic is used.
    /// </summary>
    public abstract class BlockDefinition
    {
        ...
        ///     its ancestors.
        /// </summary>
        protected long LastBlockTx;

        protected long LastBlockWeight;
        ...
        {
            var flags = this.nodeDeployments.GetFlags(chainedHeader);
        ...
        protected void OnBuild(ChainedHeader chainTip, Script scriptPubKey)
        {
            ...
            AddTransactions(out var nPackagesSelected, out var nDescendantsUpdated);
            ...
            this.BlockTemplate.FeeDetails = this.inBlock.Select(i => new {i.TransactionHash, i.Fee})
                .ToDictionary(d => d.TransactionHash, d => d.Fee);

            if (this.IncludeWitness) AddOrUpdateCoinbaseCommitmentToBlock(this.block);

            var nSerializeSize = this.block.GetSerializedSize();
            this.logger.LogDebug(
                "Serialized size is {0} bytes, block weight is {1}, number of txs is {2}, tx fees are {3}, number of sigops is {4}.",
                nSerializeSize, this.block.GetBlockWeight(this.Network.Consensus), this.BlockTx, this.fees,
        ...
        /// <summary>
        ///     Adds the coinbase commitment to the coinbase transaction according to
        ...
        ///     <para>
        ///         The block's <see cref="BlockSigOpsCost" /> and <see cref="BlockWeight" /> values are adjusted.
        ///     </para>
        /// </summary>
        ...
        protected virtual void AddTransactions(out int nPackagesSelected, out int nDescendantsUpdated)
        {
            ...
            var ancestorScoreList = this.MempoolLock.ReadAsync(() => this.Mempool.MapTx.AncestorScore)
            ...
            while (ancestorScoreList.Any() || mapModifiedTx.Any())
            {
                ...
                if (mi != null)
                    ...
                    if (mapModifiedTx.ContainsKey(mi.TransactionHash) || this.inBlock.Contains(mi) ||
                        failedTx.Contains(mi))
                    {
                ...
                var packageSigOpsCost = iter.SigOpCostWithAncestors;
                if (fUsingModified)
                {
                ...
                if (!TestPackage(iter, packageSize, packageSigOpsCost))
                {
                    ...
                    if (nConsecutiveFailed > MaxConsecutiveAddTransactionFailures &&
                ...
                this.MempoolLock
                    .ReadAsync(() => this.Mempool.CalculateMemPoolAncestors(iter, ancestors, nNoLimit, nNoLimit,
                        nNoLimit, nNoLimit, out dummy, false)).ConfigureAwait(false).GetAwaiter().GetResult();
        ...
        /// </summary>
        void OnlyUnconfirmed(TxMempool.SetEntries testSet)
        {
        ...
        protected virtual bool TestPackage(TxMempoolEntry entry, long packageSize, long packageSigOpsCost)
        {
            ...
            if (this.BlockWeight + this.Network.Consensus.Options.WitnessScaleFactor * packageSize >=
                this.Options.BlockMaxWeight)
            {
        ...
        ///     </para>
        /// </summary>
        bool TestPackageTransactions(TxMempool.SetEntries package)
        {
