...
namespace UnnamedCoin.Bitcoin.Features.Miner
{
    ...
    public class MiningFeature : FullNodeFeature
    {
        ...
        public void StartStaking(string walletName, string walletPassword)
        {
            ...
            {
                var errorMessage =
                    "Staking cannot start, your system time does not match that of other nodes on the network." + Environment
                                                                                                                    .NewLine
        ...
        public override Task InitializeAsync()
        {
            if ((this.minerSettings.Mine || this.minerSettings.Stake) && this.connectionManagerSettings.IsGateway)
                throw new ConfigurationException(
                    "The node cannot be configured as a gateway and mine or stake at the same time.");

            if (this.minerSettings.Mine)
            {
                var mineToAddress = this.minerSettings.MineAddress;
