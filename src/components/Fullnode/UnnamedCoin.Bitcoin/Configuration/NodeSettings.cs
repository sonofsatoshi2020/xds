...
{
    static class NormalizeDirectorySeparatorExt
    {
        /// <summary>
    ...
    public class NodeSettings : IDisposable
    {
        ...
        public NodeSettings(Network network = null, ProtocolVersion protocolVersion = SupportedProtocolVersion,
            string agent = "anon", string[] args = null, NetworksSelector networksSelector = null)
        {
            ...
            this.ConfigurationFile = this.ConfigReader.GetOrDefault<string>("conf", null, this.Logger)
                ?.NormalizeDirectorySeparator();
            this.DataDir = this.ConfigReader.GetOrDefault<string>("datadir", null, this.Logger)
                ?.NormalizeDirectorySeparator();
            this.DataDirRoot = this.ConfigReader.GetOrDefault("datadirroot", "FullNodeRoot", this.Logger);
            ...
            if (this.Network == null)
            {
                ...
                var testNet = this.ConfigReader.GetOrDefault("testnet", false, this.Logger);
                var regTest = this.ConfigReader.GetOrDefault("regtest", false, this.Logger);

                if (testNet && regTest)
                    throw new ConfigurationException("Invalid combination of regtest and testnet.");

                this.Network = testNet ? networksSelector.Testnet() :
                    regTest ? networksSelector.Regtest() : networksSelector.Mainnet();
            ...
            if (this.DataDir == null)
            {
                ...
                this.DataDir = CreateDefaultDataDirectories(Path.Combine(this.DataDirRoot, this.Network.RootFolderName),
                    this.Network);
            }
            else
            {
                // Combine the data directory with the network's root folder and name.
                var directoryPath = Path.Combine(this.DataDir, this.Network.RootFolderName, this.Network.Name);
            ...
            this.EnableSignalR = this.ConfigReader.GetOrDefault("enableSignalR", false, this.Logger);
        ...
        /// <summary>
        ///     A list of paths to folders which Full Node components use to store data. These folders are found
        ...
        /// <summary>
        ///     The path to the Full Node's configuration file.
        ///     This value is read-only and can only be set via the NodeSettings constructor's arguments.
        /// </summary>
        public string ConfigurationFile { get; }

        /// <summary>
        ///     A combination of the settings from the Full Node's configuration file and the command
        ...
        /// <summary>
        ///     The network which the node is configured to run on. The network can be a "mainnet", "testnet",
        ...
        /// <summary>
        ///     A string that is used to help identify the Full Node when it connects to other peers on a network.
        /// </summary>
        ...
        /// <summary>
        ///     The default fee for a kB of transactions on the node. This value is used if no fee is specified for
        ///     a transaction.
        /// </summary>
        public FeeRate FallbackTxFeeRate { get; private set; }

        /// <summary>
        ///     The minimum relay transaction fee for a kB of transactions on the node. A miner may not be prepared
        ...
        public bool PrintHelpAndExit =>
            this.ConfigReader.GetOrDefault("help", false, this.Logger) ||
            this.ConfigReader.GetOrDefault("-help", false, this.Logger);
        ...
        void LoadConfiguration()
        {
            ...
            this.FallbackTxFeeRate =
                new FeeRate(config.GetOrDefault("fallbackfee", this.Network.FallbackFee, this.Logger));
            this.MinRelayTxFeeRate =
                new FeeRate(config.GetOrDefault("minrelaytxfee", this.Network.MinRelayTxFee, this.Logger));
        }
        ...
        string CreateDefaultDataDirectories(string appName, Network network)
        {
        ...
        public static void PrintHelp(Network network)
        {
            ...
            builder.AppendLine(
                "-datadirroot=<Path>       The path to the root data directory, which holds all node data on the machine. Defaults to 'FullNodeRoot'.");
            builder.AppendLine(
                "-debug[=<string>]         Set 'Debug' logging level. Specify what to log via e.g. '-debug=UnnamedCoin.Bitcoin.Miner,UnnamedCoin.Bitcoin.Wallet'.");
            builder.AppendLine(
            ...
            builder.AppendLine(
                $"-minrelaytxfee=<number>   Minimum relay fee rate. Defaults to {network.MinRelayTxFee}.");
