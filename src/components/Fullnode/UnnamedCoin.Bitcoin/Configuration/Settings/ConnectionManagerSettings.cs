...
namespace UnnamedCoin.Bitcoin.Configuration.Settings
{
    /// <summary>
    ...
    public sealed class ConnectionManagerSettings
    {
        ...
        readonly List<IPEndPoint> addNode;

        /// <summary>
        ///     Protects access to the list of addnode endpoints.
        /// </summary>
        readonly object addNodeLock;
        ...
        public ConnectionManagerSettings(NodeSettings nodeSettings)
        {
            ...
            try
            {
                foreach (var addNode in config.GetAll("addnode", this.logger)
                    .Select(c => c.ToIPEndPoint(nodeSettings.Network.DefaultPort)))
                    AddAddNode(addNode);
            }
            catch (FormatException)
            ...
            this.Port = config.GetOrDefault("port", nodeSettings.Network.DefaultPort, this.logger);

            try
            {
                var whitebindEndpoints = config.GetAll("whitebind", this.logger).Select(s => s.ToIPEndPoint(this.Port));

                this.Bind = whitebindEndpoints.Where(x => x.Address.AnyIP())
                    .Select(x => new NodeServerEndpoint(x, true)).ToList();

                foreach (var endPoint in whitebindEndpoints.Where(x => !x.Address.AnyIP()))
                {
            ...
            try
            {
                foreach (var endPoint in config.GetAll("bind")
            ...
            var externalIp = config.GetOrDefault<string>("externalip", null, this.logger);
            if (externalIp != null)
                try
                {
            ...
            if (this.ExternalEndpoint == null) this.ExternalEndpoint = new IPEndPoint(IPAddress.Loopback, this.Port);

            this.BanTimeSeconds =
                config.GetOrDefault("bantime", nodeSettings.Network.DefaultBanTimeSeconds, this.logger);
            ...
            this.Listen = config.GetOrDefault("listen", !this.Connect.Any(), this.logger);

            this.MaxOutboundConnections = config.GetOrDefault("maxoutboundconnections",
                nodeSettings.Network.DefaultMaxOutboundConnections, this.logger);
            if (this.MaxOutboundConnections <= 0)
                throw new ConfigurationException("The 'maxoutboundconnections' must be greater than zero.");

            this.MaxInboundConnections = config.GetOrDefault("maxinboundconnections",
                nodeSettings.Network.DefaultMaxInboundConnections, this.logger);
            ...
            this.SyncTimeEnabled = config.GetOrDefault("synctime", true, this.logger);
            this.RelayTxes = !config.GetOrDefault("blocksonly", DefaultBlocksOnly, this.logger);
            this.IpRangeFiltering = config.GetOrDefault("IpRangeFiltering", true, this.logger);
            ...
            this.IsGateway = config.GetOrDefault("gateway", false, this.logger);
        }
        ...
        /// <summary>
        ///     The amount of connections to be reached before a 1 second connection interval in the
        ///     <see cref="P2P.PeerConnectorDiscovery" /> is set.
        ///     <para>
        ///         When the <see cref="P2P.PeerConnectorDiscovery" /> starts up, a 100ms delay is set as the connection interval
        ///         in order for
        ...
        /// <summary>
        ///     <c>true</c> to sync time with other peers and calculate adjusted time, <c>false</c> to use our system clock
        ///     only.
        /// </summary>
        public bool SyncTimeEnabled { get; }
        ...
        /// <summary>
        ///     List of white listed IP endpoint. The node will flags peers that connects to the node, or that the node
        ///     connects to, as whitelisted.
        /// </summary>
        ...
        public static void BuildDefaultConfigurationFile(StringBuilder builder, Network network)
        {
            ...
            builder.AppendLine(
                $"#Number of seconds to keep misbehaving peers from reconnecting. Default {network.DefaultBanTimeSeconds}.");
            builder.AppendLine("#bantime=<number>");
            builder.AppendLine(
                $"#The maximum number of outbound connections. Default {network.DefaultMaxOutboundConnections}.");
            builder.AppendLine("#maxoutboundconnections=<number>");
            builder.AppendLine(
                $"#The maximum number of inbound connections. Default {network.DefaultMaxInboundConnections}.");
            ...
            builder.AppendLine(
                $"#An optional prefix for the node's user agent shared with peers. Truncated if over {MaximumAgentPrefixLength} characters.");
            builder.AppendLine("#agentprefix=<string>");
            builder.AppendLine(
                $"#Enable bandwidth saving setting to send and received confirmed blocks only. Defaults to {(DefaultBlocksOnly ? 1 : 0)}.");
        ...
        public static void PrintHelp(Network network)
        {
            ...
            builder.AppendLine(
                $"-port=<port>              The default network port to connect to. Default {network.DefaultPort}.");
            builder.AppendLine(
            ...
            builder.AppendLine(
                $"-bantime=<number>         Number of seconds to keep misbehaving peers from reconnecting. Default {network.DefaultBanTimeSeconds}.");
            builder.AppendLine(
                $"-maxoutboundconnections=<number> The maximum number of outbound connections. Default {network.DefaultMaxOutboundConnections}.");
            builder.AppendLine(
                $"-maxinboundconnections=<number> The maximum number of inbound connections. Default {network.DefaultMaxInboundConnections}.");
            builder.AppendLine(
            ...
            builder.AppendLine(
                $"-blocksonly=<0 or 1>      Enable bandwidth saving setting to send and received confirmed blocks only. Defaults to {DefaultBlocksOnly}.");
            builder.AppendLine(
