...
namespace UnnamedCoin.Bitcoin.Configuration.Settings
{
    ...
    public class LogSettings
    {
        ...
        {
            this.DebugArgs = config.GetOrDefault("debug", string.Empty).Split(',').Where(s => !string.IsNullOrEmpty(s))
                .ToList();

            // Get the minimum log level. The default is either Information or Debug depending on the DebugArgs.
            this.LogLevel = this.DebugArgs.Any() ? LogLevel.Debug : LogLevel.Info;

            var logLevelArg = config.GetOrDefault("loglevel", string.Empty);
            if (!string.IsNullOrEmpty(logLevelArg))
                this.LogLevel = LogLevel.FromString(logLevelArg);
        }
    }
}