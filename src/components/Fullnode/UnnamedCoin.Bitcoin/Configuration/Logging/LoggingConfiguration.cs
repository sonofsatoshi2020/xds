...
namespace UnnamedCoin.Bitcoin.Configuration.Logging
{
    /// <summary>
    ...
    public static class LoggingConfiguration
    {
        ...
        static readonly Dictionary<string, string> KeyCategories =
            new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
            {
                ...
                // { "lock", "" },
                // { "mempoolrej", "" },
                {"net", $"{nameof(UnnamedCoin)}.{nameof(Bitcoin)}.{nameof(Connection)}.*"},
                // { "proxy", "" },
                // { "prune", "" },
                // { "rand", "" },
                // { "reindex", "" },
                ...
                // Short Names
                {"configuration", $"{nameof(UnnamedCoin)}.{nameof(Bitcoin)}.{nameof(Configuration)}.*"},
                {"fullnode", $"{nameof(UnnamedCoin)}.{nameof(Bitcoin)}.{nameof(FullNode)}"}
            };
        ...
        /// </summary>
        /// <param name="settings">
        ///     Node log settings to extend the rules from the configuration file, or null if no extension is
        ///     required.
        /// </param>
        /// <param name="dataFolder">Data folder to determine path to log files.</param>
        static void AddFilters(LogSettings settings = null, DataFolder dataFolder = null)
        {
            ...
            foreach (var debugTarget in debugTargets)
            {
                ...
                {
                    var currentArchive =
                        debugFileTarget.ArchiveFileName.Render(new LogEventInfo {TimeStamp = DateTime.UtcNow});
            ...
            var defaultRule = new LoggingRule($"{nameof(UnnamedCoin)}.*", settings.LogLevel, mainTarget);
                ...
                lock (KeyCategories)
                {
                    ...
                    foreach (var key in settings.DebugArgs)
                    {
                        if (!KeyCategories.TryGetValue(key.Trim(), out var category))
                            // Allow direct specification - e.g. "-debug=UnnamedCoin.Bitcoin.Miner".
                            category = key.Trim();
            ...
            if (settings != null)
                if (settings.DebugArgs.Any())
                {
                    if (settings.DebugArgs[0] == "1")
                        ...
                        consoleLoggerSettings.Switches.Add($"{nameof(UnnamedCoin)}.{nameof(Bitcoin)}", LogLevel.Debug);
                    else
                        lock (KeyCategories)
                        {
                            var usedCategories = new HashSet<string>(StringComparer.Ordinal);
                            ...
                            foreach (var key in settings.DebugArgs)
                            {
                                ...
                                {
                                    usedCategories.Add(category);
                                    consoleLoggerSettings.Switches.Add(category.TrimEnd('*').TrimEnd('.'),
                                        LogLevel.Debug);
                                }
                            }
                        }
