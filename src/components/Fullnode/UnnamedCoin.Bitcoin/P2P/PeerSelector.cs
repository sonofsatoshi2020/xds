...
namespace UnnamedCoin.Bitcoin.P2P
{
    ...
    public sealed class PeerSelector : IPeerSelector
    {
        ...
        const int DiscoveryThresholdHours = 24;
        ...
        /// </summary>
        readonly ConcurrentDictionary<IPEndPoint, PeerAddress> peerAddresses;
        ...
        public PeerSelector(IDateTimeProvider dateTimeProvider, ILoggerFactory loggerFactory,
            ConcurrentDictionary<IPEndPoint, PeerAddress> peerAddresses, ISelfEndpointTracker selfEndpointTracker)
        {
        ...
        {
            var notBanned = NotBanned();

            var attemptedReachedThresholdCount =
                notBanned.Count(p => p.ConnectionAttempts >= PeerAddress.AttemptThreshold);
            var areAllPeersReachedThreshold = attemptedReachedThresholdCount == notBanned.Count();
        ...
        {
            var notBanned = NotBanned().ToList();
        ...
        public IEnumerable<PeerAddress> SelectPeersForDiscovery(int peerCount)
        {
            ...
            var filteredDiscoverable = this.peerAddresses.Values.Where(p =>
                p.LastDiscoveredFrom != null && p.LastDiscoveredFrom <
                this.dateTimeProvider.GetUtcNow().AddHours(-DiscoveryThresholdHours));
        ...
        public IEnumerable<PeerAddress> SelectPeersForGetAddrPayload(int peerCount)
        {
            ...
            var connectedAndHandshaked = Connected(0).Concat(Handshaked(0)).OrderBy(p => this.random.Next()).ToList();
            var freshAndAttempted = Attempted().Concat(Fresh()).OrderBy(p => this.random.Next()).ToList();
        ...
        {
            IEnumerable<PeerAddress> filteredPeers = peers.Where(p =>
                    p.HandshakedAttempts < PeerAddress.AttemptHandshakeThreshold ||
                    p.LastHandshakeAttempt?.AddHours(PeerAddress.AttempThresholdHours) <
        ...
        public IEnumerable<PeerAddress> Connected(int throttlePeriodSeconds = 60)
        {
            var result = this.peerAddresses.Values
                ...
                .Where(p => p.LastConnectionSuccess <
                            this.dateTimeProvider.GetUtcNow().AddSeconds(-throttlePeriodSeconds));
            return result;
        }
        ...
        public IEnumerable<PeerAddress> Handshaked(int throttlePeriodSeconds = 60)
        {
            var result = this.peerAddresses.Values
                ...
                .Where(p => p.LastConnectionHandshake <
                            this.dateTimeProvider.GetUtcNow().AddSeconds(-throttlePeriodSeconds));
            return result;
        }
        ...
        IEnumerable<PeerAddress> SelectPreferredPeers()
        {
            ...
            var attempted = Attempted().ToList();
            var fresh = Fresh().ToList();
        ...
        PeerAddress Random(IEnumerable<PeerAddress> peers)
        {
            if (peers.Count() == 1)
                return peers.First();

            var randomPeerIndex = this.random.Next(peers.Count());
        ...
        /// </summary>
        bool IsBanned(PeerAddress peerAddress)
        {
