...
namespace UnnamedCoin.Bitcoin.P2P.Protocol.Payloads
{
    ...
    public class RejectPayload : Payload
    {
        ...
        VarString message = new VarString();

        /// <summary>Details of the error.</summary>
        VarString reason = new VarString();

        /// <summary>"tx" or "block".</summary>
        public string Message
        {
            get => Encoders.ASCII.EncodeData(this.message.GetString(true));

            set => this.message = new VarString(Encoders.ASCII.DecodeData(value));
        }

        public RejectCode Code
        {
            get => (RejectCode) this.code;
        ...
        {
            get => Encoders.ASCII.EncodeData(this.reason.GetString(true));

            set => this.reason = new VarString(Encoders.ASCII.DecodeData(value));
        }
