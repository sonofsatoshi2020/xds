...
namespace UnnamedCoin.Bitcoin.P2P.Protocol.Payloads
{
    ...
    {
        const int MaxSubversionLength = 256;

        static string userAgentNBitcoin;

        NetworkAddress addressFrom = new NetworkAddress();

        NetworkAddress addressReceiver = new NetworkAddress();
        ...
        {
            get => (NetworkPeerServices) this.services;

            set => this.services = (ulong) value;
        }

        public DateTimeOffset Timestamp
        {
            get => Utils.UnixTimeToDateTime((uint) this.timestamp);

            set => this.timestamp = Utils.DateTimeToUnixTime(value);
        }

        public IPEndPoint AddressReceiver
        {
            get => this.addressReceiver.Endpoint;

            set => this.addressReceiver.Endpoint =
                value ?? throw new InvalidOperationException("Can't set 'AddressReceiver' to null.");
        }

        public IPEndPoint AddressFrom
        {
            get => this.addressFrom.Endpoint;

            set => this.addressFrom.Endpoint =
                value ?? throw new InvalidOperationException("Can't set 'AddressFrom' to null.");
        }
        ...
        {
            get => Encoders.ASCII.EncodeData(this.userAgent.GetString());
