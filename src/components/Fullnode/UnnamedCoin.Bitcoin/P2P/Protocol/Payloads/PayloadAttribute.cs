...
namespace UnnamedCoin.Bitcoin.P2P.Protocol.Payloads
{
    ...
    ///     This is used by the P2P code to map and deserialize messages that are received from the tcp network to a concrete
    ///     type.
    /// </summary>
    public class PayloadProvider
    {
        ...
        /// <summary>
        ///     Discover all payloads from the provided assembly, if no assembly is provided defaults to
        ///     <see cref="PayloadAttribute" />.
        /// </summary>
        ...
        {
            if (!this.nameToType.TryGetValue(commandName, out var result))
