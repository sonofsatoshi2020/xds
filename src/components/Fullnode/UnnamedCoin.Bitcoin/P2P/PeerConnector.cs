...
namespace UnnamedCoin.Bitcoin.P2P
{
    ...
    public interface IPeerConnector : IDisposable
    {
        ...
        ///     <para>
        ///         If the maximum amount of connections has been reached (<see cref="MaxOutboundConnections" />), the action gets
        ///         skipped.
        ///     </para>
        /// </summary>
    ...
    public abstract class PeerConnector : IPeerConnector
    {
        ...
        readonly TimeSpan connectionInterval;
        ...
        public void StartConnectAsync()
        {
            ...
            this.asyncLoop = this.asyncProvider.CreateAndRunAsyncLoop($"{GetType().Name}.{nameof(ConnectAsync)}",
                async token =>
                {
                    if (!this.PeerAddressManager.Peers.Any() ||
                        this.ConnectorPeers.Count >= this.MaxOutboundConnections)
                        return;
        ...
        bool IsPeerConnected(IPEndPoint ipEndpoint)
        {
        ...
        public async Task ConnectAsync(PeerAddress peerAddress)
        {
            ...
            {
                this.logger.LogDebug("Connect aborted: {0} is banned until {1}.", peerAddress.Endpoint,
                    peerAddress.BanUntil);
                return;
            }
            ...
            try
            {
                    ...
                    CancellationTokenSource.CreateLinkedTokenSource(this.NodeLifetime.ApplicationStopping))
                {
                    ...
                    var clonedConnectParameters = this.CurrentParameters.Clone();
                    timeoutTokenSource.CancelAfter(5000);
                    clonedConnectParameters.ConnectCancellation = timeoutTokenSource.Token;

                    peer = await this.networkPeerFactory
                        .CreateConnectedNetworkPeerAsync(peerAddress.Endpoint, clonedConnectParameters,
                            this.networkPeerDisposer).ConfigureAwait(false);
            ...
            catch (OperationCanceledException)
            {
                ...
                {
                    this.logger.LogDebug("Peer {0} connection canceled because application is stopping.",
            ...
            {
                this.logger.LogDebug("Exception occurred while connecting: {0}",
                    exception is SocketException ? exception.Message : exception.ToString());
                peerAddress.SetHandshakeAttempted(this.dateTimeProvider.GetUtcNow());
        ...
        {
            return this.ConnectorPeers.Count < this.ConnectionSettings.InitialConnectionTarget
                ? TimeSpans.Ms100
                : TimeSpans.Second;
        }
        ...
        void OnPeerDisposed(INetworkPeer peer)
        {
            RemovePeer(peer);
        }
    }
