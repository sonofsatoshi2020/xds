...
namespace UnnamedCoin.Bitcoin.P2P.Peer
{
    ...
    ///     </para>
    ///     <para>
    ///         When a new peer is created (and the <see cref="OnPeerDisconnectedHandler" /> callback is used as an
    ///         <see cref="NetworkPeer.onDisconnected" /> in the constructor)
    ///         by a component that utilizes this class, <see cref="AddPeer" /> should be used to inform  this class about it.
    ///         Once the peer is added, the owning component no
    ...
    public class NetworkPeerDisposer : IDisposable
    {
        ...
        readonly ConcurrentDictionary<int, INetworkPeer> connectedPeers;

        /// <summary>Instance logger.</summary>
        readonly ILogger logger;

        /// <summary>Callback that is called before the peer is disposed.</summary>
        readonly Action<INetworkPeer> onPeerDisposed;

        /// <summary>Queue of disconnected peers to be disposed.</summary>
        readonly IAsyncDelegateDequeuer<INetworkPeer> peersToDispose;
        ...
        public NetworkPeerDisposer(ILoggerFactory loggerFactory, IAsyncProvider asyncProvider,
            Action<INetworkPeer> onPeerDisposed = null)
        {
            ...
            var dequeuerName = $"{nameof(NetworkPeerDisposer)}-{nameof(this.peersToDispose)}";
            this.peersToDispose =
                asyncProvider.CreateAndRunAsyncDelegateDequeuer<INetworkPeer>(dequeuerName, OnEnqueueAsync);
        }

        /// <summary>Gets the connected peers count.</summary>
        public int ConnectedPeersCount => this.connectedPeers.Count;
        ...
        public void Dispose()
        {
            ...
            foreach (var peer in this.connectedPeers.Values)
            {
        ...
        Task OnEnqueueAsync(INetworkPeer peer, CancellationToken cancellationToken)
        {
            this.onPeerDisposed?.Invoke(peer);

            peer.Dispose();

            this.connectedPeers.TryRemove(peer.Connection.Id, out var unused);
