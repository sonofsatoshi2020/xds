...
namespace UnnamedCoin.Bitcoin.Base.Deployments
{
    ...
    public class ThresholdConditionCache
    {
        ...
        const int VersionbitsLastOldBlockVersion = 4;
        ...
        const int VersionbitsNumBits = 29;

        // Cache of BIP9 deployment states keyed by block hash.
        readonly Dictionary<uint256, ThresholdState?[]> cache = new Dictionary<uint256, ThresholdState?[]>();

        // Used to access the deployments, confirmation window and activation threshold.
        readonly IConsensus consensus;
        ...
        public ThresholdState[] GetStates(ChainedHeader pindexPrev)
        {
            ...
            for (var i = 0; i < array.Length; i++) array[i] = GetState(pindexPrev, i);
            ...
            ThresholdState[] thresholdStates)
        {
            ...
            for (var deploymentIndex = 0; deploymentIndex < array.Length; deploymentIndex++)
            {
                ...
                {
                    if (Condition(headerTemp, deploymentIndex)) votes++;
                ...
                if (firstSeenHash.Key != null) sinceHeight = indexPrev.FindAncestorOrSelf(firstSeenHash.Key).Height;
        ...
        public ThresholdState GetState(ChainedHeader indexPrev, int deployment)
        {
            ...
            var period = this.consensus.MinerConfirmationWindow;
            var threshold = this.consensus.BIP9Deployments[deployment].Threshold;
            ...
            var state = Get(indexPrev?.HashBlock, deployment);
            ...
            while (vToCompute.Count != 0)
            {
                ...
                switch (state)
                {
                    case ThresholdState.Defined:
                    {
                        ...
                        else if (indexPrev.GetMedianTimePast() >= timeStart) stateNext = ThresholdState.Started;
                    ...
                    case ThresholdState.Started:
                    {
                        ...
                        var pindexCount = indexPrev;
                        var count = 0;
                        for (var i = 0; i < period; i++)
                        {
                            if (Condition(pindexCount, deployment)) count++;

                            pindexCount = pindexCount.Previous;
                        }

                        // If the threshold has been met then lock in the BIP activation.
                        if (count >= threshold) stateNext = ThresholdState.LockedIn;
                ...
                Set(indexPrev?.HashBlock, deployment, state = stateNext);
            }
        ...
        ThresholdState Get(uint256 hash, int deployment)
        {
            if (hash == null)
        ...
        void Set(uint256 hash, int deployment, ThresholdState state)
        {
            if (hash == null)
                return;
            ThresholdState?[] threshold;
        ...
        bool ContainsKey(uint256 hash, int deployment)
        {
            if (hash == null)
                return true;
            ThresholdState?[] threshold;
        ...
        /// </summary>
        bool Condition(ChainedHeader pindex, int deployment)
        {
            ...
            // When a block nVersion does not have top bits 001, it is treated as if all bits are 0 for the purposes of deployments.
            return (pindex.Header.Version & VersionbitsTopMask) == VersionbitsTopBits &&
                   (pindex.Header.Version & Mask(deployment)) != 0;
        }
