...
namespace UnnamedCoin.Bitcoin.Builder
{
    ...
    public class FullNodeFeatureExecutor : IFullNodeFeatureExecutor
    {
        ...
        void Execute(Action<IFullNodeFeature> callback, bool disposing = false)
        {
            ...
            if (disposing)
                ...
                foreach (var feature in this.node.Services.Features.Reverse())
                    try
                    {
                        callback(feature);
                    }
                ...
                try
                {
                    ...
                    foreach (var feature in this.node.Services.Features.OrderByDescending(f => f.InitializeBeforeBase))
                        callback(feature);
                }
                catch (Exception exception)
                {
        ...
        void LogAndAddException(List<Exception> exceptions, Exception exception)
        {
