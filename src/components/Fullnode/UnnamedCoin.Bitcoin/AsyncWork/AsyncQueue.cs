...
namespace UnnamedCoin.Bitcoin.AsyncWork
{
    ...
    class AsyncQueue<T> : IDisposable, IAsyncQueue<T>, IAsyncDelegateDequeuer<T>
    {
        /// <summary>
        ...
        volatile int unfinishedDequeueCount;
        ...
        internal Task ConsumerTask { get; }
        ...
        public async Task<T> DequeueAsync(CancellationToken cancellation = default)
        {
            ...
            try
            {
                ...
                using (var cancellationSource =
                    CancellationTokenSource.CreateLinkedTokenSource(cancellation, this.cancellationTokenSource.Token))
                {
                    while (true)
                    {
        ...
        ///     <c>false</c> otherwise.
        /// </param>
        void DisposeInternal(bool calledFromConsumerTask)
        {
