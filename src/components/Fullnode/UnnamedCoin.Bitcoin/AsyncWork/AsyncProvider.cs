...
namespace UnnamedCoin.Bitcoin.AsyncWork
{
    /// <summary>
    ...
    {
        const int DefaultLoopRepeatInterval = 1000;
        ...
        /// </summary>
        readonly Dictionary<IAsyncDelegate, AsyncTaskInfo> asyncDelegates;

        readonly (string Name, int Width)[] benchmarkColumnsDefinition =
        {
            (Name: "Name", Width: 80),
            (Name: "Type", Width: 15),
        ...
        readonly object lockAsyncDelegates;
        readonly object lockRegisteredTasks;
        ...
        /// </summary>
        readonly Dictionary<Task, AsyncTaskInfo> registeredTasks;
        ...
        /// <inheritdoc />
        public ISignals Signals { get; }

        /// <inheritdoc />
        public IAsyncDelegateDequeuer<T> CreateAndRunAsyncDelegateDequeuer<T>(string friendlyName,
            Func<T, CancellationToken, Task> @delegate)
        {
            ...
            lock (this.lockAsyncDelegates)
            {
                ...
                this.asyncDelegates.Add(newDelegate,
                    new AsyncTaskInfo(friendlyName, AsyncTaskInfo.AsyncTaskType.Dequeuer));
            }
        ...
        /// <inheritdoc />
        public IAsyncLoop CreateAndRunAsyncLoop(string name, Func<CancellationToken, Task> loop,
            CancellationToken cancellation, TimeSpan? repeatEvery = null, TimeSpan? startAfter = null)
        {
            ...
            loopTask = loopInstance.Run(cancellation,
                repeatEvery ?? TimeSpan.FromMilliseconds(DefaultLoopRepeatInterval), startAfter).RunningTask;
        ...
        /// <inheritdoc />
        public IAsyncLoop CreateAndRunAsyncLoopUntil(string name, CancellationToken cancellation, Func<bool> condition,
        ...
        public Task RegisterTask(string name, Task taskToRegister)
        {
            ...
            {
                this.registeredTasks.Add(taskToRegister,
                    new AsyncTaskInfo(name, AsyncTaskInfo.AsyncTaskType.RegisteredTask));
            }
        ...
        public bool IsAsyncDelegateDequeuerRunning(IAsyncDelegate asyncDelegate)
        {
            lock (this.lockAsyncDelegates)
            {
                if (this.asyncDelegates.TryGetValue(asyncDelegate, out var delegateInfo))
        ...
        public bool IsAsyncDelegateDequeuerRunning(string name)
        {
            lock (this.lockAsyncDelegates)
            {
                ...
                return this.asyncDelegates.Values.Any(@delegate =>
                    @delegate.FriendlyName == name && @delegate.Status != TaskStatus.Faulted &&
                    @delegate.Type == AsyncTaskInfo.AsyncTaskType.Dequeuer);
            }
        }
        ...
        public bool IsAsyncLoopRunning(string name)
        {
            lock (this.lockAsyncDelegates)
            {
                ...
                return this.asyncDelegates.Values.Any(@delegate =>
                    @delegate.FriendlyName == name && @delegate.Status != TaskStatus.Faulted &&
                    @delegate.Type == AsyncTaskInfo.AsyncTaskType.Loop);
            }
        }
        ...
        public bool IsRegisteredTaskRunning(string name)
        {
            lock (this.lockRegisteredTasks)
            {
                ...
                return this.registeredTasks.Values.Any(@delegate =>
                    @delegate.FriendlyName == name && @delegate.Status != TaskStatus.Faulted &&
                    @delegate.Type == AsyncTaskInfo.AsyncTaskType.RegisteredTask);
            }
        }
        ...
        public string GetStatistics(bool faultyOnly)
        {
            ...
            var running = taskInformations.Where(info => info.IsRunning).Count();
            var faulted = taskInformations.Where(info => !info.IsRunning).Count();

            var sb = new StringBuilder();
            sb.AppendLine();
            sb.AppendLine(
                $"====== Async loops ======   [Running: {running.ToString()}] [Faulted: {faulted.ToString()}]");

            if (faultyOnly && faulted == 0)
                return
                    sb.ToString(); // If there are no faulty tasks and faultOnly is set to true, return just the header.
            ...
            foreach (var item in this.benchmarkColumnsDefinition) sb.Append(item.Name.PadRight(item.Width));
            ...
            foreach (var row in data)
            {
                ...
                for (var iColumn = 0; iColumn < this.benchmarkColumnsDefinition.Length; iColumn++)
        ...
        public List<(string loopName, TaskStatus status)> GetAll()
        {
            ...
            List<(string, TaskStatus)> runningTasks =
                taskInformation.Select(a => (a.FriendlyName, a.Status)).OrderBy(a => a.Item1).ToList();

            return runningTasks;
        }

        void OnRegisteredTaskCompleted(Task task)
        {
            AsyncTaskInfo itemToRemove;
            lock (this.lockRegisteredTasks)
            {
                if (this.registeredTasks.TryGetValue(task, out itemToRemove)) this.registeredTasks.Remove(task);
            }
        ...
        void OnRegisteredTaskUnhandledException(Task task)
        {
            ...
            lock (this.lockRegisteredTasks)
            {
                ...
                this.logger.LogError(task.Exception.GetBaseException(), "Unhandled exception for registered task {0}.",
                    delegateInfo.FriendlyName);
            }
        }
        ...
        void OnAsyncDelegateUnhandledException(Task task, object state)
        {
            AsyncTaskInfo delegateInfo;
            lock (this.lockAsyncDelegates)
            {
                ...
                {
                    // Should never happen.
                    this.logger.LogError("Cannot find the AsyncDelegateInfo related to the faulted task with Id {0}.",
                        task.Id);
                    return;
                }

                this.logger.LogError(task.Exception.GetBaseException(),
                    "Unhandled exception for async delegate worker {0}.", delegateInfo.FriendlyName);
            }
        }
        ...
        /// <param name="task">The delegate task.</param>
        /// <param name="state">The <see cref="IAsyncDelegate" /> that's run by the delegateTask</param>
        void OnAsyncDelegateCompleted(Task task, object state)
        {
            AsyncTaskInfo itemToRemove;
            lock (this.lockAsyncDelegates)
            {
                if (this.asyncDelegates.TryGetValue((IAsyncDelegate) state, out itemToRemove))
                {
                    ...
                    if (state is AsyncLoop asyncLoop && asyncLoop.UncaughtException != null)
                    {
                        ...
                        this.logger.LogError(
                            "Async Loop '{0}' completed with an UncaughtException, marking it as faulted. Task Id: {1}.",
                            itemToRemove.FriendlyName, task.Id);
                        return;
                    }
