...
namespace UnnamedCoin.Bitcoin.AsyncWork
{
    ...
    public class AsyncLoop : IAsyncLoop
    {
        ...
        public IAsyncLoop Run(CancellationToken cancellation, TimeSpan? repeatEvery = null, TimeSpan? startAfter = null)
        {
            ...
            this.RunningTask = StartAsync(cancellation, startAfter);
        ...
        Task StartAsync(CancellationToken cancellation, TimeSpan? delayStart = null)
        {
            return Task.Run(async () =>
            {
                try
                {
                    if (delayStart != null)
                    {
                        this.logger.LogInformation("{0} starting in {1} seconds.", this.Name,
                            delayStart.Value.TotalSeconds);
                ...
                if (this.UncaughtException != null)
                {
                    ...
                    this.logger.LogCritical(new EventId(0), this.UncaughtException,
                        this.Name + " threw an unhandled exception");

                    // You can touch this one.
                    this.logger.LogError("{0} threw an unhandled exception: {1}", this.Name,
                        this.UncaughtException.ToString());
                }
            }, cancellation);
        }
    }
