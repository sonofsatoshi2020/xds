...
namespace UnnamedCoin.Bitcoin.Consensus
{
    ...
    public class ConsensusManagerBehavior : NetworkPeerBehavior
    {
        ...
        const int AutosyncIntervalMinutes = 10;
        ...
        readonly object bestSentHeaderLock;
        ...
        readonly List<BlockHeader> cachedHeaders;
        ...
        public ConsensusManagerBehavior(ChainIndexer chainIndexer, IInitialBlockDownloadState initialBlockDownloadState,
        ...
        protected IInitialBlockDownloadState InitialBlockDownloadState { get; }
        ...
        protected ChainIndexer ChainIndexer { get; }

        /// <inheritdoc cref="IPeerBanning" />
        protected IPeerBanning PeerBanning { get; }
        ...
        /// </summary>
        /// <remarks>
        ///     The announced tip is accepted if it seems to be valid. Validation is only done on headers and so the announced tip
        ///     may refer to invalid block.
        /// </remarks>
        ...
        {
            var lastHeader = this.ConsensusManager.Tip;
        ...
        /// <param name="getHeadersPayload">
        ///     The <see cref="GetHeadersPayload" /> payload that triggered the creation of this
        ///     payload.
        /// </param>
        /// <param name="lastHeader">
        ///     <see cref="ChainedHeader" /> of the last header that was added to the
        ///     <see cref="HeadersPayload" />.
        /// </param>
        /// <returns>Payload with headers from locator towards consensus tip or <c>null</c> in case locator was invalid.</returns>
        protected virtual Payload ConstructHeadersPayload(GetHeadersPayload getHeadersPayload,
            out ChainedHeader lastHeader)
        {
            var fork = this.ChainIndexer.FindFork(getHeadersPayload.BlockLocator);
            ...
            var header = GetLastHeaderToSend(fork, getHeadersPayload.HashStop);

            for (var heightIndex = header.Height; heightIndex > fork.Height; heightIndex--)
            {
                lastHeader = header;
            ...
            this.logger.LogDebug("{0} headers were selected for sending, last one is '{1}'.",
                headersPayload.Headers.Count, headersPayload.Headers.LastOrDefault()?.GetHash());
        ...
        {
            this.logger.LogDebug("Received {0} headers. First: '{1}'  Last: '{2}'.", headers.Count,
                headers.FirstOrDefault()?.ToString(), headers.LastOrDefault()?.ToString());

            if (headers.Count == 0)
            {
                this.logger.LogDebug(
                    "Headers payload with no headers was received. Assuming we're synced with the peer.");
                this.logger.LogTrace("(-)[NO_HEADERS]");
                return;
            }

            if (!ValidateHeadersFromPayload(peer, headers, out var validationError))
            {
            ...
            using (await this.asyncLock.LockAsync().ConfigureAwait(false))
            {
                ...
                ) // TODO when proven headers are implemented combine this with size threshold of N mb.
                {
                ...
                {
                    var cachedHeader = this.cachedHeaders.Last().GetHash();
                    var prevNewHeader = headers.First().HashPrevBlock;
                    ...
                    if (cachedHeader == prevNewHeader)
                    {
                        ...
                        this.logger.LogDebug("{0} headers were added to cache, new cache size is {1}.", headers.Count,
                            this.cachedHeaders.Count);
                    ...
                    this.logger.LogDebug(
                        "Header {0} could not be connected to last cached header {1}, clear cache and resync.",
                        headers[0].GetHash(), cachedHeader);
                    this.logger.LogTrace("(-)[FAILED_TO_ATTACH_TO_CACHE]");
                    return;
                }

                var result = await PresentHeadersLockedAsync(headers).ConfigureAwait(false);
                ...
                this.logger.LogDebug("{0} is {1} and {2} is {3}", nameof(this.BestReceivedTip), this.BestReceivedTip,
                    nameof(result.Consumed), result.Consumed);
                ...
                if (result.Consumed.HashBlock != headers.Last().GetHash())
                {
                    ...
                    var consumedCount = GetConsumedHeadersCount(headers, result.Consumed.Header);
                    this.cachedHeaders.AddRange(headers.Skip(consumedCount));

                    this.logger.LogDebug("{0} out of {1} items were not consumed and added to cache.",
                        headers.Count - consumedCount, headers.Count);
                }
        ...
        protected bool ValidateHeadersFromPayload(INetworkPeer peer, List<BlockHeader> headers,
            out string validationError)
        {
            ...
            {
                this.logger.LogDebug(
                    "Headers payload with {0} headers was received. Protocol violation. Banning the peer.",
                    headers.Count);
            ...
            for (var i = 1; i < headers.Count; i++)
            {
                ...
                {
                    this.logger.LogDebug(
                        "Peer '{0}' presented non-consecutiveness hashes at position {1} with prev hash '{2}' not matching hash '{3}'.",
        ...
        protected async Task<ConnectNewHeadersResult> PresentHeadersLockedAsync(List<BlockHeader> headers,
            bool triggerDownload = true)
        {
            ...
            {
                this.logger.LogDebug("Peer's header is invalid. Peer will be banned and disconnected. Error: {0}.",
                    exception.ConsensusError);
                this.PeerBanning.BanAndDisconnectPeer(peer.PeerEndPoint,
                    $"Peer presented invalid header, error: {exception.ConsensusError}.");
            }
            ...
            catch (CheckpointMismatchException)
            {
                ...
                this.PeerBanning.BanAndDisconnectPeer(peer.PeerEndPoint,
                    "Peer presented header that violates a checkpoint.");
            }
        ...
        bool CheckIfUnsolicitedFutureHeader(List<BlockHeader> headers)
        {
        ...
        public void UpdateBestSentHeader(ChainedHeader header)
        {
            ...
            lock (this.bestSentHeaderLock)
            {
                ...
                {
                    var ancestorOrSelf = this.BestSentHeader.FindAncestorOrSelf(header);
        ...
        /// <summary>
        ///     Tries to sync the chain with the peer by sending it <see cref="GetHeadersPayload" /> in case peer's state is
        ///     <see cref="NetworkPeerState.HandShaked" />.
        /// </summary>
        public async Task ResyncAsync()
        {
            if (this.cachedHeaders.Any())
            {
                ...
                throw new ConsensusException(
                    $"Resync is only allowed to be called when the cache is empty. The current cache count is {this.cachedHeaders.Count}.");
            }
            ...
            {
                var getHeadersPayload = BuildGetHeadersPayload();
                ...
                try
                {
                    this.logger.LogDebug("Sending getheaders payload with first hash: '{0}'.",
                        getHeadersPayload.BlockLocator.Blocks.First());

                    peer.SendMessage(getHeadersPayload);
                }
                catch (OperationCanceledException)
                {
                    this.logger.LogDebug("Unable to send getheaders ({0}) message to peer '{1}'.",
                        getHeadersPayload.GetType().Name, peer.RemoteSocketEndpoint);
                }
            }
            else
            {
        ...
        protected async Task ProcessInvAsync(INetworkPeer peer, List<InventoryVector> inventory)
        {
            ...
            var blockHashAnnounced = false;
            foreach (var inventoryVector in inventory)
                if (inventoryVector.Type == InventoryType.MSG_BLOCK &&
                    this.ChainIndexer.GetHeader(inventoryVector.Hash) == null)
                {
        ...
        protected override void AttachCore()
        {
            ...
            var interval = (int) TimeSpan.FromMinutes(AutosyncIntervalMinutes).TotalMilliseconds;
        ...
        {
            return new ConsensusManagerBehavior(this.ChainIndexer, this.InitialBlockDownloadState,
                this.ConsensusManager, this.PeerBanning, this.LoggerFactory);
        }
        ...
        /// <summary>
        ///     Gets the count of consumed headers in a <paramref name="headers" /> list, giving a
        ///     <paramref name="consumedHeader" /> reference.
        ...
        int GetConsumedHeadersCount(List<BlockHeader> headers, BlockHeader consumedHeader)
        {
            var consumedHeaderHash = consumedHeader.GetHash();
