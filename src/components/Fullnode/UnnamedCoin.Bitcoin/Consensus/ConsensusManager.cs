...
using UnnamedCoin.Bitcoin.Consensus.PerformanceCounters.ConsensusManager;
using UnnamedCoin.Bitcoin.Consensus.ValidationResults;
...
namespace UnnamedCoin.Bitcoin.Consensus
{
    ...
    public class ConsensusManager : IConsensusManager
    {
        ...
        /// <remarks>All access should be protected by <see cref="peerLock" />.</remarks>
        readonly Dictionary<uint256, long> expectedBlockSizes;

        readonly IFinalizedBlockInfoRepository finalizedBlockInfo;

        readonly IFullValidator fullValidator;

        readonly IInitialBlockDownloadState ibdState;

        readonly IIntegrityValidator integrityValidator;
        ...
        /// <summary>
        ///     Protects access to the <see cref="blockPuller" />, <see cref="chainedHeaderTree" />,
        ///     <see cref="expectedBlockSizes" /> and <see cref="expectedBlockDataBytes" />.
        /// </summary>
        ...
        readonly Dictionary<int, INetworkPeer> peersByPeerId;

        readonly ConsensusManagerPerformanceCounter performanceCounter;

        readonly AsyncLock reorgLock;

        readonly ISignals signals;

        readonly Queue<BlockDownloadRequest> toDownloadQueue;

        /// <remarks>All access should be protected by <see cref="peerLock" />.</remarks>
        long expectedBlockDataBytes;
        ...
        /// </summary>
        long maxUnconsumedBlocksDataBytes { get; }
        ...
        long ConsumptionThresholdBytes => this.maxUnconsumedBlocksDataBytes / 10;
        ...
        public async Task InitializeAsync(ChainedHeader chainTip)
        {
            ...
            var consensusTipHash = this.ConsensusRules.GetBlockHash();
            ...
            while (true)
            {
                ...
                this.logger.LogInformation(
                    "Consensus at height {0} is ahead of the block store at height {1}, rewinding consensus.",
                    pendingTip, this.chainState.BlockStoreTip);

                // In case block store initialized behind, rewind until or before the block store tip.
                // The node will complete loading before connecting to peers so the chain will never know if a reorg happened.
                var transitionState = await this.ConsensusRules.RewindAsync().ConfigureAwait(false);
        ...
        /// <inheritdoc />
        public ConnectNewHeadersResult HeadersPresented(INetworkPeer peer, List<BlockHeader> headers,
            bool triggerDownload = true)
        {
            ...
            lock (this.peerLock)
            {
                ...
                this.chainState.IsAtBestChainTip = IsConsensusConsideredToBeSyncedLocked(out var bestPeerTip);
        ...
        public async Task<ChainedHeader> BlockMinedAsync(Block block)
        {
            ...
            using (await this.reorgLock.LockAsync().ConfigureAwait(false))
            {
                ...
                validationContext =
                    await this.partialValidator.ValidateAsync(chainedHeader, block).ConfigureAwait(false);
                ...
                if (validationContext.Error == null)
                {
                    ...
                    if (fullValidationRequired)
                    {
                        var fullValidationResult =
                            await FullyValidateLockedAsync(validationContext.ChainedHeaderToValidate, true)
                                .ConfigureAwait(false);
                        if (!fullValidationResult.Succeeded)
                        {
                            this.logger.LogDebug("Miner produced an invalid block, full validation failed: {0}",
                                fullValidationResult.Error.Message);
        ...
        public void GetOrDownloadBlocks(List<uint256> blockHashes, OnBlockDownloadedCallback onBlockDownloadedCallback)
        {
            ...
            {
                var chainedHeaderBlock = GetBlockData(blockHash);
        ...
        public ChainedHeaderBlock GetBlockData(uint256 blockHash)
        {
            ...
            var block = this.blockStore.GetBlock(blockHash);
            if (block != null)
            {
        ...
        public ChainedHeaderBlock[] GetBlockData(List<uint256> blockHashes)
        {
            ...
            lock (this.peerLock)
            {
                var blocks = this.chainedHeaderTree.GetChainedHeaderBlocks(blockHashes);

                for (var i = 0; i < blocks.Length; i++)
                {
                    var chainedHeaderBlock = blocks[i];
            ...
            var getFromStore = chainedHeaderBlocks.Where(kv => kv.Value != null && kv.Value.Block == null)
                .Select(kv => kv.Key).ToList();

            // Read those blocks from store and create new chained header block entries for them.
            foreach (var (block, index) in this.blockStore.GetBlocks(getFromStore).Select((b, n) => (b, n)))
                chainedHeaderBlocks[getFromStore[index]] =
                    new ChainedHeaderBlock(block, chainedHeaderBlocks[getFromStore[index]].ChainedHeader);
        ...
        void PeerDisconnectedLocked(int peerId)
        {
            var removed = this.peersByPeerId.Remove(peerId);

            if (removed)
            {
                var shuttingDown = this.nodeLifetime.ApplicationStopping.IsCancellationRequested;
        ...
        /// </summary>
        void ProcessDownloadedBlock(ChainedHeaderBlock chainedHeaderBlock)
        {
            if (chainedHeaderBlock.Block == null)
            {
                this.logger.LogTrace("(-)[DOWNLOAD_FAILED_NO_PEERS_CLAIMED_BLOCK]:'{0}'",
                    chainedHeaderBlock.ChainedHeader);
                return;
            }

            var partialValidationRequired = false;

            lock (this.peerLock)
            {
                partialValidationRequired =
                    this.chainedHeaderTree.BlockDataDownloaded(chainedHeaderBlock.ChainedHeader,
                        chainedHeaderBlock.Block);
            }

            this.logger.LogDebug("Partial validation is{0} required.",
                partialValidationRequired ? string.Empty : " NOT");

            if (partialValidationRequired)
                this.partialValidator.StartPartialValidation(chainedHeaderBlock.ChainedHeader, chainedHeaderBlock.Block,
                    OnPartialValidationCompletedCallbackAsync);
        }

        async Task OnPartialValidationCompletedCallbackAsync(ValidationContext validationContext)
        {
            ...
            if (validationContext.Error == null)
            {
                await OnPartialValidationSucceededAsync(validationContext.ChainedHeaderToValidate)
                    .ConfigureAwait(false);
            }
            else
            {
                ...
                lock (this.peerLock)
                {
                    ...
                    var peerIdsToBan =
                        this.chainedHeaderTree.PartialOrFullValidationFailed(validationContext.ChainedHeaderToValidate);

                    this.logger.LogDebug("Validation of block '{0}' failed, banning and disconnecting {1} peers.",
                        validationContext.ChainedHeaderToValidate, peerIdsToBan.Count);

                    foreach (var peerId in peerIdsToBan)
                        if (this.peersByPeerId.TryGetValue(peerId, out var peer))
                            peersToBan.Add(peer);
                }

                foreach (var peer in peersToBan)
                    this.peerBanning.BanAndDisconnectPeer(peer.RemoteSocketEndpoint,
                        validationContext.BanDurationSeconds,
                        $"Invalid block received: {validationContext.Error.Message}");
            }
        }
        ...
        async Task OnPartialValidationSucceededAsync(ChainedHeader chainedHeader)
        {
            using (this.performanceCounter.MeasureTotalConnectionTime())
            {
                ...
                using (await this.reorgLock.LockAsync().ConfigureAwait(false))
                {
                    ...
                    lock (this.peerLock)
                    {
                        chainedHeaderBlocksToValidate =
                            this.chainedHeaderTree.PartialValidationSucceeded(chainedHeader,
                                out fullValidationRequired);
                    }

                    this.logger.LogDebug("Full validation is{0} required.",
                        fullValidationRequired ? string.Empty : " NOT");
                ...
                if (connectBlocksResult != null)
                {
                    if (connectBlocksResult.PeersToBan != null)
                    {
                        ...
                        lock (this.peerLock)
                        {
                            foreach (var peerId in connectBlocksResult.PeersToBan)
                                if (this.peersByPeerId.TryGetValue(peerId, out var peer))
                                    peersToBan.Add(peer);
                        }
                        ...
                        foreach (var peer in peersToBan)
                            this.peerBanning.BanAndDisconnectPeer(peer.PeerEndPoint,
                                connectBlocksResult.BanDurationSeconds, connectBlocksResult.BanReason);
                    }
                ...
                if (chainedHeaderBlocksToValidate != null)
                    ...
                    {
                        this.logger.LogDebug("Partial validation of {0} block will be started.",
                            chainedHeaderBlocksToValidate.Count);
                        ...
                        foreach (var toValidate in chainedHeaderBlocksToValidate)
                            this.partialValidator.StartPartialValidation(toValidate.ChainedHeader, toValidate.Block,
                                OnPartialValidationCompletedCallbackAsync);
                    }
            }
        }
        ...
        /// </summary>
        async Task NotifyBehaviorsOnConsensusTipChangedAsync()
        {
            var behaviors = new List<ConsensusManagerBehavior>();

            lock (this.peerLock)
            {
                foreach (var peer in this.peersByPeerId.Values)
                    behaviors.Add(peer.Behavior<ConsensusManagerBehavior>());
            }

            var blocksToDownload = new List<ConnectNewHeadersResult>();

            foreach (var consensusManagerBehavior in behaviors)
            {
                var connectNewHeadersResult =
                    await consensusManagerBehavior.ConsensusTipChangedAsync().ConfigureAwait(false);

                var peerId = consensusManagerBehavior.AttachedPeer?.Connection?.Id;
                ...
                this.logger.LogDebug("{0} headers were added to download by peer ID {1}.",
                    connectNewHeadersResult.DownloadTo.Height - connectNewHeadersResult.DownloadFrom.Height + 1,
                    peerId);
            }

            foreach (var newHeaders in blocksToDownload)
        ...
        /// <remarks>
        ///     It is possible that during connection we find out that blocks that we tried to connect are invalid and we switch
        ...
        async Task<ConnectBlocksResult> FullyValidateLockedAsync(ChainedHeader newTip, bool blockMined = false)
        {
            var oldTip = this.Tip;

            var fork = oldTip.FindFork(newTip);
            ...
            var blocksToConnect = TryGetBlocksToConnect(newTip, fork.Height + 1);
            ...
            var connectBlockResult = await ConnectChainAsync(blocksToConnect, blockMined).ConfigureAwait(false);
            ...
            if (connectBlockResult.Succeeded)
            {
                if (!isExtension)
                    ...
                    foreach (var disconnectedBlock in disconnectedBlocks)
                    {
            ...
            var reconnectionResult = await ReconnectOldChainAsync(disconnectedBlocks).ConfigureAwait(false);
        ...
        ///     Thrown in case <paramref name="fork" /> is ahead of the <paramref name="oldTip" />
        ///     .
        /// </exception>
        /// <returns>List of blocks that were disconnected.</returns>
        async Task<List<ChainedHeaderBlock>> RewindToForkPointAsync(ChainedHeader fork, ChainedHeader oldTip)
        {
        ...
        /// <param name="blockMined">Was the block mined or received from the network.</param>
        async Task<ConnectBlocksResult> ConnectChainAsync(List<ChainedHeaderBlock> blocksToConnect,
            bool blockMined = false)
        {
            ChainedHeader lastValidatedBlockHeader = null;
            ConnectBlocksResult connectBlockResult = null;

            foreach (var blockToConnect in blocksToConnect)
            {
                using (this.performanceCounter.MeasureBlockConnectionFV())
                {
                    ...
                    {
                        var newFinalizedHeight = blockToConnect.ChainedHeader.Height -
                                                 (int) this.network.Consensus.MaxReorgLength;

                        if (newFinalizedHeight > 0)
                        {
                            var newFinalizedHash =
                                blockToConnect.ChainedHeader.GetAncestor(newFinalizedHeight).HashBlock;

                            this.finalizedBlockInfo.SaveFinalizedBlockHashAndHeight(newFinalizedHash,
                                newFinalizedHeight);
                        }
                    }
                }
            ...
            var peersToResync = SetConsensusTip(lastValidatedBlockHeader, blockMined);
        ...
        async Task<ConnectBlocksResult> ReconnectOldChainAsync(List<ChainedHeaderBlock> blocksToReconnect)
        {
            // Connect back the old blocks.
            var connectBlockResult = await ConnectChainAsync(blocksToReconnect).ConfigureAwait(false);
            if (connectBlockResult.Succeeded)
            {
        ...
        async Task ResyncPeersAsync(List<int> peerIds)
        {
            ...
            lock (this.peerLock)
            {
                foreach (var peerId in peerIds)
                {
                    if (this.peersByPeerId.TryGetValue(peerId, out var peer))
                    {
                        ...
                        var task = peer.Behavior<ConsensusManagerBehavior>().ResetPeerTipInformationAndSyncAsync();
        ...
        async Task<ConnectBlocksResult> ConnectBlockAsync(ChainedHeaderBlock blockToConnect)
        {
            ...
            {
                this.logger.LogError("Block '{0}' must be partially or fully validated but it is {1}.", blockToConnect,
                    blockToConnect.ChainedHeader.BlockValidationState);
            ...
            var validationContext = await this.fullValidator
                .ValidateAsync(blockToConnect.ChainedHeader, blockToConnect.Block).ConfigureAwait(false);
            ...
            if (validationContext.Error != null)
            {
                ...
                {
                    failureResult.BanDurationSeconds =
                        (int) (validationContext.RejectUntil.Value.ToUnixTimestamp() -
                               this.dateTimeProvider.GetAdjustedTimeAsUnixTimestamp());
        ...
        /// <returns>
        ///     Collection of blocks that were loaded. In case at least one block was not present <c>null</c> will be
        ///     returned.
        /// </returns>
        List<ChainedHeaderBlock> TryGetBlocksToConnect(ChainedHeader proposedNewTip, int heightOfFirstBlock)
        {
            var currentHeader = proposedNewTip;
            var chainedHeaderBlocks = new List<ChainedHeaderBlock>();

            while (currentHeader.Height >= heightOfFirstBlock)
            {
                var chainedHeaderBlock = GetBlockData(currentHeader.HashBlock);
        ...
        /// <param name="blockMined">Was the block mined or received from the network.</param>
        List<int> SetConsensusTip(ChainedHeader newTip, bool blockMined = false)
        {
            lock (this.peerLock)
            {
                var peerIdsToResync = this.chainedHeaderTree.ConsensusTipChanged(newTip, blockMined);

                SetConsensusTipInternalLocked(newTip);

                var ibd = this.ibdState.IsInitialBlockDownload();
        ...
        /// <remarks>Have to be locked by <see cref="peerLock" />.</remarks>
        /// <param name="newTip">New consensus tip.</param>
        void SetConsensusTipInternalLocked(ChainedHeader newTip)
        {
            this.Tip = newTip;
        ...
        ///     If <paramref name="chainedHeaders" /> is not an array of consecutive headers it will be split to batches of
        ///     consecutive header requests.
        ///     Callbacks of all entries are added to <see cref="callbacksByBlocksRequestedHash" />. If a block header was already
        ///     requested
        ///     to download and not delivered yet, it will not be requested again, instead just it's callback will be called when
        ///     the block arrives.
        /// </summary>
        ...
        void DownloadBlocks(ChainedHeader[] chainedHeaders, OnBlockDownloadedCallback onBlockDownloadedCallback = null)
        {
            ...
            lock (this.blockRequestedLock)
            {
                ...
                {
                    var blockAlreadyAsked =
                        this.callbacksByBlocksRequestedHash.TryGetValue(chainedHeader.HashBlock,
                            out var downloadedCallbacks);
                    ...
                    var blockIsNotConsecutive = previousHeader != null &&
                                                chainedHeader.Previous.HashBlock != previousHeader.HashBlock;
        ...
        void BlockDownloaded(uint256 blockHash, Block block, int peerId)
        {
            if (this.nodeLifetime.ApplicationStopping.IsCancellationRequested)
            {
                this.logger.LogTrace("(-)[NODE_DISPOSED]");
            ...
            lock (this.peerLock)
            {
                if (this.expectedBlockSizes.TryGetValue(blockHash, out expectedSize))
                {
                    ...
                    this.logger.LogDebug(
                        "Expected block data bytes was set to {0} and we are expecting {1} blocks to be delivered.",
                        this.expectedBlockDataBytes, this.expectedBlockSizes.Count);
                }
                else
                {
                ...
                if (block == null)
                {
                    ...
                    {
                        this.logger.LogDebug("Block download failed but will not be reassigned as it's state is {0}",
                            chainedHeader.BlockDataAvailability);
                    }
                }
            }
            ...
            if (block != null)
            {
                var result = this.integrityValidator.VerifyBlockIntegrity(chainedHeader, block);
                ...
                if (result.Error != null)
                {
                    ...
                    if (this.peersByPeerId.TryGetValue(peerId, out var peer))
                        this.peerBanning.BanAndDisconnectPeer(peer.PeerEndPoint, result.BanDurationSeconds,
            ...
            if (downloadedCallbacks != null)
            {
                ...
                if (downloadedCallbacks.ConsensusRequested) ProcessDownloadedBlock(chainedHeaderBlock);

                if (downloadedCallbacks.Callbacks != null)
                {
                    this.logger.LogDebug("Calling {0} callbacks for block '{1}'.", downloadedCallbacks.Callbacks.Count,
                        chainedHeader);
                    foreach (var blockDownloadedCallback in downloadedCallbacks.Callbacks)
        ...
        ///     The amount of blocks in 1 batch to downloaded depends on the average value in
        ///     <see cref="IBlockPuller.GetAverageBlockSizeBytes" />.
        ///     Should be protected by the <see cref="peerLock" />.
        /// </remarks>
        void ProcessDownloadQueueLocked()
        {
            while (this.toDownloadQueue.Count > 0)
            {
                var awaitingBlocksCount = this.expectedBlockSizes.Count;

                var freeSlots = MaxBlocksToAskFromPuller - awaitingBlocksCount;
                ...
                var freeBytes = this.maxUnconsumedBlocksDataBytes - this.chainedHeaderTree.UnconsumedBlocksDataBytes -
                                this.expectedBlockDataBytes;
                ...
                var avgSize = (long) this.blockPuller.GetAverageBlockSizeBytes();
                if (avgSize == 0) avgSize = this.network.Consensus.Options.MaxBlockBaseSize;

                var maxBlocksToAsk = Math.Min((int) (freeBytes / avgSize), freeSlots);

                this.logger.LogDebug("With {0} average block size, we have {1} download slots available.", avgSize,
                    maxBlocksToAsk);

                var request = this.toDownloadQueue.Peek();
                ...
                {
                    this.logger.LogDebug("Splitting enqueued job of size {0} into 2 pieces of sizes {1} and {2}.",
                        request.BlocksToDownload.Count, maxBlocksToAsk,
                        request.BlocksToDownload.Count - maxBlocksToAsk);
                ...
                foreach (var chainedHeader in request.BlocksToDownload)
        ...
        bool IsConsensusConsideredToBeSyncedLocked(out ChainedHeader bestTip)
        {
            ...
            var isConsideredSynced = this.Tip.Height + ConsensusIsConsideredToBeSyncedMargin > bestTip.Height;
        ...
        void AddInlineStats(StringBuilder log)
        {
            lock (this.peerLock)
            {
                var bestTip = this.chainedHeaderTree.GetBestPeerTip();

                if (bestTip == null || bestTip.Height < this.Tip.Height)
                    bestTip = this.Tip;

                var headersLog = "Headers.Height: ".PadRight(LoggingConfiguration.ColumnLength + 1) +
                                 bestTip.Height.ToString().PadRight(8) +
                                 " Headers.Hash: ".PadRight(LoggingConfiguration.ColumnLength - 1) + bestTip.HashBlock;

                log.AppendLine(headersLog);
            }

            var consensusLog = "Consensus.Height: ".PadRight(LoggingConfiguration.ColumnLength + 1) +
                               this.Tip.Height.ToString().PadRight(8) +
        ...
        void AddBenchStats(StringBuilder benchLog)
        {
        ...
        void AddComponentStats(StringBuilder log)
        {
            ...
            lock (this.peerLock)
            {
                ...
                var currentTime = this.dateTimeProvider.GetTime();
                var tipAge = currentTime - this.chainState.ConsensusTip.Header.BlockTime.ToUnixTimeSeconds();
                long maxTipAge = this.consensusSettings.MaxTipAge;

                log.AppendLine(
                    $"Tip Age: {TimeSpan.FromSeconds(tipAge).ToString(@"dd\.hh\:mm\:ss")} (maximum is {TimeSpan.FromSeconds(maxTipAge).ToString(@"dd\.hh\:mm\:ss")})");
                log.AppendLine($"In IBD Stage: {(this.isIbd ? "Yes" : "No")}");

                log.AppendLine(
                    $"Chained header tree size: {this.chainedHeaderTree.ChainedBlocksDataBytes.BytesToMegaBytes()} MB");

                var unconsumedBlocks = FormatBigNumber(this.chainedHeaderTree.UnconsumedBlocksCount);

                var filledPercentage =
                    Math.Round(
                        this.chainedHeaderTree.UnconsumedBlocksDataBytes / (double) this.maxUnconsumedBlocksDataBytes *
                        100, 2);

                log.AppendLine(
                    $"Unconsumed blocks: {unconsumedBlocks} -- ({this.chainedHeaderTree.UnconsumedBlocksDataBytes.BytesToMegaBytes()} / {this.maxUnconsumedBlocksDataBytes.BytesToMegaBytes()} MB). Cache is filled by: {filledPercentage}%");

                var pendingDownloadCount = this.callbacksByBlocksRequestedHash.Count;
                var currentlyDownloadingCount = this.expectedBlockSizes.Count;

                log.AppendLine(
                    $"Downloading blocks: {currentlyDownloadingCount} queued out of {pendingDownloadCount} pending");
            }
        }

        /// <summary>Formats the big number.</summary>
        /// <remarks><c>123456789</c> => <c>123,456,789</c>.</remarks>
        string FormatBigNumber(long number)
        {
            return $"{number:#,##0}";
        }
        ...
        {
            /// <summary>
            ///     The consensus code has requested this block, invoke the method <see cref="ProcessDownloadedBlock" /> when
            ///     block is delivered.
            /// </summary>
