...
namespace UnnamedCoin.Bitcoin.Consensus.PerformanceCounters.ConsensusManager
{
    ...
    public class ConsensusManagerPerformanceSnapshot
    {
        ...
        public override string ToString()
        {
            ...
            builder.AppendLine(
                $"Total connection time (FV, CHT upd, Rewind, Signaling): {this.TotalConnectionTime.GetAvgExecutionTimeMs()} ms");

            builder.AppendLine(
                $"Block connection (FV excluding rewind): {this.ConnectBlockFV.GetAvgExecutionTimeMs()} ms");
    ...
    public class ExecutionsCountAndDelay
    {
        ...
        {
            return Math.Round(TimeSpan.FromTicks(this.totalDelayTicks).TotalMilliseconds / this.totalExecutionsCount,
                4);
        }
