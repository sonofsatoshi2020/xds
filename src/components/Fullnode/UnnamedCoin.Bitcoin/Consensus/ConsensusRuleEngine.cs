...
using UnnamedCoin.Bitcoin.Consensus.PerformanceCounters.Rules;
...
namespace UnnamedCoin.Bitcoin.Consensus
{
    ...
    {
        readonly ConsensusRulesContainer consensusRules;

        /// <inheritdoc cref="IInvalidBlockHashStore" />
        readonly IInvalidBlockHashStore invalidBlockHashStore;

        /// <summary>Instance logger.</summary>
        readonly ILogger logger;

        /// <inheritdoc cref="ConsensusRulesPerformanceCounter" />
        ConsensusRulesPerformanceCounter performanceCounter;
        ...
        public virtual ValidationContext HeaderValidation(ChainedHeader header)
        {
            ...
            var ruleContext = CreateRuleContext(validationContext);
        ...
        public virtual async Task<ValidationContext> FullValidationAsync(ChainedHeader header, Block block)
        {
            ...
            {
                this.logger.LogWarning("Block '{0}' failed full validation with error: '{1}'.", header,
        ...
        public virtual async Task<ValidationContext> PartialValidationAsync(ChainedHeader header, Block block)
        {
            ...
            if (validationContext.Error != null)
            {
                this.logger.LogWarning("Block '{0}' failed partial validation with error: '{1}'.", header,
                    validationContext.Error);
                HandleConsensusError(validationContext);
            }
        ...
        void SetupConsensusRules(IEnumerable<ConsensusRuleBase> rules)
        {
        ...
        async Task ExecuteRulesAsync(IEnumerable<AsyncConsensusRule> asyncRules, RuleContext ruleContext)
        {
            try
            {
        ...
        /// <summary>
        ///     Adds block hash to a list of failed header unless specific consensus error was used that doesn't require block
        ///     banning.
        /// </summary>
        void HandleConsensusError(ValidationContext validationContext)
        {
            ...
            var hashToBan = validationContext.ChainedHeaderToValidate.HashBlock;
        ...
        void ExecuteRules(IEnumerable<SyncConsensusRule> rules, RuleContext ruleContext)
        {
            try
            {
                ruleContext.SkipValidation = ruleContext.ValidationContext.ChainedHeaderToValidate.IsAssumedValid;
