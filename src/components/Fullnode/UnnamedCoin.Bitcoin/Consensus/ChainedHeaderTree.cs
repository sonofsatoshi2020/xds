...
namespace UnnamedCoin.Bitcoin.Consensus
{
    ...
    /// </summary>
    /// <remarks>
    ///     This component is an extension of <see cref="ConsensusManager" /> and is strongly linked to its functionality, it
    ///     should never be called outside of CM.
    ///     <para>
    ...
    ///     </para>
    ///     <para>
    ///         It is a role of the <see cref="ConsensusManager" /> to decide which of the presented chains is going to be
    ///         treated as our best chain.
    ...
    interface IChainedHeaderTree
    {
        ...
        /// </summary>
        /// <remarks>
        ///     In case partial validation was successful we want to partially validate all the next blocks for which we have block
        ///     data for.
        ///     <para>
        ///         If block that was just partially validated has more cumulative chainwork than our consensus tip we want to
        ///         switch our consensus tip to this block.
        ///     </para>
        /// </remarks>
        /// <param name="chainedHeader">The chained header.</param>
        /// <param name="fullValidationRequired">
        ///     <c>true</c> in case we want to switch our consensus tip to
        ///     <paramref name="chainedHeader" />.
        /// </param>
        /// <returns>
        ///     List of chained header blocks with block data that should be partially validated next. Or <c>null</c> if none
        ///     should be validated.
        /// </returns>
        List<ChainedHeaderBlock> PartialValidationSucceeded(ChainedHeader chainedHeader,
            out bool fullValidationRequired);
        ...
        ///     Blocks associated with headers that are interesting (i.e. represent a chain with greater chainwork than our
        ///     consensus tip)
        ...
        /// <returns>
        ///     Information about which blocks need to be downloaded together with information about which input headers were
        ///     processed.
        ///     Only headers that we can validate will be processed. The rest of the headers will be submitted later again for
        ///     processing.
        /// </returns>
        /// <exception cref="ConnectHeaderException">
        ///     Thrown when first presented header can't be connected to any known chain in
        ///     the tree.
        /// </exception>
        ...
        ///     If the header is not in the tree <see cref="ChainedHeaderBlock" /> will be <c>null</c>, the
        ///     <see cref="ChainedHeaderBlock.Block" /> may also be null.
        /// </summary>
        /// <remarks>
        ///     The block can be <c>null</c> when the block data has not yet been downloaded or if the block data has been
        ///     persisted to the database and removed from the memory.
        /// </remarks>
        /// <returns>
        ///     The block and its chained header (the <see cref="ChainedHeaderBlock.Block" /> can be <c>null</c> or the
        ///     <see cref="ChainedHeaderBlock" /> result can be <c>null</c>).
        /// </returns>
    ...
    /// <inheritdoc />
    class ChainedHeaderTree : IChainedHeaderTree
    {
        ...
        /// </summary>
        readonly Dictionary<uint256, ChainedHeader> chainedHeadersByHash;

        readonly IChainState chainState;
        readonly ICheckpoints checkpoints;
        readonly ConsensusSettings consensusSettings;
        readonly IFinalizedBlockInfoRepository finalizedBlockInfo;
        readonly IHeaderValidator headerValidator;
        readonly IInvalidBlockHashStore invalidHashesStore;
        ...
        /// <remarks>
        ///     During the consensus tip changing process, which includes both the reorganization and advancement on the same
        ///     chain,
        ///     it happens that there are two entries for <see cref="LocalPeerId" />. This means that two different blocks are
        ///     being
        ...
        ///     </para>
        /// </remarks>
        readonly Dictionary<uint256, HashSet<int>> peerIdsByTipHash;

        /// <summary>A list of peer identifiers that are mapped to their tips.</summary>
        readonly Dictionary<int, uint256> peerTipsByPeerId;
        ...
        public ChainedHeaderBlock GetChainedHeaderBlock(uint256 blockHash)
        {
            ...
            if (this.chainedHeadersByHash.TryGetValue(blockHash, out var chainedHeader))
            {
        ...
        public ChainedHeaderBlock[] GetChainedHeaderBlocks(List<uint256> blockHashes)
        {
            ...
            foreach (var key in blockHashes)
                if (this.chainedHeadersByHash.TryGetValue(key, out var chainedHeader))
        ...
        {
            if (this.chainedHeadersByHash.TryGetValue(blockHash, out var chainedHeader)) return chainedHeader;
        ...
        {
            if (!this.peerTipsByPeerId.TryGetValue(networkPeerId, out var peerTipHash))
            {
            ...
            {
                this.logger.LogError("Header '{0}' not found but it is claimed by {1} as its tip.", peerTipHash,
                    networkPeerId);
        ...
        /// <inheritdoc />
        public List<ChainedHeaderBlock> PartialValidationSucceeded(ChainedHeader chainedHeader,
            out bool fullValidationRequired)
        {
            ...
            if (chainedHeader.ChainWork > GetConsensusTip().ChainWork)
            {
                ...
                this.logger.LogDebug(
                    "Partially validated chained header '{0}' has more work than the current consensus tip.",
                    chainedHeader);
            ...
            foreach (var header in chainedHeader.Next)
        ...
        public List<int> PartialOrFullValidationFailed(ChainedHeader chainedHeader)
        {
            ...
            var peersToBan = RemoveSubtree(chainedHeader);
        ...
        {
            var oldConsensusTip = GetConsensusTip();
            var fork = newConsensusTip.FindFork(oldConsensusTip);
            var currentHeader = newConsensusTip;

            this.logger.LogDebug("Old consensus tip: '{0}', new consensus tip: '{1}', fork point: '{2}'.",
                oldConsensusTip, newConsensusTip, fork);
            ...
            while (newConsensusTip.Block != null && currentHeader != fork && !blockMined)
            {
                ...
                this.logger.LogDebug(
                    "UnconsumedBlocks decreased by block {0} (byte: {1}), new count: {2}, new size: {3}", currentHeader,
                    currentHeader.Block.BlockSize.Value, this.UnconsumedBlocksCount, this.UnconsumedBlocksDataBytes);
            ...
            var peerIdsToResync = FindPeersToResync(newConsensusTip);
        ...
        public bool BlockDataDownloaded(ChainedHeader chainedHeader, Block block)
        {
            ...
            this.logger.LogDebug("UnconsumedBlocks increased by block {0} (byte: {1}), new count: {2}, new size: {3}",
                chainedHeader, chainedHeader.Block.BlockSize.Value, this.UnconsumedBlocksCount,
                this.UnconsumedBlocksDataBytes);

            var partialValidationRequired =
                chainedHeader.Previous.BlockValidationState == ValidationState.PartiallyValidated
                || chainedHeader.Previous.BlockValidationState == ValidationState.FullyValidated;

            this.logger.LogDebug("[BLOCK_DOWNLOAD_PREVIOUS_STATE]{0}.{1}:{2}", nameof(chainedHeader),
                nameof(chainedHeader.Previous), chainedHeader.Previous);
        ...
        public ConnectNewHeadersResult ConnectNewHeaders(int networkPeerId, List<BlockHeader> headers)
        {
            ...
            var lastHash = headers.Last().GetHash();
            ...
            var newChainedHeaders = CreateNewHeaders(headers, out var insufficientInfo);

            if (insufficientInfo)
            {
                if (TryFindLastValidatedHeader(headers, out var lastAlreadyValidatedHeader))
                {
            ...
            var earliestNewHeader = newChainedHeaders.First();
            var latestNewHeader = newChainedHeaders.Last();

            AddOrReplacePeerTip(networkPeerId, latestNewHeader.HashBlock);

            ConnectNewHeadersResult connectNewHeadersResult = null;

            var isAssumedValidEnabled = this.consensusSettings.BlockAssumedValid != null;
            var isBelowLastCheckpoint = this.consensusSettings.UseCheckpoints &&
                                        earliestNewHeader.Height <= this.checkpoints.GetLastCheckpointHeight();

            if (isBelowLastCheckpoint || isAssumedValidEnabled)
            {
                var currentChainedHeader = latestNewHeader;
                ...
                while (currentChainedHeader != earliestNewHeader.Previous)
                {
                    ...
                    {
                        this.logger.LogDebug("Chained header '{0}' represents an assumed valid block.",
                            currentChainedHeader);

                        var assumeValidBelowLastCheckpoint =
                            this.consensusSettings.UseCheckpoints && currentChainedHeader.Height <=
                            this.checkpoints.GetLastCheckpointHeight();
                        connectNewHeadersResult = HandleAssumedValidHeader(currentChainedHeader, latestNewHeader,
                            assumeValidBelowLastCheckpoint);
                        break;
                    }

                    var checkpoint = this.checkpoints.GetCheckpoint(currentChainedHeader.Height);
                    if (checkpoint != null)
                    {
                        this.logger.LogDebug("Chained header '{0}' is a checkpoint.", currentChainedHeader);

                        connectNewHeadersResult = HandleCheckpointsHeader(currentChainedHeader, latestNewHeader,
                            checkpoint, networkPeerId);
                        break;
                    }
            ...
            {
                this.logger.LogDebug(
                    "Chained header '{0}' is the tip of a chain with more work than our current consensus tip.",
                    latestNewHeader);
        ...
        {
            var headerToBeCreated = block.Header;
            if (block is PosBlock posBlock)
                headerToBeCreated =
                    ((PosConsensusFactory) this.network.Consensus.ConsensusFactory).CreateProvenBlockHeader(posBlock);

            CreateNewHeaders(new List<BlockHeader> {headerToBeCreated}, out _);

            var chainedHeader = GetChainedHeader(block.GetHash());
        ...
        public ChainedHeader GetBestPeerTip()
        {
            ...
            foreach (var idTipHashPair in this.peerTipsByPeerId)
            {
                if (idTipHashPair.Key == LocalPeerId)
                    continue;

                var tip = this.chainedHeadersByHash[idTipHashPair.Value];
        ...
        public void BlockRewinded(ChainedHeaderBlock disconnectedBlock)
        {
            ...
            this.logger.LogDebug("UnconsumedBlocks increased by block {0} (byte: {1}), new count: {2}, new size: {3}",
                disconnectedBlock.ChainedHeader, disconnectedBlock.Block.BlockSize.Value, this.UnconsumedBlocksCount,
                this.UnconsumedBlocksDataBytes);
        }

        /// <summary>Gets the consensus tip.</summary>
        ChainedHeader GetConsensusTip()
        {
            var consensusTipHash = this.peerTipsByPeerId[LocalPeerId];
        ...
        void ClaimPeerTip(int networkPeerId, uint256 tipHash)
        {
        ...
        /// <summary>
        ///     Checks each peer's tip if it violates max reorg rule.
        ///     Peers that violate it must be resynced.
        /// </summary>
        /// <param name="consensusTip">Consensus tip.</param>
        /// <returns>List of peers which tips violate max reorg rule.</returns>
        List<int> FindPeersToResync(ChainedHeader consensusTip)
        {
            var peerIdsToResync = new List<int>();
            var maxReorgLength = this.chainState.MaxReorgLength;

            // Find peers with chains that now violate max reorg.
            if (maxReorgLength != 0)
                foreach (var peerIdToTipHash in this.peerTipsByPeerId)
                {
                    var peerTip = this.chainedHeadersByHash[peerIdToTipHash.Value];
                    var peerId = peerIdToTipHash.Key;

                    var fork = FindForkIfChainedHeadersNotOnSameChain(peerTip, consensusTip);

                    var finalizedHeight = this.finalizedBlockInfo.GetFinalizedBlockInfo().Height;
                    ...
                    if (fork != null && fork.Height < finalizedHeight)
                    {
                        ...
                        this.logger.LogDebug(
                            "Peer with Id {0} claims a chain that violates max reorg, its tip is '{1}' and the last finalized block height is {2}.",
                            peerId, peerTip, finalizedHeight);
                    }
                }
        ...
        ChainedHeader FindForkIfChainedHeadersNotOnSameChain(ChainedHeader chainedHeader1, ChainedHeader chainedHeader2)
        {
            var fork = chainedHeader1.FindFork(chainedHeader2);

            if (fork != chainedHeader2 && fork != chainedHeader1) return fork;

            return null;
        }

        /// <summary>
        ///     Cleans the block data for chained headers that are old. This data will still exist in the block store if it is
        ///     enabled.
        /// </summary>
        /// <param name="consensusTip">Consensus tip.</param>
        void CleanOldBlockDataFromMemory(ChainedHeader consensusTip)
        {
            var lastBlockHeightToKeep = consensusTip.Height - KeepBlockDataForLastBlocks;
            ...
            var currentBlockToDeleteData = consensusTip.GetAncestor(lastBlockHeightToKeep).Previous;
            ...
            while (currentBlockToDeleteData.Block != null && currentBlockToDeleteData.Previous != null)
            {
                ...
                this.logger.LogDebug("Block data for '{0}' was removed from memory, block data availability is {1}.",
                    currentBlockToDeleteData, currentBlockToDeleteData.BlockDataAvailability);
        ...
        List<int> RemoveSubtree(ChainedHeader subtreeRoot)
        {
            ...
            {
                var header = headersToProcess.Pop();

                foreach (var nextHeader in header.Next)
                    headersToProcess.Push(nextHeader);

                if (this.peerIdsByTipHash.TryGetValue(header.HashBlock, out var peers))
                {
        ...
        void DisconnectChainHeader(ChainedHeader header)
        {
            ...
            if (header.Block != null)
            {
                ...
                this.logger.LogDebug(
                    "UnconsumedBlocks decreased by block {0} (byte: {1}), new count: {2}, new size: {3}", header,
                    header.Block.BlockSize.Value, this.UnconsumedBlocksCount, this.UnconsumedBlocksDataBytes);
            }
        }
        ...
        ConnectNewHeadersResult MarkBetterChainAsRequired(ChainedHeader lastRequiredHeader, ChainedHeader lastNewHeader)
        {
            ...
            var current = lastRequiredHeader;
            var next = current;
        ...
        void MarkTrustedChainAsAssumedValid(ChainedHeader chainedHeader)
        {
            var current = chainedHeader;
        ...
        ///     If the header's cumulative work is better then <see cref="IChainState.ConsensusTip" /> the header and all its
        ///     predecessors will be marked with <see cref="BlockDataAvailabilityState.BlockRequired" />.
        /// </summary>
        ...
        /// <param name="isBelowLastCheckpoint">
        ///     Set to <c>true</c> if <paramref name="assumedValidHeader" /> is below the last checkpoint,
        ///     <c>false</c> otherwise or if checkpoints are disabled.
        /// </param>
        ConnectNewHeadersResult HandleAssumedValidHeader(ChainedHeader assumedValidHeader,
            ChainedHeader latestNewHeader, bool isBelowLastCheckpoint)
        {
            var bestTip = GetConsensusTip();
            ...
            if (latestNewHeader.ChainWork > bestTip.ChainWork)
            {
                ...
                var latestHeaderToMark = isBelowLastCheckpoint ? assumedValidHeader : latestNewHeader;
        ...
        /// <exception cref="CheckpointMismatchException">Thrown if checkpointed header doesn't match the checkpoint hash.</exception>
        ConnectNewHeadersResult HandleCheckpointsHeader(ChainedHeader chainedHeader, ChainedHeader latestNewHeader,
            CheckpointInfo checkpoint, int peerId)
        {
            if (checkpoint.Hash != chainedHeader.HashBlock)
            {
                ...
                this.logger.LogDebug("Chained header '{0}' does not match checkpoint '{1}'.", chainedHeader,
                    checkpoint.Hash);
                this.logger.LogTrace("(-)[INVALID_HEADER_NOT_MATCHING_CHECKPOINT]");
                throw new CheckpointMismatchException();
            }

            var subchainTip = chainedHeader;
            if (chainedHeader.Height == this.checkpoints.GetLastCheckpointHeight())
                subchainTip = latestNewHeader;

            var connectNewHeadersResult = MarkBetterChainAsRequired(subchainTip, latestNewHeader);
        ...
        /// </summary>
        bool HeaderWasRequested(ChainedHeader chainedHeader)
        {
        ...
        /// <summary>
        ///     Check whether a header is in one of the following states: <see cref="ValidationState.PartiallyValidated" />,
        ///     <see cref="ValidationState.FullyValidated" />
        ///     or is assumed to be valid.
        /// </summary>
        bool HeaderWasMarkedAsValidated(ChainedHeader chainedHeader)
        {
        ...
        void RemoveUnclaimedBranch(ChainedHeader chainedHeader)
        {
            var currentHeader = chainedHeader;
            while (currentHeader.Previous != null)
            {
                // If current header is an ancestor of some other tip claimed by a peer, do nothing.
                var headerHasDecendents = currentHeader.Next.Count > 0;
                ...
                var headerIsClaimedByPeer = this.peerIdsByTipHash.ContainsKey(currentHeader.HashBlock);
        ...
        void RemovePeerClaim(int networkPeerId, ChainedHeader chainedHeader)
        {
            ...
            this.logger.LogDebug("Tip claim of peer ID {0} removed from chained header '{1}'.", networkPeerId,
                chainedHeader);
        ...
        void AddOrReplacePeerTip(int networkPeerId, uint256 newTip)
        {
            var oldTipHash = this.peerTipsByPeerId.TryGet(networkPeerId);
            ...
            if (oldTipHash != null)
            {
                var oldTip = this.chainedHeadersByHash.TryGet(oldTipHash);
        ...
        /// <param name="insufficientInfo">
        ///     <c>true</c> if there wasn't enough data to validate even the first header of all
        ///     presented.
        /// </param>
        ...
        /// <exception cref="ConsensusErrorException">Thrown if header validation failed.</exception>
        List<ChainedHeader> CreateNewHeaders(List<BlockHeader> headers, out bool insufficientInfo)
        {
            insufficientInfo = false;

            if (!TryFindNewHeaderIndex(headers, out var newHeaderIndex))
            {
            ...
            {
                this.logger.LogDebug("Previous hash '{0}' of block hash '{1}' was not found.",
                    headers[newHeaderIndex].GetHash(), headers[newHeaderIndex].HashPrevBlock);
            ...
            var newChainedHeader =
                CreateAndValidateNewChainedHeader(headers[newHeaderIndex], previousChainedHeader, out insufficientInfo);
            ...
            if (newChainedHeader != null)
            {
                ...
                try
                {
                    ...
                    {
                        newChainedHeader =
                            CreateAndValidateNewChainedHeader(headers[newHeaderIndex], previousChainedHeader, out _);
        ...
        /// <exception cref="ConsensusErrorException">Thrown if header validation failed.</exception>
        ChainedHeader CreateAndValidateNewChainedHeader(BlockHeader currentBlockHeader,
            ChainedHeader previousChainedHeader, out bool insufficientHeaderInformation)
        {
            var newHeaderHash = currentBlockHeader.GetHash();
            ...
            var result = this.headerValidator.ValidateHeader(newChainedHeader);
        ...
        /// <summary>
        ///     Find the first header in the given list of <see cref="headers" /> that does not exist in
        ///     <see cref="chainedHeadersByHash" />.
        /// </summary>
        bool TryFindNewHeaderIndex(List<BlockHeader> headers, out int newHeaderIndex)
        {
            for (newHeaderIndex = 0; newHeaderIndex < headers.Count; newHeaderIndex++)
            {
                var currentBlockHash = headers[newHeaderIndex].GetHash();
                if (!this.chainedHeadersByHash.ContainsKey(currentBlockHash))
                {
                    this.logger.LogDebug("A new header with hash '{0}' was found that is not connected to the tree.",
                        currentBlockHash);
                    return true;
                }
            }
        ...
        bool TryFindLastValidatedHeader(List<BlockHeader> headers, out ChainedHeader lastValidatedHeader)
        {
            lastValidatedHeader = null;
            foreach (var header in headers)
            {
                var currentBlockHash = header.GetHash();
                if (this.chainedHeadersByHash.TryGetValue(currentBlockHash, out var currentChainedHeader))
        ...
        /// <summary>
        ///     Checks if switching to specified <paramref name="chainedHeader" /> would require rewinding consensus behind the
        ///     finalized block height.
        /// </summary>
        /// <param name="chainedHeader">The header that needs to be checked for reorg.</param>
        /// <exception cref="MaxReorgViolationException">Thrown in case maximum reorganization rule is violated.</exception>
        void CheckMaxReorgRuleViolated(ChainedHeader chainedHeader)
        {
            var maxReorgLength = this.chainState.MaxReorgLength;
            var consensusTip = GetConsensusTip();
            if (maxReorgLength != 0)
            {
                var fork = chainedHeader.FindFork(consensusTip);
                ...
                {
                    var reorgLength = consensusTip.Height - fork.Height;
                    ...
                    {
                        this.logger.LogDebug(
                            "Reorganization of length {0} prevented, maximal reorganization length is {1}, consensus tip is '{2}' and the last finalized block height is {3}.",
                            reorgLength, maxReorgLength, consensusTip, finalizedHeight);
                        this.logger.LogTrace("(-)[MAX_REORG_VIOLATION]");
                        throw new MaxReorgViolationException();
                    }

                    this.logger.LogDebug("Reorganization of length {0} accepted, consensus tip is '{1}'.", reorgLength,
                        consensusTip);
                }
            }
        }
    ...
    public class ConnectNewHeadersResult
    {
        ...
        {
            return
                $"{nameof(this.DownloadFrom)}='{this.DownloadFrom}',{nameof(this.DownloadTo)}='{this.DownloadTo}',{nameof(this.Consumed)}='{this.Consumed}'";
        }
