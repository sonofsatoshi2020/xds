...
namespace UnnamedCoin.Bitcoin.Connection
{
    ...
    public class ConnectionManagerController : FeatureController
    {
        ...
        {
            var endpoint = endpointStr.ToIPEndPoint(this.ConnectionManager.Network.DefaultPort);
            switch (command)
            {
                case "add":
        ...
        /// </summary>
        /// <param name="endpoint">
        ///     The endpoint in string format. Specify an IP address. The default port for the network will be
        ///     added automatically.
        /// </param>
        /// <param name="command">The command to run. {add, remove, onetry}</param>
        /// <returns>
        ///     Json formatted <c>True</c> indicating success. Returns <see cref="IActionResult" /> formatted exception if
        ///     fails.
        /// </returns>
        ...
        public IActionResult AddNodeAPI([FromQuery] string endpoint, string command)
        {
            try
            {
                ...
                return Json(AddNodeRPC(endpoint, command));
            }
            catch (Exception e)
            {
        ...
        public List<PeerNodeModel> GetPeerInfoRPC()
        {
            ...
            foreach (var peer in peers)
                if (peer != null && peer.RemoteSocketAddress != null)
                {
                    ...
                    if (peer.TimeOffset != null) peerNode.TimeOffset = peer.TimeOffset.Value.Seconds;
        ...
        /// <remarks>This is an API implementation of an RPC call.</remarks>
        /// <returns>
        ///     Json formatted <see cref="List{T}<see cref="PeerNodeModel" />"/> of connected nodes. Returns
