...
namespace UnnamedCoin.Bitcoin.Controllers
{
    ...
    public class NodeController : Controller
    {
        ...
        readonly Network network; // Not readonly because of ValidateAddress
        ...
        public IActionResult Status()
        {
            ...
            foreach (var peer in this.connectionManager.ConnectedPeers)
            {
                ...
                var connectedPeer = new ConnectedPeerModel
                {
                    ...
                    TipHeight = chainHeadersBehavior.BestReceivedTip != null
                        ? chainHeadersBehavior.BestReceivedTip.Height
                        : peer.PeerVersion?.StartHeight ?? -1,
        ...
        ///     A flag that specifies whether to return the block header in the JSON format. Defaults to
        ///     true. A value of false is currently not supported.
        /// </param>
        /// <returns>
        ///     Json formatted <see cref="BlockHeaderModel" />. <c>null</c> if block not found. Returns
        ...
        public IActionResult GetBlockHeader([FromQuery] string hash, bool isJsonFormat = true)
        {
            try
            {
                ...
                if (blockHeader != null) model = new BlockHeaderModel(blockHeader);
        ...
        /// <returns>
        ///     Json formatted <see cref="TransactionBriefModel" /> or <see cref="TransactionVerboseModel" />. <c>null</c> if
        ///     transaction not found. Returns <see cref="IActionResult" /> formatted error if otherwise fails.
        /// </returns>
        ...
        public async Task<IActionResult> GetRawTransactionAsync([FromQuery] string trxid, bool verbose = false)
        {
            try
            {
                ...
                uint256 txid;
                if (!uint256.TryParse(trxid, out txid)) throw new ArgumentException(nameof(trxid));

                // First tries to find a pooledTransaction. If can't, will retrieve it from the blockstore if it exists.
                var trx = this.pooledTransaction != null
                    ? await this.pooledTransaction.GetTransaction(txid).ConfigureAwait(false)
                    : null;
                if (trx == null) trx = this.blockStore?.GetTransactionById(txid);

                if (trx == null) return Json(null);

                if (verbose)
                {
                    var block = GetTransactionBlock(txid, this.fullNode, this.chainIndexer);
        ...
        /// <returns>
        ///     Json formatted <see cref="ValidatedAddress" /> containing a boolean indicating address validity. Returns
        ///     <see cref="IActionResult" /> formatted error if fails.
        /// </returns>
        ...
        public IActionResult ValidateAddress([FromQuery] string address)
        {
            try
            {
                ...
                // P2SH
                else if (BitcoinScriptAddress.IsValid(address, this.network)) res.IsValid = true;
        ...
        /// <returns>
        ///     Json formatted <see cref="GetTxOutModel" />. <c>null</c> if no unspent outputs given parameters. Returns
        ///     <see cref="IActionResult" /> formatted error if fails.
        /// </returns>
            ...
            bool includeMemPool = true)
        {
            try
            {
                ...
                if (unspentOutputs == null) return Json(null);
        ...
        ///     <a href="https://docs.microsoft.com/en-us/aspnet/core/security/cors?view=aspnetcore-2.1">here</a>.
        /// </param>
        /// <remarks>
        ...
        public IActionResult GetAsyncLoops()
        {
            ...
            try
            {
                ...
                foreach (var (loopName, status) in this.asyncProvider.GetAll())
                    loops.Add(new AsyncLoopModel
                        {LoopName = loopName, Status = Enum.GetName(typeof(TaskStatus), status)});
        ...
        internal ChainedHeader GetTransactionBlock(uint256 trxid, IFullNode fullNode, ChainIndexer chain)
        {
            ...
            if (blockid != null) block = chain?.GetHeader(blockid);
