...
namespace UnnamedCoin.Bitcoin.BlockPulling
{
    ...
    public interface IBlockPullerBehavior : INetworkPeerBehavior
    {
        ...
        /// <remarks>
        ///     It's a value from <see cref="BlockPullerBehavior.MinQualityScore" /> to
        ///     <see cref="BlockPullerBehavior.MaxQualityScore" />.
        /// </remarks>
        ...
        ///     Thrown in case peer is in the wrong state or TCP connection was closed
        ///     during sending a message.
        /// </exception>
    ...
    public class BlockPullerBehavior : NetworkPeerBehavior, IBlockPullerBehavior
    {
        ...
        public BlockPullerBehavior(IBlockPuller blockPuller, IInitialBlockDownloadState ibdState,
            IDateTimeProvider dateTimeProvider, ILoggerFactory loggerFactory)
        {
            ...
            var samplesCount = ibdState.IsInitialBlockDownload() ? IbdSamplesCount : NormalSamplesCount;
        ...
        {
            var adjustedDelay = delaySinceRequestedSeconds;

            if (this.lastDeliveryTime != null)
            {
                var deliveryDiff = (this.dateTimeProvider.GetUtcNow() - this.lastDeliveryTime).Value.TotalSeconds;
        ...
        {
            var maxSamplesToPenalize =
                (int) (this.averageDelaySeconds.GetMaxSamples() * MaxSamplesPercentageToPenalize);
            var penalizeTimes = notDeliveredBlocksCount < maxSamplesToPenalize
                ? notDeliveredBlocksCount
                : maxSamplesToPenalize;
            if (penalizeTimes < 1)
                penalizeTimes = 1;

            this.logger.LogDebug("Peer will be penalized {0} times.", penalizeTimes);

            for (var i = 0; i < penalizeTimes; i++)
        ...
        public void OnIbdStateChanged(bool isIbd)
        {
            ...
            var samplesCount = isIbd ? IbdSamplesCount : NormalSamplesCount;
        ...
        Task OnMessageReceivedAsync(INetworkPeer peer, IncomingMessage message)
        {
            if (message.Message.Payload is BlockPayload block)
            {
                ...
                var blockHash = block.Obj.GetHash();
