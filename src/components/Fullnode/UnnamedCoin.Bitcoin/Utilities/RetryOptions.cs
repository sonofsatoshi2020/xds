...
namespace UnnamedCoin.Bitcoin.Utilities
{
    ...
    public class RetryOptions
    {
        ...
        public RetryOptions(short retryCount, TimeSpan delay, RetryStrategyType type = RetryStrategyType.Simple,
            params Type[] exceptionTypeTypes)
        {
            foreach (var exceptionType in exceptionTypeTypes)
                if (!typeof(Exception).IsAssignableFrom(exceptionType))
                    throw new ArgumentException(
                        $"All exception types must be valid exceptions. {exceptionType} is not an Exception.");
        ...
        {
            return
                $"Retry count: {this.RetryCount}; Delay: {this.Delay}; Exceptions: {string.Join(", ", this.ExceptionTypes.Select(et => et.FullName))}";
        }
    }
}