...
namespace UnnamedCoin.Bitcoin.Utilities
{
    ...
    public class DBreezeSerializer
    {
        ...
        public byte[] Serialize(object obj)
        {
            ...
            {
                var array = obj as object[] ?? collection.ToArray();

                var serializedItems = new byte[array.Length][];
                var itemIndex = 0;
                foreach (var arrayObject in array)
                {
                    var serializedObject = Serialize(arrayObject);
        ...
        public object Deserialize(byte[] bytes, Type type)
        {
            ...
            {
                var header = this.consensusFactory.CreateBlockHeader();
            ...
            {
                var transaction = this.consensusFactory.CreateTransaction();
