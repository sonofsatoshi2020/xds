...
namespace UnnamedCoin.Bitcoin.Utilities
{
    ...
    public class AsyncExecutionEvent<TSender, TArg> : IDisposable
    {
        ...
        /// </summary>
        /// <remarks>
        ///     This allows <see cref="Register(AsyncExecutionEventCallback{TSender, TArg}, bool)" /> and
        ///     <see cref="Unregister(AsyncExecutionEventCallback{TSender, TArg})" />
        ///     to recognize whether they are executing from a callback or not.
        /// </remarks>
        readonly AsyncLocal<bool> callbackExecutionInProgress;
        ...
        readonly LinkedList<AsyncExecutionEventCallback<TSender, TArg>> callbackList;

        /// <summary>Mapping of registered callbacks to nodes of <see cref="callbackList" /> to allow fast lookup during removals.</summary>
        /// <remarks>All access to this object has to be protected with <see cref="asyncLock" />.</remarks>
        readonly Dictionary<AsyncExecutionEventCallback<TSender, TArg>,
            LinkedListNode<AsyncExecutionEventCallback<TSender, TArg>>> callbackToListNodeMapping;

        /// <summary>
        ///     Cancellation source to abort waiting for <see cref="asyncLock" /> after <see cref="Dispose" /> has been
        ///     executed.
        /// </summary>
        ...
        public AsyncExecutionEvent()
        {
            ...
            this.callbackToListNodeMapping =
                new Dictionary<AsyncExecutionEventCallback<TSender, TArg>,
                    LinkedListNode<AsyncExecutionEventCallback<TSender, TArg>>>();
        ...
        /// <param name="addFirst">
        ///     <c>true</c> to insert the new callback as the first callback to be called,
        ///     <c>false</c> to add it as the last one.
        /// </param>
        ...
        public void Register(AsyncExecutionEventCallback<TSender, TArg> callbackAsync, bool addFirst = false)
        {
            ...
            var lockReleaser = !this.callbackExecutionInProgress.Value
                ? this.asyncLock.Lock(this.cancellationSource.Token)
                : null;
            try
            {
        ...
        ///     Thrown if <paramref name="callbackAsync" /> was not found among registered
        ///     callbacks.
        /// </exception>
        /// <remarks>
        ///     The caller is guaranteed that once this method completes, <paramref name="callbackAsync" /> will not be called by
        ///     this executor.
        ...
        /// <remarks>
        ///     It is necessary to hold the lock while calling the callbacks to provide guarantees described in
        ///     <see cref="Unregister(AsyncExecutionEventCallback{TSender, TArg})" />.
        ///     However, we do support new callbacks to be registered or unregistered while callbacks are being executed,
        ///     but this is only possible from the same execution context - i.e. another task or thread is unable to register or
        ...
        public async Task ExecuteCallbacksAsync(TSender sender, TArg arg)
        {
            ...
            try
            {
                using (await this.asyncLock.LockAsync(this.cancellationSource.Token).ConfigureAwait(false))
                {
                    ...
                    foreach (var callbackAsync in listCopy)
