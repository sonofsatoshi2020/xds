...
namespace UnnamedCoin.Bitcoin.Utilities
{
    public static class RetryStrategy
    {
        ...
        public static void Run(RetryOptions retryOptions, Action actionToExecute, ILogger logger = null)
        {
            ...
            for (var i = 0; i <= retryOptions.RetryCount; i++)
                try
                {
                    actionToExecute();
                    return;
                ...
                {
                    if (retryOptions.ExceptionTypes.All(et => et != ex.GetType()) ||
                        i >= retryOptions.RetryCount) throw;
                    ...
                    var delay = retryOptions.Type == RetryStrategyType.Simple
                        ? retryOptions.Delay
