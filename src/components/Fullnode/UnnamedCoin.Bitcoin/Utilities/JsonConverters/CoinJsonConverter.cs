...
namespace UnnamedCoin.Bitcoin.Utilities.JsonConverters
{
    /// <summary>
    ...
    public class CoinJson
    {
        ...
        public CoinJson(ICoin coin, Network network)
        {
            ...
            if (coin is ScriptCoin) this.RedeemScript = ((ScriptCoin) coin).Redeem;

            if (coin is Coin) this.Value = ((Coin) coin).Amount;
            ...
            if (coin is ColoredCoin)
            {
                ...
                if (scc != null) this.RedeemScript = scc.Redeem;
            }
        }
        ...
        public ICoin ToCoin()
        {
            var coin = this.RedeemScript == null
                ? new Coin(new OutPoint(this.TransactionId, this.Index), new TxOut(this.Value, this.ScriptPubKey))
                : new ScriptCoin(new OutPoint(this.TransactionId, this.Index), new TxOut(this.Value, this.ScriptPubKey),
                    this.RedeemScript);
