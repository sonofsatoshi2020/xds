...
namespace UnnamedCoin.Bitcoin.Utilities
{
    public static class ParallelAsync
    {
        ...
        public static async Task ForEachAsync<TSource>(this IEnumerable<TSource> collection, int maxDegreeOfParallelism,
            CancellationToken cancellationToken, Func<TSource, CancellationToken, Task> action)
        {
            ...
            foreach (var item in collection)
            {
                ...
                if (unfinished.Count >= maxDegreeOfParallelism)
                {
                    ...
                    foreach (var toRemove in unfinished.Where(x => x.IsCompleted).ToList())
