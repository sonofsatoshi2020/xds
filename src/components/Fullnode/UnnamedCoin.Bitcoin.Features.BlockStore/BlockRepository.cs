...
{
    /// <summary>
    ///     <see cref="IBlockRepository" /> is the interface to all the logics interacting with the blocks stored in the
    ///     database.
    /// </summary>
    public interface IBlockRepository : IBlockStore
    {
        ...
        ///     If <see cref="TxIndex" /> is true, we store the block hash alongside the transaction hash in the transaction table,
        ///     otherwise clear the transaction table.
        /// </summary>
        void ReIndex();
    ...
    public class BlockRepository : IBlockRepository
    {
        ...
        static readonly byte[] RepositoryTipKey = new byte[0];

        static readonly byte[] TxIndexKey = new byte[1];

        readonly DBreezeSerializer dBreezeSerializer;
        readonly IReadOnlyDictionary<uint256, Transaction> genesisTransactions;
        ...
        public virtual void Initialize()
        {
            ...
            using (var transaction = this.DBreeze.GetTransaction())
            {
                ...
                if (LoadTipHashAndHeight(transaction) == null)
                {
                    SaveTipHashAndHeight(transaction, new HashHeightPair(genesis.GetHash(), 0));
                    doCommit = true;
                }

                if (LoadTxIndex(transaction) == null)
                {
                    SaveTxIndex(transaction, false);
                    doCommit = true;
                }
        ...
        public Transaction GetTransactionById(uint256 trxid)
        {
            ...
            if (this.genesisTransactions.TryGetValue(trxid, out var genesisTransaction)) return genesisTransaction;
            ...
            using (var transaction = this.DBreeze.GetTransaction())
            {
                ...
                var transactionRow = transaction.Select<byte[], byte[]>(TransactionTableName, trxid.ToBytes());
                ...
                var blockRow = transaction.Select<byte[], byte[]>(BlockTableName, transactionRow.Value);
        ...
        public Transaction[] GetTransactionsByIds(uint256[] trxids, CancellationToken cancellation = default)
        {
            ...
            using (var transaction = this.DBreeze.GetTransaction())
            {
                ...
                for (var i = 0; i < trxids.Length; i++)
                {
                    ...
                    var alreadyFetched = trxids.Take(i).Any(x => x == trxids[i]);
                    ...
                    if (this.genesisTransactions.TryGetValue(trxids[i], out var genesisTransaction))
                    {
                        txes[i] = genesisTransaction;
                        continue;
                    }

                    var transactionRow = transaction.Select<byte[], byte[]>(TransactionTableName, trxids[i].ToBytes());
                    if (!transactionRow.Exists)
                    {
                    ...
                    var block = this.dBreezeSerializer.Deserialize<Block>(blockRow.Value);
                    var tx = block.Transactions.FirstOrDefault(t => t.GetHash() == trxids[i]);
        ...
        public uint256 GetBlockIdByTransactionId(uint256 trxid)
        {
            ...
            if (this.genesisTransactions.ContainsKey(trxid)) return this.network.GenesisHash;
        ...
        public void ReIndex()
        {
            using (var dbreezeTransaction = this.DBreeze.GetTransaction())
            {
                ...
                if (this.TxIndex)
                {
                    ...
                    var blockRows = dbreezeTransaction.SelectForward<byte[], byte[]>(BlockTableName);
                    foreach (var blockRow in blockRows)
                    {
                        var block = this.dBreezeSerializer.Deserialize<Block>(blockRow.Value);
                        foreach (var transaction in block.Transactions)
                            dbreezeTransaction.Insert(TransactionTableName, transaction.GetHash().ToBytes(),
                                block.GetHash().ToBytes());
        ...
        public void PutBlocks(HashHeightPair newTip, List<Block> blocks)
        {
            ...
            using (var transaction = this.DBreeze.GetTransaction())
            {
                ...
                OnInsertBlocks(transaction, blocks);

                // Commit additions
                SaveTipHashAndHeight(transaction, newTip);
                transaction.Commit();
            }
        }
        ...
        public void SetTxIndex(bool txIndex)
        {
            using (var transaction = this.DBreeze.GetTransaction())
            {
                SaveTxIndex(transaction, txIndex);
                transaction.Commit();
            }
        }
        ...
        public Block GetBlock(uint256 hash)
        {
            ...
            using (var transaction = this.DBreeze.GetTransaction())
            {
                ...
                var results = GetBlocksFromHashes(transaction, new List<uint256> {hash});
        ...
        public List<Block> GetBlocks(List<uint256> hashes)
        {
            ...
            using (var transaction = this.DBreeze.GetTransaction())
            {
                ...
                blocks = GetBlocksFromHashes(transaction, hashes);
            }
        ...
        public bool Exist(uint256 hash)
        {
            ...
            using (var transaction = this.DBreeze.GetTransaction())
            {
                ...
                var blockRow = transaction.Select<byte[], byte[]>("Block", key);
                if (blockRow.Exists)
                    res = true;
            }
        ...
        public void Delete(HashHeightPair newTip, List<uint256> hashes)
        {
            ...
            using (var transaction = this.DBreeze.GetTransaction())
            {
                ...
                var blocks = GetBlocksFromHashes(transaction, hashes);
                OnDeleteBlocks(transaction, blocks.Where(b => b != null).ToList());
                SaveTipHashAndHeight(transaction, newTip);
                transaction.Commit();
            }
        }
        ...
        protected virtual void OnInsertBlocks(DBreeze.Transactions.Transaction dbreezeTransaction, List<Block> blocks)
        {
            ...
            foreach (var kv in blockList)
            {
                ...
                var blockRow = dbreezeTransaction.Select<byte[], byte[]>(BlockTableName, blockId.ToBytes());
                if (!blockRow.Exists)
                {
                    dbreezeTransaction.Insert(BlockTableName, blockId.ToBytes(),
                        this.dBreezeSerializer.Serialize(block));
            ...
            if (this.TxIndex)
                OnInsertTransactions(dbreezeTransaction, transactions);
        }

        protected virtual void OnInsertTransactions(DBreeze.Transactions.Transaction dbreezeTransaction,
            List<(Transaction, Block)> transactions)
        {
            var byteListComparer = new ByteListComparer();
            transactions.Sort((pair1, pair2) =>
                byteListComparer.Compare(pair1.Item1.GetHash().ToBytes(), pair2.Item1.GetHash().ToBytes()));
        ...
        bool? LoadTxIndex(DBreeze.Transactions.Transaction dbreezeTransaction)
        {
            bool? res = null;
            var row = dbreezeTransaction.Select<byte[], bool>(CommonTableName, TxIndexKey);
            if (row.Exists)
            {
        ...
        void SaveTxIndex(DBreeze.Transactions.Transaction dbreezeTransaction, bool txIndex)
        {
            this.TxIndex = txIndex;
            dbreezeTransaction.Insert(CommonTableName, TxIndexKey, txIndex);
        }

        HashHeightPair LoadTipHashAndHeight(DBreeze.Transactions.Transaction dbreezeTransaction)
        {
            if (this.TipHashAndHeight == null)
            {
                dbreezeTransaction.ValuesLazyLoadingIsOn = false;

                var row = dbreezeTransaction.Select<byte[], byte[]>(CommonTableName, RepositoryTipKey);
                if (row.Exists)
        ...
        void SaveTipHashAndHeight(DBreeze.Transactions.Transaction dbreezeTransaction, HashHeightPair newTip)
        {
        ...
        protected virtual void OnDeleteTransactions(DBreeze.Transactions.Transaction dbreezeTransaction,
            List<(Transaction, Block)> transactions)
        {
            foreach (var (transaction, block) in transactions)
                dbreezeTransaction.RemoveKey(TransactionTableName, transaction.GetHash().ToBytes());
        }
        ...
        protected virtual void OnDeleteBlocks(DBreeze.Transactions.Transaction dbreezeTransaction, List<Block> blocks)
        {
            if (this.TxIndex)
            {
                ...
                OnDeleteTransactions(dbreezeTransaction, transactions);
            }

            foreach (var block in blocks)
                dbreezeTransaction.RemoveKey(BlockTableName, block.GetHash().ToBytes());
        }

        public List<Block> GetBlocksFromHashes(DBreeze.Transactions.Transaction dbreezeTransaction,
            List<uint256> hashes)
        {
            ...
            foreach (var key in keys)
            {
                ...
                var blockRow = dbreezeTransaction.Select<byte[], byte[]>(BlockTableName, key.Item2);
                if (blockRow.Exists)
                {
