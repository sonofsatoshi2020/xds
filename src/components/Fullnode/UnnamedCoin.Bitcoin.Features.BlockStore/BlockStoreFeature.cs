...
using UnnamedCoin.Bitcoin.Features.BlockStore.AddressIndexing;
using UnnamedCoin.Bitcoin.Features.BlockStore.Pruning;
...
namespace UnnamedCoin.Bitcoin.Features.BlockStore
{
    ...
    {
        readonly IAddressIndexer addressIndexer;

        readonly IBlockStoreQueue blockStoreQueue;

        readonly BlockStoreSignaled blockStoreSignaled;
        ...
        readonly IPrunedBlockRepository prunedBlockRepository;
        ...
        {
            var highestBlock = this.chainState.BlockStoreTip;
            ...
            if (highestBlock != null)
            {
                ...
                builder.Append(" BlockStore.Hash: ".PadRight(LoggingConfiguration.ColumnLength - 1) +
        ...
        public override Task InitializeAsync()
        {
            ...
            if (!this.storeSettings.PruningEnabled && this.prunedBlockRepository.PrunedTip != null)
                throw new BlockStoreException(
                    "The node cannot start as it has been previously pruned, please clear the data folders and resync.");

            if (this.storeSettings.PruningEnabled)
            {
                if (this.storeSettings.AmountOfBlocksToKeep < this.network.Consensus.MaxReorgLength)
                    throw new BlockStoreException(
                        $"The amount of blocks to prune [{this.storeSettings.AmountOfBlocksToKeep}] (blocks to keep) cannot be less than the node's max reorg length of {this.network.Consensus.MaxReorgLength}.");
            ...
            if (this.network.Consensus.IsProofOfStake)
                this.connectionManager.Parameters.TemplateBehaviors.Add(new ProvenHeadersBlockStoreBehavior(
                    this.network, this.chainIndexer, this.chainState, this.loggerFactory, this.consensusManager,
                    this.checkpoints, this.blockStoreQueue));
            else
                this.connectionManager.Parameters.TemplateBehaviors.Add(new BlockStoreBehavior(this.chainIndexer,
                    this.chainState, this.loggerFactory, this.consensusManager, this.blockStoreQueue));

            // Signal to peers that this node can serve blocks.
            // TODO: Add NetworkLimited which is what BTC uses for pruned nodes.
            this.connectionManager.Parameters.Services = this.storeSettings.PruningEnabled
                ? NetworkPeerServices.Nothing
                : NetworkPeerServices.Network;
        ...
        public static IFullNodeBuilder UseBlockStore(this IFullNodeBuilder fullNodeBuilder)
        {
            ...
            fullNodeBuilder.ConfigureFeature(features =>
            {
                features
                    ...
                    {
                        services.AddSingleton<IBlockStoreQueue, BlockStoreQueue>()
                            .AddSingleton<IBlockStore>(provider => provider.GetService<IBlockStoreQueue>());
