...
namespace UnnamedCoin.Bitcoin.Features.BlockStore.AddressIndexing
{
    ...
    {
        const string DbAddressDataKey = "AddrData";

        readonly LiteCollection<AddressIndexerData> addressIndexerDataCollection;

        readonly ILogger logger;

        public AddressIndexRepository(LiteDatabase db, ILoggerFactory loggerFactory,
            int maxBalanceChangesToKeep = 50_000) : base(maxBalanceChangesToKeep)
        {
            this.logger = loggerFactory.CreateLogger(GetType().FullName);
            this.addressIndexerDataCollection = db.GetCollection<AddressIndexerData>(DbAddressDataKey);
            this.addressIndexerDataCollection.EnsureIndex("BalanceChangedHeightIndex",
                "$.BalanceChanges[*].BalanceChangedHeight");
        }

        /// <summary>
        ///     Retrieves address data, either the cached version if it exists, or directly from the underlying database.
        ///     If it is a previously unseen address an empty record will be created and added to the cache.
        /// </summary>
        /// <param name="address">The address to retrieve data for.</param>
        public AddressIndexerData GetOrCreateAddress(string address)
        {
            if (!TryGetValue(address, out var data))
            {
                this.logger.LogDebug("Not found in cache.");
                data = this.addressIndexerDataCollection.FindById(address) ?? new AddressIndexerData
                           {Address = address, BalanceChanges = new List<AddressBalanceChange>()};
            }

            var size = 1 + data.BalanceChanges.Count / 10;
        ...
        public List<string> GetAddressesHigherThanHeight(int height)
        {
            ...
            var affectedAddresses =
                this.addressIndexerDataCollection.Find(Query.GT("BalanceChangedHeightIndex", height));
