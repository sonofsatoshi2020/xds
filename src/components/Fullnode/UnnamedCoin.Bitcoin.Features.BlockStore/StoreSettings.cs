...
namespace UnnamedCoin.Bitcoin.Features.BlockStore
{
    ...
    public class StoreSettings
    {
        ...
        public StoreSettings(NodeSettings nodeSettings)
        {
            ...
            this.AmountOfBlocksToKeep = config.GetOrDefault("prune", 0, this.logger);
            ...
            if (this.PruningEnabled && this.AmountOfBlocksToKeep < GetMinPruningAmount())
                throw new ConfigurationException(
                    $"The minimum amount of blocks to keep can't be less than {GetMinPruningAmount()}.");
            ...
            this.TxIndex = config.GetOrDefault("txindex", false, this.logger);
