...
using UnnamedCoin.Bitcoin.Features.BlockStore.Models;
...
namespace UnnamedCoin.Bitcoin.Features.BlockStore.Controllers
{
    ...
    public class BlockStoreController : Controller
    {
        ...
        public IActionResult GetAddressIndexerTip()
        {
            try
            {
                var addressIndexerTip = this.addressIndexer.IndexerTip;
                return Json(new AddressIndexerTipModel
                    {TipHash = addressIndexerTip?.HashBlock, TipHeight = addressIndexerTip?.Height});
            }
            catch (Exception e)
            {
        ...
        /// <returns>
        ///     <see cref="BlockModel" /> if block is found, <see cref="NotFoundObjectResult" /> if not found. Returns
        ///     <see cref="IActionResult" /> with error information if exception thrown.
        /// </returns>
        ...
        public IActionResult GetBlock([FromQuery] SearchByHashRequest query)
        {
            ...
            try
            {
                var blockId = uint256.Parse(query.Hash);
                ...
                if (!query.OutputJson) return Json(block);

                var blockModel = query.ShowTransactionDetails
                ...
                if (this.network.Consensus.IsProofOfStake)
                {
                    ...
                    blockModel.PosChainTrust =
                        chainedHeader.ChainWork.ToString(); // this should be similar to ChainWork
                }
        ...
        /// <returns>
        ///     The current tip height. Returns <c>null</c> if fails. Returns <see cref="IActionResult" /> with error
        ///     information if exception thrown.
        /// </returns>
        ...
        /// <summary>
        ///     Provides balance of the given addresses confirmed with at least <paramref name="minConfirmations" />
        ///     confirmations.
        /// </summary>
        ...
        /// <returns>
        ///     A result object containing the balance for each requested address and if so, a meesage stating why the indexer
        ///     is not queryable.
        /// </returns>
        ...
        public IActionResult GetVerboseAddressesBalancesData(string addresses)
        {
            try
            {
                var addressesArray = addresses?.Split(',') ?? new string[] { };

                this.logger.LogDebug("Asking data for {0} addresses.", addressesArray.Length);

                var result = this.addressIndexer.GetAddressIndexerState(addressesArray);
