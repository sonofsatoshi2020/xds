...
namespace UnnamedCoin.Bitcoin.Features.BlockStore.Pruning
{
    ...
    public sealed class PruneBlockStoreService : IPruneBlockStoreService
    {
        ...
        {
            this.PrunedUpToHeaderTip =
                this.chainState.BlockStoreTip.GetAncestor(this.prunedBlockRepository.PrunedTip.Height);

            this.asyncLoop = this.asyncProvider.CreateAndRunAsyncLoop($"{GetType().Name}.{nameof(PruneBlocks)}",
                token =>
                {
                    PruneBlocks();
        ...
        public void PruneBlocks()
        {
            if (this.PrunedUpToHeaderTip == null)
                throw new BlockStoreException(
                    $"{nameof(this.PrunedUpToHeaderTip)} has not been set, please call initialize first.");
            ...
            if (this.blockRepository.TipHashAndHeight.Height <=
                this.PrunedUpToHeaderTip.Height + this.storeSettings.AmountOfBlocksToKeep)
            {
                this.logger.LogTrace("(-)[PRUNE_ABORTED_BLOCKSTORE_TIP_BELOW_OR_EQUAL_THRESHOLD]");
                return;
            }

            var heightToPruneFrom =
                this.blockRepository.TipHashAndHeight.Height - this.storeSettings.AmountOfBlocksToKeep;
            var startFrom = this.chainState.BlockStoreTip.GetAncestor(heightToPruneFrom);
            if (startFrom == null)
            {
                this.logger.LogInformation("(-)[PRUNE_ABORTED_START_BLOCK_NOT_FOUND]{0}:{1}", nameof(heightToPruneFrom),
                    heightToPruneFrom);
                return;
            }

            this.logger.LogInformation("Pruning triggered, delete from {0} to {1}.", heightToPruneFrom,
                this.PrunedUpToHeaderTip.Height);
            ...
            var prunedTip = chainedHeadersToDelete.First();
