...
namespace UnnamedCoin.Bitcoin.Features.BlockStore.Pruning
{
    ...
    ///     <para>
    ///         If the height of the node's block store is more than <see cref="PruneBlockStoreService.MaxBlocksToKeep" />, the
    ///         node will
    ///         be pruned, leaving a margin of <see cref="PruneBlockStoreService.MaxBlocksToKeep" /> in the block database.
    ///     </para>
    ///     <para>
    ///         For example if the block store's height is 5000, the node will be pruned up to height 4000, meaning that 1000
    ///         blocks will be kept on disk.
    ///     </para>
    /// </summary>
    public interface IPruneBlockStoreService : IDisposable
    {
        ...
        ///     <para>
        ///         It should be noted that deleting (pruning) blocks from the repository only removes the reference, it does not
        ///         decrease the actual size on disk.
        ///     </para>
        /// </summary>
