...
namespace UnnamedCoin.Bitcoin.Features.MemoryPool
{
    ...
    public class BlocksDisconnectedSignaled : IDisposable
    {
        ...
        SubscriptionToken blockDisconnectedSubscription;
        ...
        void OnBlockDisconnected(BlockDisconnected blockDisconnected)
        {
            RemoveInvalidTransactionsAsync(blockDisconnected.DisconnectedBlock.Block).ConfigureAwait(false).GetAwaiter()
                .GetResult();
            AddBackToMempoolAsync(blockDisconnected.DisconnectedBlock.Block).ConfigureAwait(false).GetAwaiter()
                .GetResult();
        }
        ...
        async Task AddBackToMempoolAsync(Block block)
        {
            ...
            await this.mempoolLock.WriteAsync(async () =>
            {
                foreach (var transaction in block.Transactions)
                {
                    ...
                    var success = await this.mempoolValidator.AcceptToMemoryPool(state, transaction);

                    if (!success)
                        this.logger.LogDebug(
                            "Transaction with hash '{0}' failed to go back into mempool on block disconnect.",
                            transaction.GetHash());
                    else
                        this.logger.LogDebug("Transaction with hash '{0}' accepted back to mempool.",
                            transaction.GetHash());
                }
        ...
        /// </summary>
        async Task RemoveInvalidTransactionsAsync(Block block)
        {
            ...
            var coinbaseId = block.Transactions[0].GetHash();

            await this.mempoolLock.WriteAsync(async () =>
            {
                foreach (var mempoolEntry in this.mempool.MapTx.SpendsCoinbase.ToList())
                    if (mempoolEntry.Transaction.Inputs.Any(x => x.PrevOut.Hash == coinbaseId))
                    {
                        this.logger.LogDebug(
                            "Removing transaction '{0}' from the mempool as it spends the coinbase transaction '{1}'",
                            mempoolEntry.TransactionHash, coinbaseId);
