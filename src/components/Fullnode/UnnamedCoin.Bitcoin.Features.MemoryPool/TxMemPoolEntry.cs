...
namespace UnnamedCoin.Bitcoin.Features.MemoryPool
{
    ...
    public class TxMempoolEntry : IComparable, ITxMempoolFees
    {
            ...
            long nSigOpsCost, LockPoints lp, ConsensusOptions consensusOptions)
        {
            ...
            this.nModSize = MempoolValidator.CalculateModifiedSize(this.Transaction.GetSerializedSize(),
                this.Transaction, consensusOptions);

            this.nUsageSize =
                transaction.GetSerializedSize(); // RecursiveDynamicUsage(*tx) + memusage::DynamicUsage(Transaction);

            this.CountWithDescendants = 1;
            this.SizeWithDescendants = GetTxSize();
            this.ModFeesWithDescendants = this.Fee;
            var nValueIn = transaction.TotalOut + this.Fee;
        ...
        /// <summary>
        ///     Gets the size of the transaction with it's ancestors.
        /// </summary>
        ...
        {
            var deltaPriority = (double) (currentHeight - this.EntryHeight) * this.InChainInputValue.Satoshi /
                                this.nModSize;
            var dResult = this.entryPriority + deltaPriority;
        ...
        /// <param name="modifySigOps">
        ///     Cost to add to the total signature operators cost for all ancestor transactions for this
        ///     entry.
        /// </param>
