...
namespace UnnamedCoin.Bitcoin.Features.MemoryPool.Rules
{
    ...
    public class CheckAncestorsMempoolRule : MempoolRule
    {
        ...
        public override void CheckTransaction(MempoolValidationContext context)
        {
            ...
            var nLimitAncestors = this.settings.LimitAncestors;
            var nLimitAncestorSize = this.settings.LimitAncestorSize * 1000;
            var nLimitDescendants = this.settings.LimitDescendants;
            var nLimitDescendantSize = this.settings.LimitDescendantSize * 1000;

            if (!this.mempool.CalculateMemPoolAncestors(context.Entry, context.SetAncestors, nLimitAncestors,
                nLimitAncestorSize, nLimitDescendants, nLimitDescendantSize, out var errString))
            {
