...
namespace UnnamedCoin.Bitcoin.Features.MemoryPool.Rules
{
    ...
    {
        readonly IConsensusRuleEngine consensusRuleEngine;
        ...
        bool CheckInputs(MempoolValidationContext context, ScriptVerify scriptVerify, PrecomputedTransactionData txData)
        {
            ...
            this.consensusRuleEngine.GetRule<CoinViewRule>()
                .CheckInputs(context.Transaction, context.View.Set, this.chainIndexer.Height + 1);
            ...
            for (var iInput = 0; iInput < tx.Inputs.Count; iInput++)
            {
                ...
                if (!ctx.VerifyScript(input.ScriptSig, txout.ScriptPubKey, checker))
                {
                    if ((scriptVerify & ScriptVerify.StandardNotMandatory) == ScriptVerify.StandardNotMandatory)
                    {
                        ...
                        if (ctx.VerifyScript(input.ScriptSig, txout.ScriptPubKey, checker))
                        {
                            ...
                            context.State.Fail(MempoolErrors.NonMandatoryScriptVerifyFlagFailed, ctx.Error.ToString())
                                .Throw();
                        }
                    }
