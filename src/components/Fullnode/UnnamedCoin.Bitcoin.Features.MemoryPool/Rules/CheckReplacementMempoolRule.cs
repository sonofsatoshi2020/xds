        ...
        public override void CheckTransaction(MempoolValidationContext context)
        {
            ...
            if (context.SetConflicts.Any())
            {
                ...
                foreach (var hashConflicting in context.SetConflicts)
                {
                    ...
                    foreach (var txin in mi.Transaction.Inputs) setConflictsParents.Add(txin.PrevOut.Hash);
                ...
                for (var j = 0; j < context.Transaction.Inputs.Count; j++)
                ...
                var nDeltaFees = context.ModifiedFees - context.ConflictingFees;
                if (nDeltaFees < context.MinRelayTxFee.GetFee(context.EntrySize))
                {
                    this.logger.LogTrace("(-)[FAIL_INSUFFICIENT_DELTA_FEE]");
                    context.State.Fail(MempoolErrors.Insufficientfee,
                            $"rejecting replacement {context.TransactionHash}, not enough additional fees to relay; {nDeltaFees} < {context.MinRelayTxFee.GetFee(context.EntrySize)}")
                        .Throw();
                }
            }
        }
