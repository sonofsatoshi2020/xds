...
namespace UnnamedCoin.Bitcoin.Features.MemoryPool.Rules
{
    ...
    public class CreateMempoolEntryMempoolRule : MempoolRule
    {
        ...
        public static bool CheckSequenceLocks(Network network, ChainedHeader tip, MempoolValidationContext context,
            Transaction.LockTimeFlags flags, LockPoints lp = null, bool useExistingLockPoints = false)
        {
        ...
        bool AreInputsStandard(Network network, Transaction tx, MempoolCoinView mapInputs)
        {
            if (tx.IsCoinBase)
            {
            ...
            for (var i = 0; i < tx.Inputs.Count; i++)
            {
                ...
                var template = network.StandardScriptsRegistry.GetTemplateFromScriptPubKey(prev.ScriptPubKey);
                ...
                */
                if (template.Type == TxOutType.TX_SCRIPTHASH)
                {
                    ...
                    if (!ctx.EvalScript(txin.ScriptSig, tx, i)
                    ) // TODO: Check the semantics of SigVersion::BASE from original code
                        return false;
        ...
        ///     <seealso
        ///         cref="https://github.com/bitcoin/bitcoin/blob/aa624b61c928295c27ffbb4d27be582f5aa31b56/src/policy/policy.cpp#L196" />
        /// </summary>
        /// <param name="tx">Transaction to verify.</param>
        /// <param name="mapInputs">Map of previous transactions that have outputs we're spending.</param>
        /// <returns>Whether transaction is witness standard.</returns>
        bool IsWitnessStandard(Transaction tx, MempoolCoinView mapInputs)
        {
            if (tx.IsCoinBase)
            {
                this.logger.LogTrace("(-)[IS_COINBASE]:true");
            ...
            foreach (var input in tx.Inputs)
            {
                ...
                var prev = mapInputs.GetOutputFor(input);

                // Get the scriptPubKey corresponding to this input.
                var prevScript = prev.ScriptPubKey;
                if (prevScript.IsPayToScriptHash(this.network))
                {
                    ...
                    var sigParams =
                        PayToScriptHashTemplate.Instance.ExtractScriptSigParameters(this.network, input.ScriptSig);
                ...
                var wit = PayToWitTemplate.Instance.ExtractScriptPubKeyParameters2(this.chainIndexer.Network,
                    prevScript);
                if (wit == null)
                {
                ...
                {
                    var witness = input.WitScript;

                    // Get P2WSH script from top of stack.
                    var scriptPubKey = Script.FromBytesUnsafe(witness.GetUnsafePush(witness.PushCount - 1));

                    // Stack items are remainder of stack.
                    var sizeWitnessStack = witness.PushCount - 1;

                    // Get the witness stack items.
                    var stack = new List<byte[]>();
                    for (var i = 0; i < sizeWitnessStack; i++) stack.Add(witness.GetUnsafePush(i));
                    ...
                    for (var j = 0; j < sizeWitnessStack; j++)
