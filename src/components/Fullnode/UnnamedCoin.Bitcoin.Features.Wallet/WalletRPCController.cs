...
namespace UnnamedCoin.Bitcoin.Features.Wallet
{
    ...
    public class WalletRPCController : FeatureController
    {
        ...
        public WalletRPCController(
            ...
            IWalletTransactionHandler walletTransactionHandler) : base(fullNode, consensusManager: consensusManager,
            chainIndexer: chainIndexer, network: network)
        {
        ...
        public bool UnlockWallet(string passphrase, int timeout)
        {
            ...
            var account = GetWalletAccountReference();
        ...
        public async Task<uint256> SendToAddressAsync(BitcoinAddress address, decimal amount, string commentTx,
            string commentDest)
        {
            var account = GetWalletAccountReference();
            ...
            var context = new TransactionBuildContext(this.FullNode.Network)
            {
                ...
                Recipients = new[] {new Recipient {Amount = Money.Coins(amount), ScriptPubKey = address.ScriptPubKey}}
                    .ToList(),
                CacheSecret = false
            };
        ...
        {
            var transaction = this.FullNode.Network.CreateTransaction(hex);
            await this.broadcasterManager.BroadcastTransactionAsync(transaction);
        ...
        public NewAddressModel GetNewAddress(string account = "", string addressType = "")
        {
            if (!string.IsNullOrEmpty(account))
                throw new RPCServerException(RPCErrorCode.RPC_METHOD_DEPRECATED,
                    "Use of 'account' parameter has been deprecated");
            ...
            if (!string.IsNullOrEmpty(addressType))
                ...
                if (!addressType.Equals("legacy", StringComparison.InvariantCultureIgnoreCase))
                    throw new RPCServerException(RPCErrorCode.RPC_METHOD_NOT_FOUND,
                        "Only address type 'legacy' is currently supported.");

            var accountReference = GetWalletAccountReference();

            var hdAddress = this.walletManager.GetUnusedAddresses(accountReference, 1, alwaysnew: true).Single();

            var base58Address = hdAddress.Address;
        ...
        public NewAddressModel GetUnusedAddress(string account, string addressType)
        {
            ...
            if (!string.IsNullOrEmpty(addressType))
                ...
                if (!addressType.Equals("legacy", StringComparison.InvariantCultureIgnoreCase))
                    throw new RPCServerException(RPCErrorCode.RPC_METHOD_NOT_FOUND,
                        "Only address type 'legacy' is currently supported.");
            var hdAddress = this.walletManager.GetUnusedAddress(GetWalletAccountReference());
            var base58Address = hdAddress.Address;
        ...
        ///     Remains for backward compatibility. Must be excluded or set to "*" or "". Deprecated in
        ///     latest bitcoin core (0.17.0).
        /// </param>
        ...
        public decimal GetBalance(string accountName, int minConfirmations = 0)
        {
            if (!string.IsNullOrEmpty(accountName) && !accountName.Equals("*"))
                throw new RPCServerException(RPCErrorCode.RPC_METHOD_DEPRECATED,
                    "Account has been deprecated, must be excluded or set to \"*\"");

            var account = GetWalletAccountReference();

            Money balance = this.walletManager.GetSpendableTransactionsInAccount(account, minConfirmations)
        ...
        public async Task<ListSinceBlockModel> ListSinceBlockAsync(string blockHash, int targetConfirmations = 1)
        {
            ...
            if (!string.IsNullOrEmpty(blockHash) && uint256.TryParse(blockHash, out var hashBlock))
            ...
            var transactions = wallet.GetAllTransactions();

            var model = new ListSinceBlockModel();

            foreach (var transactionData in transactions)
            {
                var transaction = GetTransaction(transactionData.Id.ToString());

                var blockHeight = transactionData.BlockHeight ?? 0;
                ...
                var category = GetListSinceBlockTransactionCategoryModel(transaction);
        ...
        {
            if (!uint256.TryParse(txid, out var trxid))
                throw new ArgumentException(nameof(txid));

            var accountReference = GetWalletAccountReference();
            var account = this.walletManager.GetAccounts(accountReference.WalletName)
                .Single(a => a.Name == accountReference.AccountName);

            // Get the transaction from the wallet by looking into received and send transactions.
            var addresses = account.GetCombinedAddresses().ToList();
            var receivedTransactions = addresses.Where(r => !r.IsChangeAddress() && r.Transactions != null)
                .SelectMany(a => a.Transactions.Where(t => t.Id == trxid)).ToList();
            var sendTransactions = addresses.Where(r => r.Transactions != null).SelectMany(a =>
                    a.Transactions.Where(t => t.SpendingDetails != null && t.SpendingDetails.TransactionId == trxid))
                .ToList();

            if (!receivedTransactions.Any() && !sendTransactions.Any())
                throw new RPCServerException(RPCErrorCode.RPC_INVALID_ADDRESS_OR_KEY,
            ...
            if (blockHash != null)
                this.ConsensusManager.GetOrDownloadBlocks(new List<uint256> {blockHash},
            ...
            foreach (var paymentDetail in sendTransactions.Select(s => s.SpendingDetails).SelectMany(sd => sd.Payments))
            ...
            foreach (var trxInWallet in receivedTransactions)
            {
                GetTransactionDetailsCategoryModel category;
                if (isGenerated)
                    category = model.Confirmations > this.FullNode.Network.Consensus.CoinbaseMaturity
                        ? GetTransactionDetailsCategoryModel.Generate
                        : GetTransactionDetailsCategoryModel.Immature;
                else
        ...
        public AddressGroupingModel[] ListAddressGroupings()
        {
            if (!this.storeSettings.TxIndex)
                throw new RPCServerException(RPCErrorCode.RPC_INVALID_REQUEST,
                    $"{nameof(ListAddressGroupings)} is incompatible with transaction indexing turned off (i.e. -txIndex=0).");
        ...
        /// <summary>
        ///     Returns a list of grouped addresses which have had their common ownership made public by common use as inputs or as
        ///     the resulting change in past transactions.
        /// </summary
        /// <remarks>
        ///     Please see
        ///     https://github.com/bitcoin/bitcoin/blob/726d0668ff780acb59ab0200359488ce700f6ae6/src/wallet/wallet.cpp#L3641
        /// </remarks>
        ...
        List<List<string>> GetAddressGroupings(string walletName)
        {
            ...
            foreach (var item in txs) txDictionary.TryAdd(item.Id, item);
            ...
            foreach (var transaction in txDictionary)
            {
                ...
                if (tx.Inputs.Count > 0)
                {
                    ...
                    foreach (var txIn in tx.Inputs)
                    {
                        ...
                        var addressBase58 =
                            this.scriptAddressReader.GetAddressFromScriptPubKey(this.Network,
                                prevTransactionScriptPubkey);
                ...
                foreach (var txOut in tx.Outputs)
                    if (IsAddressMine(addresses, txOut.ScriptPubKey))
                    {
                        ...
                        var addressBase58 =
                            this.scriptAddressReader.GetAddressFromScriptPubKey(this.Network, txOut.ScriptPubKey);
                        if (string.IsNullOrEmpty(addressBase58))
                            continue;
        ...
        bool IsChange(IEnumerable<HdAddress> internalAddresses, Script txOutScriptPubkey)
        {
        ...
        bool IsAddressMine(IEnumerable<HdAddress> addresses, Script scriptPubKey)
        {
        ...
        public UnspentCoinModel[] ListUnspent(int minConfirmations = 1, int maxConfirmations = 9999999,
            string addressesJson = null)
        {
            var addresses = new List<BitcoinAddress>();
            if (!string.IsNullOrEmpty(addressesJson))
                JsonConvert.DeserializeObject<List<string>>(addressesJson).ForEach(i =>
                    addresses.Add(BitcoinAddress.Create(i, this.FullNode.Network)));

            var accountReference = GetWalletAccountReference();
            var spendableTransactions =
                this.walletManager.GetSpendableTransactionsInAccount(accountReference, minConfirmations);
            ...
            foreach (var spendableTx in spendableTransactions)
                if (spendableTx.Confirmations <= maxConfirmations)
                    if (!addresses.Any() ||
                        addresses.Contains(BitcoinAddress.Create(spendableTx.Address.Address, this.FullNode.Network)))
                        unspentCoins.Add(new UnspentCoinModel
                        {
                            ...
                            RedeemScriptHex =
                                null, // TODO: Currently don't support P2SH wallet addresses, review if we do.
                            Confirmations = spendableTx.Confirmations,
                            IsSpendable = !spendableTx.Transaction.IsSpent(),
                            IsSolvable =
                                !spendableTx.Transaction.IsSpent() // If it's spendable we assume it's solvable.
                        });
        ...
        /// <summary>
        ///     Gets the first account from the "default" wallet if it specified, otherwise returns the first available account in
        ///     the existing wallets.
        /// </summary>
        /// <returns>Reference to the default wallet account, or the first available if no default wallet is specified.</returns>
        WalletAccountReference GetWalletAccountReference()
        {
            string walletName = null;

            if (this.walletSettings.IsDefaultWalletEnabled())
                walletName = this.walletManager.GetWalletsNames()
                    .FirstOrDefault(w => w == this.walletSettings.DefaultWalletName);
            else
