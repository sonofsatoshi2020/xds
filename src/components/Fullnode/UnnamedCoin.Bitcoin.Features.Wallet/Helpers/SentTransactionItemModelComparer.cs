...
namespace UnnamedCoin.Bitcoin.Features.Wallet.Helpers
{
    /// <summary>
    ///     Used for comparing a list of TransactionItemModel to each other.
    /// </summary>
    /// <remarks>
    ///     This specific comparer has been made to filter out duplicate payment listings in the case of a transaction with
    ///     multiple inputs.
    ///     This causes multiple items inside the wallet data that all have the same amount, transaction/spending id and
    ///     payment details.
    ...
    public class SentTransactionItemModelComparer : IEqualityComparer<TransactionItemModel>
    {
        ...
        public bool Equals(TransactionItemModel x, TransactionItemModel y)
        {
            ...
            if (!propertiesAreEqual) return false;
            ...
            foreach (var payment in x.Payments)
                ...
                if (!y.Payments.Any(w =>
                    w.Amount == payment.Amount && w.DestinationAddress == payment.DestinationAddress))
                    return false;
        ...
        public int GetHashCode(TransactionItemModel obj)
        {
            ...
            foreach (var payment in obj.Payments)
                hashValue = hashValue ^ payment.Amount.GetHashCode() ^
                            (payment.DestinationAddress ?? string.Empty).GetHashCode();
