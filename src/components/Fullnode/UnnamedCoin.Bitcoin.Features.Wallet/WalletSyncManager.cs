...
namespace UnnamedCoin.Bitcoin.Features.Wallet
{
    public class WalletSyncManager : IWalletSyncManager, IDisposable
    {
        ...
        readonly IAsyncDelegateDequeuer<Block> blocksQueue;
        ...
        public void Start()
        {
            ...
            this.logger.LogInformation("WalletSyncManager initialized. Wallet at block {0}.",
                this.walletManager.LastBlockHeight());
            ...
            if (this.walletTip == null)
            {
                ...
                var locators = this.walletManager.ContainsWallets
                    ? this.walletManager.GetFirstWalletBlockLocator()
                    : new[] {this.chainIndexer.Tip.HashBlock};
                var blockLocator = new BlockLocator {Blocks = locators.ToList()};
                var fork = this.chainIndexer.FindFork(blockLocator);
        ...
        public virtual void ProcessBlock(Block block)
        {
            ...
            if (!this.maxQueueSizeReached)
            {
                var currentBlockQueueSize = Interlocked.Add(ref this.blocksQueueSize, block.BlockSize.Value);
        ...
        {
            var blockSyncStart = this.chainIndexer.GetHeightAtTime(date);
        ...
        /// <summary>
        ///     Called when a <see cref="Block" /> is added to the <see cref="blocksQueue" />.
        ///     Depending on the <see cref="WalletTip" /> and incoming block height, this method will decide whether the
        ///     <see cref="Block" /> will be processed by the <see cref="WalletManager" />.
        /// </summary>
        ...
        async Task OnProcessBlockAsync(Block block, CancellationToken cancellationToken)
        {
            ...
            var newTip = this.chainIndexer.GetHeader(block.GetHash());
            ...
            if (block.Header.HashPrevBlock != this.walletTip.HashBlock)
            {
                ...
                var inBestChain = this.chainIndexer.GetHeader(this.walletTip.HashBlock);
                if (inBestChain == null)
                {
                ...
                {
                    var findTip = newTip.FindAncestorOrSelf(this.walletTip);
                    if (findTip == null)
                    {
                ...
                {
                    var findTip = this.walletTip.FindAncestorOrSelf(newTip);
                    if (findTip == null)
                    {
                    ...
                    this.logger.LogDebug("Wallet tip '{0}' is ahead or equal to the new tip '{1}'.", this.walletTip,
                        newTip);
                }
            }
            else
