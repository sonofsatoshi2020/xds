...
namespace UnnamedCoin.Bitcoin.Features.Wallet
{
    public class AddressBookManager : IAddressBookManager
    {
        ...
        const string AddressBookFileName = "addressbook.json";

        /// <summary>An object capable of storing <see cref="Wallet" />s to the file system.</summary>
        readonly FileStorage<AddressBook> fileStorage;
        ...
        public AddressBookEntry AddNewAddress(string label, string address)
        {
            if (this.addressBook.Addresses.Any(i => i.Label == label || i.Address == address))
                throw new AddressBookException(
                    $"An entry with label '{label}' or address '{address}' already exist in the address book.");

            var newEntry = new AddressBookEntry {Label = label, Address = address};
        ...
        {
            var item = this.addressBook.Addresses.SingleOrDefault(i => i.Label == label);
