...
namespace UnnamedCoin.Bitcoin.Features.Wallet
{
    ...
    public class WalletTransactionHandler : IWalletTransactionHandler
    {
        ...
        /// </summary>
        static readonly Money PretendMaxFee = Money.Coins(1);
        ...
        readonly IWalletFeePolicy walletFeePolicy;
        ...
        public void FundTransaction(TransactionBuildContext context, Transaction transaction)
        {
            ...
            var newTransaction = BuildTransaction(context);
        ...
        public (Money maximumSpendableAmount, Money Fee) GetMaximumSpendableAmount(
            WalletAccountReference accountReference, FeeType feeType, bool allowUnconfirmed)
        {
            ...
            var maxSpendableAmount = this.walletManager
        ...
        /// <summary>
        ///     Loads all the private keys for each of the <see cref="HdAddress" /> in
        ///     <see cref="TransactionBuildContext.UnspentOutputs" />
        /// </summary>
        ...
        protected void AddSecrets(TransactionBuildContext context, IEnumerable<ICoin> coinsSpent)
        {
            ...
            var seedExtKey =
                this.walletManager.GetExtKey(context.AccountReference, context.WalletPassword, context.CacheSecret);
            ...
            foreach (Coin coinSpent in coinsSpent)
            {
                ...
                var address = context.UnspentOutputs.First(output => output.ToOutPoint() == coinSpent.Outpoint).Address;

                if (added.Contains(address))
                    continue;

                var addressExtKey = seedExtKey.Derive(new KeyPath(address.HdPath));
                var addressPrivateKey = addressExtKey.GetWif(wallet.Network);
        ...
        {
            var minValue = this.network.AbsoluteMinTxFee.HasValue
                ? Math.Max(this.network.AbsoluteMinTxFee.Value, this.network.MinTxFee)
                : this.network.MinTxFee;

            var minTrxFee = new Money(minValue, MoneyUnit.Satoshi);
            ...
            {
                var feeRate = context.OverrideFeeRate ??
                              this.walletFeePolicy.GetFeeRate(context.FeeType.ToConfirmations());
        ...
        protected void AddOpReturnOutput(TransactionBuildContext context)
        {
            ...
            var bytes = context.OpReturnRawData ?? Encoding.UTF8.GetBytes(context.OpReturnData);
    ...
    public class TransactionBuildContext
    {
        ...
        /// </summary>
        /// <remarks>
        ///     TODO: replace this with System.Security.SecureString
        ///     (https://github.com/dotnet/corefx/tree/master/src/System.Security.SecureString)
        ...
        /// <summary>
        ///     The raw data to be added as an extra OP_RETURN transaction output, this will take precedence over
        ///     <see cref="OpReturnData" />.
        /// </summary>
