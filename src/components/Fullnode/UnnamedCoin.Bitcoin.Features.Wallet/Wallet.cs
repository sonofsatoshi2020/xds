...
namespace UnnamedCoin.Bitcoin.Features.Wallet
{
    ...
    {
        /// <summary>
        ///     Default pattern for accounts in the wallet. The first account will be called 'account 0', then 'account 1' and
        ///     so on.
        /// </summary>
        ...
        {
            var accountRoot = this.AccountsRoot.SingleOrDefault();
        ...
        public IEnumerable<TransactionData> GetAllTransactions()
        {
            ...
            foreach (var txData in accounts.SelectMany(x => x.ExternalAddresses).SelectMany(x => x.Transactions))
                yield return txData;

            foreach (var txData in accounts.SelectMany(x => x.InternalAddresses).SelectMany(x => x.Transactions))
                yield return txData;
        }
        ...
        public IEnumerable<Script> GetAllPubKeys()
        {
            ...
            foreach (var script in accounts.SelectMany(x => x.ExternalAddresses).Select(x => x.ScriptPubKey))
                yield return script;

            foreach (var script in accounts.SelectMany(x => x.InternalAddresses).Select(x => x.ScriptPubKey))
                yield return script;
        }
        ...
        {
            var accounts = GetAccounts(accountFilter);

            var allAddresses = new List<HdAddress>();
            foreach (var account in accounts) allAddresses.AddRange(account.GetCombinedAddresses());
            return allAddresses;
        }
        ...
        /// </summary>
        /// <remarks>
        ///     The name given to the account is of the form "account (i)" by default, where (i) is an incremental index starting
        ///     at 0.
        ///     According to BIP44, an account at index (i) can only be created when the account at index (i - 1) contains at least
        ///     one transaction.
        /// </remarks>
        ...
        ///     The index at which an account will be created. If left null, a new account will be created
        ///     after the last used one.
        /// </param>
        /// <param name="accountName">
        ///     The name of the account to be created. If left null, an account will be created according to
        ///     the <see cref="Wallet.AccountNamePattern" />.
        /// </param>
        /// <returns>A new HD account.</returns>
        public HdAccount AddNewAccount(string password, DateTimeOffset accountCreationTime, int? accountIndex = null,
            string accountName = null)
        {
            Guard.NotEmpty(password, nameof(password));

            var accountRoot = this.AccountsRoot.Single();
            return accountRoot.AddNewAccount(password, this.EncryptedSeed, this.ChainCode, this.Network,
                accountCreationTime, accountIndex, accountName);
        }
        ...
        public virtual ISecret GetExtendedPrivateKeyForAddress(string password, HdAddress address)
        {
            ...
            if (!ContainsAddress(address)) throw new WalletException("Address not found on wallet.");

            // get extended private key
            var privateKey = HdOperations.DecryptSeed(this.EncryptedSeed, password, this.Network);
        ...
        public IEnumerable<UnspentOutputReference> GetAllSpendableTransactions(int currentChainHeight,
            int confirmations = 0, Func<HdAccount, bool> accountFilter = null)
        {
            var accounts = GetAccounts(accountFilter);

            return accounts.SelectMany(x =>
                x.GetSpendableTransactions(currentChainHeight, this.Network.Consensus.CoinbaseMaturity, confirmations));
        }

        /// <summary>
        ///     Lists all unspent transactions from all accounts in the wallet.
        ///     This is distinct from the list of spendable transactions. A transaction can be unspent but not yet spendable due to
        ///     coinbase/stake maturity, for example.
        /// </summary>
        /// <param name="currentChainHeight">Height of the current chain, used in calculating the number of confirmations.</param>
        /// <param name="confirmations">The number of confirmations required to consider a transaction spendable.</param>
        /// <param name="accountFilter">An optional filter for filtering the accounts being returned.</param>
        /// <returns>A collection of spendable outputs.</returns>
        public IEnumerable<UnspentOutputReference> GetAllUnspentTransactions(int currentChainHeight,
            int confirmations = 0, Func<HdAccount, bool> accountFilter = null)
        {
            var accounts = GetAccounts(accountFilter);
        ...
        {
            var allTransactions = GetAllTransactions().ToList();

            // Get a list of all the inputs spent in this transaction.
            var inputsSpentInTransaction =
                allTransactions.Where(t => t.SpendingDetails?.TransactionId == transactionId).ToList();

            if (!inputsSpentInTransaction.Any()) throw new WalletException("Not a sent transaction");

            // Get the details of the spending transaction, which can be found on any input spent.
            var spendingTransaction = inputsSpentInTransaction.Select(s => s.SpendingDetails).First();
            ...
            IEnumerable<TransactionData> changeOutput = allTransactions.Where(t =>
                t.Id == transactionId && spendingTransaction.Payments.All(p => p.OutputIndex != t.Index)).ToList();

            var inputsAmount = new Money(inputsSpentInTransaction.Sum(i => i.Amount));
            var outputsAmount =
                new Money(spendingTransaction.Payments.Sum(p => p.Amount) + changeOutput.Sum(c => c.Amount));
    ...
    public class AccountRoot
    {
        ...
        public HdAccount GetFirstUnusedAccount()
        {
            ...
            var unusedAccounts = this.Accounts.Where(Wallet.NormalAccounts).Where(acc =>
                !acc.ExternalAddresses.SelectMany(add => add.Transactions).Any() &&
                !acc.InternalAddresses.SelectMany(add => add.Transactions).Any()).ToList();
        ...
        ///     The name given to the account is of the form "account (i)" by default, where (i) is an incremental index starting
        ///     at 0.
        ///     According to BIP44, an account at index (i) can only be created when the account at index (i - 1) contains
        ///     transactions.
        ///     <seealso cref="https://github.com/bitcoin/bips/blob/master/bip-0044.mediawiki" />
        /// </remarks>
        ...
        /// <param name="accountName">
        ///     The name of the account to be created. If left null, an account will be created according to
        ///     the <see cref="AccountNamePattern" />.
        /// </param>
        /// <returns>A new HD account.</returns>
        public HdAccount AddNewAccount(string password, string encryptedSeed, byte[] chainCode, Network network,
            DateTimeOffset accountCreationTime, int? accountIndex = null, string accountName = null)
        {
            Guard.NotEmpty(password, nameof(password));
            ...
            if (hdAccounts.Any(a => a.Index == accountIndex || a.Name == accountName))
                throw new WalletException(
                    $"An account at index {accountIndex} or with name {accountName} already exists.");
            ...
            var newAccount = CreateAccount(password, encryptedSeed, chainCode, network, accountCreationTime,
                accountIndex.Value, accountName);
            ...
            int newAccountIndex, string newAccountName = null)
        {
            ...
            var accountHdPath = HdOperations.GetAccountHdPath(this.CoinType, newAccountIndex);
            var privateKey = HdOperations.DecryptSeed(encryptedSeed, password, network);
            var accountExtPubKey = HdOperations.GetExtendedPublicKey(privateKey, chainCode, accountHdPath);
        ...
        public HdAccount AddNewAccount(ExtPubKey accountExtPubKey, int accountIndex, Network network,
            DateTimeOffset accountCreationTime)
        {
            ...
            var accountHdPath = HdOperations.GetAccountHdPath(this.CoinType, accountIndex);
    ...
    public class HdAccount
    {
        ...
        {
            return GetFirstUnusedAddress(false);
        }
        ...
        {
            return GetFirstUnusedAddress(true);
        }
        ...
        /// </summary>
        /// <returns>An unused address</returns>
        HdAddress GetFirstUnusedAddress(bool isChange)
        {
            ...
            if (!unusedAddresses.Any()) return null;
        ...
        public HdAddress GetLastUsedAddress(bool isChange)
        {
            ...
            if (!usedAddresses.Any()) return null;
        ...
        public IEnumerable<TransactionData> GetTransactionsById(uint256 id)
        {
            ...
            var addresses = GetCombinedAddresses();
        ...
        public (Money ConfirmedAmount, Money UnConfirmedAmount) GetBalances()
        {
            ...
            var confirmed = allTransactions.Sum(t => t.GetUnspentAmount(true));
            var total = allTransactions.Sum(t => t.GetUnspentAmount(false));
        ...
        /// </summary>
        /// <remarks>
        ///     Returns a collection because a transaction can be contained in a change address as well as in a receive address (as
        ///     a spend).
        /// </remarks>
        ...
        public IEnumerable<HdAddress> GetCombinedAddresses()
        {
            ...
            if (this.ExternalAddresses != null) addresses = this.ExternalAddresses;

            if (this.InternalAddresses != null) addresses = addresses.Concat(this.InternalAddresses);
        ...
        public IEnumerable<HdAddress> CreateAddresses(Network network, int addressesQuantity, bool isChange = false)
        {
            ...
            var firstNewAddressIndex = 0;
            if (addresses.Any()) firstNewAddressIndex = addresses.Max(add => add.Index) + 1;

            var addressesCreated = new List<HdAddress>();
            for (var i = firstNewAddressIndex; i < firstNewAddressIndex + addressesQuantity; i++)
            {
                // Retrieve the pubkey associated with the private key of this address index.
                var pubkey = HdOperations.GeneratePublicKey(this.ExtendedPubKey, i, isChange);

                // Generate the P2PKH address corresponding to the pubkey.
                var address = pubkey.GetAddress(network);
                var witAddress = pubkey.GetSegwitAddress(network);

                // Add the new address details to the list of addresses.
                var newAddress = new HdAddress
                {
                    Index = i,
                    HdPath = HdOperations.CreateHdPath(GetCoinType(), this.Index, isChange, i),
        ...
        /// </summary>
        /// <param name="currentChainHeight">
        ///     The current height of the chain. Used for calculating the number of confirmations a
        ///     transaction has.
        /// </param>
        ...
        /// <remarks>
        ///     Note that coinbase and coinstake transaction outputs also have to mature with a sufficient number of confirmations
        ///     before
        ///     they are considered spendable. This is independent of the confirmations parameter.
        /// </remarks>
        public IEnumerable<UnspentOutputReference> GetSpendableTransactions(int currentChainHeight,
            long coinbaseMaturity, int confirmations = 0)
        {
            ...
            foreach (var address in GetCombinedAddresses())
            {
                ...
                foreach (var transactionData in address.UnspentTransactions())
                {
                    int? confirmationCount = 0;
                    if (transactionData.BlockHeight != null)
                        confirmationCount = countFrom >= transactionData.BlockHeight
                            ? countFrom - transactionData.BlockHeight
                            : 0;

                    if (confirmationCount < confirmations)
                        continue;

                    var isCoinBase = transactionData.IsCoinBase ?? false;
                    var isCoinStake = transactionData.IsCoinStake ?? false;
    ...
    public class HdAddress
    {
        ...
        /// </summary>
        /// <remarks>
        ///     This is typically only used for mining, as the valid script types for mining are constrained.
        ...
        ///     The script itself is of the format: {pubkey} OP_CHECKSIG
        /// </remarks>
        ...
        {
            if (this.Transactions == null) return new List<TransactionData>();
    ...
    public class TransactionData
    {
        ...
        public Money GetUnspentAmount(bool confirmedOnly)
        {
            ...
            if (IsSpent() || confirmedOnly && !IsConfirmed()) return Money.Zero;
