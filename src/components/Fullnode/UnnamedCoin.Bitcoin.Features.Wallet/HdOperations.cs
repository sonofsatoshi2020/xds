...
namespace UnnamedCoin.Bitcoin.Features.Wallet
{
    ...
    public class HdOperations
    {
        ...
        public static PubKey GeneratePublicKey(string accountExtPubKey, int index, bool isChange)
        {
            ...
            var extPubKey = ExtPubKey.Parse(accountExtPubKey).Derive(keyPath);
        ...
        public static ISecret GetExtendedPrivateKey(Key privateKey, byte[] chainCode, string hdPath, Network network)
        {
            ...
            var addressExtKey = seedExtKey.Derive(new KeyPath(hdPath));
            var addressPrivateKey = addressExtKey.GetWif(network);
        ...
        public static ExtPubKey GetExtendedPublicKey(Key privateKey, byte[] chainCode, string hdPath)
        {
            ...
            var seedExtKey = new ExtKey(privateKey, chainCode);
            var addressExtKey = seedExtKey.Derive(new KeyPath(hdPath));
            var extPubKey = addressExtKey.Neuter();
            return extPubKey;
        }
        ...
        /// <remarks>
        ///     Refer to <seealso cref="https://github.com/bitcoin/bips/blob/master/bip-0044.mediawiki#path-levels" /> for the
        ///     format of the HD path.
        /// </remarks>
        ...
        public static int GetCoinType(string hdPath)
        {
            ...
            var pathElements = hdPath.Split('/');
            if (pathElements.Length < 3)
                throw new FormatException($"Could not parse CoinType from HdPath {hdPath}.");

            var coinType = 0;
            if (int.TryParse(pathElements[2].Replace("'", string.Empty), out coinType)) return coinType;
        ...
        public static bool IsChangeAddress(string hdPath)
        {
            ...
            var result = 0;
            if (int.TryParse(hdPathParts[4], out result)) return result == 1;
